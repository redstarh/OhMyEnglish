# 폐기 스냅샷 — 2026-08-29 이관 · **파일은 2026-09-05에 제거됐다**

> ⚠️ **이 폴더의 3개 HTML은 2026-09-05에 제거됐다.** 이 README만 남아 **무엇이 왜 폐기됐는지의
> 기록**으로 쓴다. 판정 기준은 둘을 함께 만족하는 것이었다 — **mtime 2일 이상 경과 + 보관소 밖
> 참조 0건**(`grep -rIl` 확인). 전부 git 추적 파일이라 **바이트는 이력에 남는다**:
> `git log --diff-filter=D -- docs/backup/2026-08-29-superseded/`로 삭제 커밋을 찾고
> `git show <커밋>^:<경로>`로 원문을 꺼낸다. 정리 근거는
> `docs/backup/2026-09-05-superseded/README.md`가 소유한다.
>
> 같은 판정으로 `../status-report-2026-08-25.html`·`../status-report-2026-08-25-architecture.html`
> (더 이른 개정판)과 `handoff/backup/2026-08-30/HANDOFF-v1.1-implementation.md`도 함께 제거됐다.

여기 있던 3개 파일은 **2026-08-25 상태 보고 세트**(Part 1·2·3)였다. 서로 상대 링크로 묶여 있어
**함께** 옮겼다 — 한 개만 옮기면 나머지의 "← Part 1" 링크가 깨졌다.

대체 문서: `status-report-2026-08-29.html` → 그것도 2026-09-05에 폐기됐다. **현행은
`docs/status-report-2026-09-05.html`**이고, 옮겨진 사본은
`docs/backup/2026-09-05-superseded/status-report-2026-08-29.html`이다.
