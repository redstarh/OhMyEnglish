...
3. **`analysis_jobs_target_matches_job_type`** 에 그 분기를 더한다 —
   ...
   함정을 이미 적었다.
4. **`learning_sessions_mode_check`** 에 `'scenario_intake'` 를 더한다. 014 가 `pronunciation` 을
   더한 것과 같은 형태다. ⛔ 이 값이 없으면 §5 의 흐름 1번을 만들 수 없다 — 진입을 가릴 표지가
   `learning_source` 로는 서지 않는다(그 절이 근거를 갖는다).
...
1. 세션이 **`mode='scenario_intake'`** 로 열린다.

⛔ **`learning_source='additional'` 로 이 세션을 가릴 수 없다 — 2026-09-12 에 반증됐다.**
`app/frontend/app/page.tsx` 의 추가 학습 메뉴 여섯 가운데 **다섯이 `source: "additional"`** 이고
그중 **셋(자유 대화 · 약점 패턴 집중 · 질문 답변 5개)이 `mode` 를 갖지 않아 서로 구별되지
않는다**(세션 `ohmyenglish-19` 가 코드로 확인해 알려 줬다). ⇒ 그 값을 조건으로 쓰면 **자유 대화와
약점 패턴 세션에도 질문 5개 지시가 실린다.** ⚠️ 그리고 게이트는 그것에 침묵한다 — 단정이
「`additional` 이면 실린다」로만 쓰이면 **항진명제**가 되어 오배치를 잡지 못한다.

⇒ **`mode` 값역에 값 하나를 더한다.** 014 가 `pronunciation` 을 더한 것이 선례이고, 결정 67 이
세운 「세션 행이 자기 진입을 적는다」 형태와 일관된다. 프런트는 그 메뉴의 `entry` 에
`mode: "scenario_intake"` 를 더하면 된다(`?mode=` 경로가 이미 있다).
...
- ⚠️ **`nova.py` 의 프롬프트에 질문 5개 지시를 싣는 자리가 필요하다.** 그 파일을 소유한 갈래
  (`TASK-128` · 세션 `ohmyenglish-19`)와 2026-09-12 에 조율했고 **먼저 해도 된다**는 답을 받았다.
  그쪽이 그 시점에 착수하지 않았고 시점을 약속할 수 없다고 했으므로 기다리지 않는다.
- ⛔ **유일한 실제 겹침은 `factory.create_voice_adapter` 의 인자다.** 그쪽 `TASK-128.1`·`.2` 가
  그 시그니처를 바꿀 예정이고(`pronunciation_sound: str | None` → `pronunciation_mode: bool`),
  이 설계도 질문 블록 재료를 그 자리로 넘긴다. ⇒ **나중에 하는 쪽이 현재 상태를 읽고 맞춘다.**
  `build_system_prompt` 의 블록 목록은 그쪽 범위가 아니다.
- ⛔ **조립 규약 둘을 지킨다** (그 갈래가 근거와 함께 넘겨 준 것이다):
  ⑴ **재료를 인자로 받고 기본값을 두지 않는다** — `build_system_prompt` docstring 의 C-1 이 그것이고,
  기본값을 두면 호출부가 재료를 빠뜨려도 조용히 통과해 **「질문이 안 실린 세션」이 정상처럼 보인다.**
  ⑵ **「실을지 말지」의 판단을 조립기 안에 두지 않는다** — `factory.py` 주석이 같은 이음매를 명시한다
  (*"어느 소리를 오늘의 초점으로 삼을지는 제품 정책이고 소켓 계층이 정한다"*). ⇒ 조립기가 `mode` 를
  읽어 판단하지 않고 **소켓이 읽어 데이터로 넘긴다.**
- ⚠️ **`_SOUND_INSTRUCTION` 의 「칸을 더 열지 마라」는 이 경우에 걸리지 않는다** — 그 못박음은 tool
  보고 계약(*"무엇을 어느 이름으로 보고할지가 한 곳에서만 정해진다"*)에 국한되고 질문 5개 블록은 그
  계약과 무관하다. 그 갈래가 직접 확인해 줬다. ⇒ **칸을 여는 것이 아니라 블록을 하나 더 붙인다** —
  `build_system_prompt` 가 이미 그렇게 조립한다(놓친 소리 목록 → `Today's setting:` → 계획 블록).
...
**대안 3 — 새 `mode` 값을 만들지 않고 `learning_source='additional'` 로 진입을 가른다.** 값역을
안 늘린다.
⛔ **기각 — 반증됐다.** 추가 학습 메뉴 여섯 중 다섯이 그 값을 쓰고 그중 셋이 `mode` 를 갖지 않아
서로 구별되지 않는다(§5 가 그 목록을 갖는다). ⇒ 자유 대화·약점 패턴 세션에 질문 지시가 새고,
「`additional` 이면 실린다」는 단정은 **항진명제**라 그 오배치를 잡지 못한다.
⚠️ 이 안은 팀리드가 처음 고른 것이고 `nova.py` 소유 갈래가 코드로 반증했다 — **구현 전에 잡혔다.**

**대안 4 — 별도 표(`user_scenarios`)를 만든다.** 시드와 완전히 격리된다.
