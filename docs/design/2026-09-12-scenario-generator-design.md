...
### 정제 2026-09-20 (`TASK-259`) — 재시도는 **사유에 따라** 갈린다

⛔ **위 「5회까지 재시도」를 모든 거부에 적용하지 않는다.** 재시도의 전제는 「다른 표본이 성공할 수
있다」인데, 그 전제가 **거짓인 거부**가 있다.

| 사유 | 재시도 | 근거 |
|---|---|---|
| `category` 가 명시적 `null`(`ScenarioNoStage`) | **하지 않음 — 첫 시도에 `failed`** | 프롬프트가 지시한 답이고 입력이 같아 같은 답이 온다 |
| 전사문이 비었다 | **하지 않음** | 같은 이유(입력이 같다) |
| 그 밖의 계약 위반(제목·지시문·중복·값역 밖) | 5회까지 | 다른 표본이 계약을 지킬 수 있다 |

**실측이 이 정제를 부른 근거다**(2026-09-20 · 격리 DB).

1. 무대를 말하지 않은 5회 대화 전사문으로 실물 모델을 **8회** 불러 **8회 전건** `category=null` 이
   왔고 8회 전건이 파서에 거부됐음. ⇒ 이 갈래는 드문 사고가 아니라 **결정론적 경로**임.
2. 그 상태에서 큐가 5회까지 물었음 — **유료 호출 5건** · 백오프 1·2·3·4분. 첫 호출에서 결론이 정해진
   사안에 **4건이 낭비**됨. 고친 뒤 같은 실험을 다시 돌려 **호출 1건 · `attempts` 1 · `failed`** 를
   관측했음.
3. 증거: `tests/harness/runs/2026-09-20-task259-null-retry.md` · 탐침
   `tests/harness/scenario_null_probe.py`.

⛔ **`done` 으로 닫지 않는 이유**: 아무것도 저장하지 않으므로 `done` 은 흔적 없는 성공이 되고,
「왜 무대가 없는가」를 나중에 물을 자리가 사라진다. `process_weekly` 가 `done` 으로 닫는 것은 **빈
리포트 행을 남기기** 때문이고 여기는 남길 행이 없다 — 두 경우가 다르다.
⚠️ **남은 자리 하나**: 학습자에게 「무대를 만들지 못했다」를 알리는 화면이 없다. 그 결정은 요구사항
쪽이므로 여기서 발명하지 않는다.
⛔ 큐 쪽 장치는 `services/jobs.fail_terminally`·`report_terminal_failure` 이고 그 함수의 docstring 이
「왜 `fail_or_retry` 와 갈라 두는가」를 소유한다.
