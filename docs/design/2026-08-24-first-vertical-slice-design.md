...
> ⛔ **이 문서의 `database-schema.md:NNN` 인용 5자리는 «2026-08-24 시점»의 줄 번호이고 지금은 밀렸다**
> (`TASK-113` 실측 2026-09-12 — 그 문서에 절이 삽입될 때마다 뒤 줄이 밀린다 · 함정 `H-H`).
> **번호를 갱신하지 않는다** — 아래 「리뷰 이력」과 §의 표들은 **그 회차의 기록**이고 고치면 무엇을
> 근거로 판정했는지가 바뀐다. 다섯 자리가 실제로 가리키는 것을 2026-09-12에 직접 조회했다:
> `:49`(복습 간격 14일 — **이 문서의 판정대로 삭제돼 지금은 없다.** 1·3·7일은 「복습 우선순위」 절) ·
> `:47`(우선순위 공식 → **「복습 우선순위」 절**) · `:59`(녹음 「즉시 접근 불가」 → **「개인정보 원칙」
> 절**) · `:35`(영문 카테고리 코드 `verb_tense` → **`error_patterns` 절의 카테고리 표**).
> ⚠️ **살아 있는 문서에서 이 문서를 다시 인용할 때는 줄 번호를 옮겨 적지 말고 절 이름을 쓴다.**
...
`analyze_utterance`가 턴마다 도는 것이 채택 아키텍처(`nova-sonic-claude-architecture.md:87-88`)다. 확정 전사문 저장 직후 등록되고(`utterance_id` 대상), 오류 카테고리·패턴 키를 산출해 `error_patterns` upsert + `error_occurrences` insert를 수행한다. 세션 종료를 기다리지 않는다.

**`summarize_session`(세션 총평, `nova-sonic-claude-architecture.md:99`)은 다음 슬라이스로 연기한다** (4차 리뷰 반영 — 캡틴의 MVP 지시). 근거: `HANDOFF.md §2`의 첫 슬라이스 흐름은 `error_patterns 저장 → 결과 화면`까지이고, 결과 화면은 교정 상위 2개(§5.5)만으로 성립한다. 연기하면 준비 대기 재큐 로직·AC11·총평 UI가 통째로 빠져 큐 명세가 얇아진다. 단 `analysis_jobs` 스키마(§6.1a)는 `session_id` 컬럼과 `job_type` CHECK를 그대로 두어 다음 슬라이스에서 마이그레이션 없이 켠다 — 스키마는 미리, 동작은 나중에.
...
- 같은 패턴의 복수 발생은 행이 여러 개인 것 자체로 보존된다. 위치는 기존 `original_span`(원문 조각 텍스트)으로 제시한다. ~~문자 오프셋(`span_start`/`span_end`) 저장~~은 철회 — UI 요구가 근거 문서에 없고, LLM은 정확한 문자 오프셋 산출에 취약해 검증 로직만 늘린다 (4차 리뷰).
...
**확정 전사문 저장과 `analyze_utterance` 등록은 한 트랜잭션이다.** 분리하면 그 사이 크래시에서 분석되지 않는 발화가 영구히 남는다. (다음 슬라이스에서 `summarize_session`을 켤 때도 같은 원칙 — 세션 종료 기록과 등록을 한 트랜잭션으로.)

### 5.4 claim과 lease

- 워커는 FastAPI 기동 시 `asyncio` 루프로 뜬다. **동시성은 1이다** — 단일 사용자 로컬 도구라 병렬 처리 이유가 없고, §5.4의 잠금 규칙들은 동시성 1에서도 재기동·이중 기동을 방어하기 위한 것이다
- 인덱스 `(status, available_at)`
- 재시도 백오프: `available_at = now() + attempts × 1분` (수치는 설계자 기본값 — 근거 문서 없음, 구현 중 조정 가능)
- **짧은 트랜잭션에서 `FOR UPDATE SKIP LOCKED`로 claim → 트랜잭션을 닫고 Claude 호출 → 별도 트랜잭션에서 완료 처리.** Claude 호출을 트랜잭션 안에 두면 커넥션을 수십 초 잡는다
- **lease 기간 5분** (Claude 호출 상한 대비 여유값으로 설계자가 정한 기본값 — 근거 문서 없음). claim 조건은 `status='pending' and available_at <= now()` 또는 `status='running' and locked_at < now() - interval '5 minutes'`(회수). 이 규칙이 없으면 오래 걸리는 정상 호출을 다른 워커가 중복 claim한다
...
- **`attempts >= 5`면 terminal `failed`로 보내고 재시도를 멈춘다** (상한 5도 설계자 기본값). `last_error`를 남긴다. 상한이 없으면 영구 재시도한다
...
| # | 결정 |
|---|---|
| B2 | ~~`impact_score` 저장~~ → **3단계(복습 우선순위)로 연기** (4차 리뷰). 우선순위 공식(`database-schema.md:47`)의 소비자는 복습 과제 생성인데 그것이 3단계다. 첫 슬라이스의 상위 2개 선정(§5.5)은 severity·confidence만 쓴다 |
...
`error_patterns` / `error_occurrences` 분리는 적절한 정규화다. 재사용 단위(`pattern_key`, `target_form`)는 패턴에, 원문 조각과 실제 교정(`original_span`, `correction`)은 발생에 둔다. `frequency`·`last_seen_at`은 발생에서 파생되는 캐시이므로 **분석 완료 트랜잭션에서 원자적으로 갱신하는 것을 규약으로 한다.** `last_seen_at`은 `error_occurrences.created_at`이 아니라 **`utterances.created_at`(발화 시각) 기준**으로 산정한다 — replace(§5.2)가 occurrence를 새 `created_at`으로 재삽입하므로, occurrence 시각을 쓰면 재시도마다 last_seen_at이 현재로 당겨진다.
...
- **결과 저장 후 status 갱신 전 크래시**: 가장 까다로운 실패다. 같은 작업이 재실행되지만 **발화 단위 replace**(§5.2)가 이전 결과를 새 결과로 교체하므로 중복이 생기지 않고, `frequency` 재계산 규약이 부풀림을 막는다.
...
- **작업 등록 누락**: 확정 전사문 저장 ↔ `analyze_utterance` 등록이 한 트랜잭션이다 (§5.3). 분리하면 그 사이 크래시에서 분석되지 않는 발화가 영구히 남는다.
...
- **읽기 전 갱신 보장**: 결과 화면의 조회 대상과 확정 표시 시점은 §5.5가 정본이다 — 세션 occurrence 조인, 모든 `analyze_utterance` terminal 후 확정 표시. 완료를 가정하고 읽으면 빈 결과를 확정 결과처럼 보여준다.
- **작업 등록 순서**: `analyze_utterance`는 턴마다 확정 전사문 저장과 함께 등록된다 — 세션 종료를 기다리지 않는다 (`nova-sonic-claude-architecture.md:87-88`).
- **정리 순서**: WebSocket 종료 → Nova 스트림 종료 → 세션 종료 기록(`ended_at` + `status`). 순서가 뒤바뀌면 종료 시각이 없는 세션이 남는다.
...
**AC7 — 재시도 멱등성**
Given 결과 저장은 끝났지만 `status` 갱신 전에 워커가 죽어 같은 작업이 재실행될 때, When 재분석이 완료되면, Then 발화 단위 replace(§5.2)로 `error_occurrences`가 최신 결과 집합 하나만 남고 `frequency`가 부풀지 않는다.
...
**AC11 — (다음 슬라이스로 이동)**
세션 요약 AC는 `summarize_session` 연기(§5.1)와 함께 다음 슬라이스로 옮긴다. 번호는 유지한다.
