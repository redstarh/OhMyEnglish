...
1. ~~**발음 패턴의 `next_review_at`은 영구히 null이고 `review_tasks` 행도 생기지 않는다.**~~
   ⛔ **철회됐다 (2026-09-08, `TASK-44` · 커밋 `9df2369`).** 발음 패턴은 이제 `next_review_at`을
   받고 `review_tasks` 행을 얻으며 **복습 목록을 통해 초점 허용 집합에도 들어온다.** R11-9는
   §4.4의 별도 경로가 아니라 **복습 경로로** 충족된다. 정본은
   `docs/design/2026-09-08-pronunciation-review-cycle-design.md`.
   ⚠️ **이 줄이 「버그가 아니다」로 남아 있던 동안 다음 세션이 조사를 되돌릴 위험이 있었다** —
   코드 리뷰 HIGH-2가 그것을 잡았다. (당시 근거였던 설계서 §4.1 「구현이 채운 공백」 4번에도
   철회 상자를 얹었다.)
...
| 3 | Step 2의 red 가 "현재 SQL이 수준을 안 보므로 **A2 시나리오가 붙는다**"로 나타난다 | **테스트 DB에는 시나리오가 0행이다**(직접 조회). `recreate_database`는 `db/migrations/*.sql`만 적용하고 `migrate.py`의 `SEED_SCENARIOS`를 **부르지 않는다** — 시드 3행은 **dev DB에만** 있다. 그래서 `B1` 하나만 심으면 **현재 SQL도 그것을 고르고 테스트가 처음부터 초록**이다(전역 제약이 금지한 "통과하지만 이유가 틀린 테스트") | 수준이 **안 맞는** 시나리오(`A2`)를 **더 이른 `created_at`으로 함께 심는다.** 그러면 현재 SQL은 `A2`를, 고친 SQL은 `B1`을 골라 red 가 실제로 관측된다. ⚠️ **이 배치만으로는 부족했다**(2026-09-05 리뷰 I-1): red 는 실제로 났지만 그것은 "수준 무시 + 가장 이른 행"과의 구분만 고정하고, **"수준 무시 + 가장 최신 행"** 구현을 통과시킨다. 실제 배치(불일치 행을 양쪽 끝에 두는 3행)와 조합별 누출 실측은 **`tests/unit/test_sessions.py`의 두 시나리오 테스트 주석**이 소유한다 — 이 칸을 테스트 설계 표준으로 읽지 말 것 |
...
`chronic.py`의 첫 소비자이므로 여기서 테스트로 덮는다.

✅ **구현 후 정정 (2026-09-04) — LOW-12의 주장 3개 중 2개가 틀렸다.** 구현자가 뮤테이션으로 확인하고
메인이 SQL로 재확인했다. 아래는 **실측된 사실**이고, 원래 이연 항목의 서술(슬라이스 1 리뷰가 남긴 것)은
이 정도로 정정된다:
1. **정렬(`order by pattern_key`)** — 원래 주장대로 무보호였고 테스트가 실제로 덮는다. ✅
2. **최대 공백** — 원래 주장한 "`lag`의 tie-break"는 **뮤테이션해도 green이다**(테스트 데이터에 동시각이
   없어 tie-break가 갈리지 않는다). 실제로 red가 나는 것은 `max`→`min`이다. 테스트는 유효하고
   **주석의 뮤테이션 지목만 틀렸다.**
3. **`where prev_at is not null`** — **결과를 바꾸지 않는 중복 방어다.** 그 값이 `left join`되고
   `max()`가 NULL을 무시하므로 필터가 있든 없든 결과가 같다(직접 확인: 다건·1건 모두 동일).
   즉 "무보호"가 아니라 **애초에 보호할 동작이 없다.** 슬라이스 1의 strict `>` 지적이 같은 결말이었다.

**그래서 처리 원칙**: `chronic.py`는 **고치지 않는다**(동작이 옳다). 테스트는 **유지한다**(참인 동작을
단정한다). 고치는 것은 **주석뿐**이다 — 뮤테이션을 잘못 지목한 문구를 남기면 다음 사람이 그것을
보호 근거로 착각한다. **LOW-14**(`users.timezone`이 CHECK 없는
...
⚠️ **Task 6 리뷰에서 이 태스크로 넘어온 것 2건** (2026-09-04, 프로브로 확인된 사실):

**① 지어낸 `pattern_id`를 거부하는 가드를 이 태스크가 만든다 — 조용한 데이터 손상을 막는 유일한 장치다.**
Task 6이 프롬프트에 id를 실어 "그대로 복사하라"고 요구하게 고쳤지만, **문구만으로는 부족하다**:
모델이 **형식만 맞는 UUID를 지어내면** pydantic도 통과하고(`UUID` 타입만 본다) DB도 통과한다
(`focus_pattern_ids uuid[]`에 **FK가 없다**). 그러면 존재하지 않는 패턴 id가 **조용히 저장되고**
초점·복습 조회가 영구히 어긋난다 — 실패보다 나쁘다.

**고침**: `parse_plan`에 **허용된 id 집합**을 넘겨 `focus[].pattern_id`가 그 집합에 속하는지 검사하고,
아니면 `PlanValidationError`로 거부한다. 그 집합은 **프롬프트에 실린 것과 같아야 한다** —
`{r.pattern_id for r in data.due_reviews} | {m.pattern_id for m in data.chronic}`.
**이 태스크가 소유하는 이유**: 그 집합을 아는 것은 `parse_plan`의 **호출자**이고 그것이 여기다.
`parse_plan`이 이미 `current_level`을 받아 "출력만으로는 판정할 수 없는 것"을 검사하는 선례가 있으니
같은 자리에 얹는다. 원칙도 이미 세워져 있다 — `InvalidTimezoneError` docstring의
"**틀린 계획보다 실패가 낫다**". ⚠️ `models/plan.py`의 시그니처를 넓히는 것이 이 태스크의 범위에
포함된다(브리프 파일 목록에 그 파일을 더한다).

**② 콜드스타트 — 후보가 0건이면 Claude를 부르지 않는다.**
첫 세션 직후에는 복습 목록과 만성 지표가 **둘 다 빌 수 있다**(occurrence 가 아직 없다). 그때 프롬프트는
"위 목록에서 골라라"라고 말하는데 `PlanOutput.focus`는 **최소 1개**를 요구한다 → 모델이 무엇을 내도
거부되거나 지어낸 id가 위 가드에 걸린다. **결과는 "계획 없음 → 폴백"으로 설계된 동작이지만**
(§3.3), **확실히 거부될 호출에 비용을 쓴다.**
**고침**: `due_reviews`와 `chronic`이 **둘 다 비면** Claude를 부르지 않고
`report_failure(pool, job, "no focus candidates yet")`로 종결한다. ⚠️ **폴백을 막지 않는다** — 계획 행이
없으면 세션 시작이 시나리오 뱅크로 떨어지는 것이 §3.3이 정한 동작이고, 그 경로는 그대로다.
`logger.info`가 아니라 큐의 사유로 남기는 이유: 나중에 "왜 계획이 없었나"를 job 이력에서 읽을 수 있다.

⚠️ **Task 5 리뷰에서 이 태스크로 넘어온 것 1건** (2026-09-04, 프로브로 확인된 사실):
**`level.action`이 실제 이동과 어긋나도 출력 계약은 통과한다.** 확인된 세 조합 —
`action="up"`인데 목표가 현재와 같음 · `action="keep"`인데 A2→B1 · **`action="down"`인데 상향 A2→B1**.
Task 5의 검증은 `action` 값역(`keep`/`up`/`down`)과 **한 단계 제약**만 보고 **둘의 정합은 보지 않는다.**

**이 태스크가 그것을 소유한다** — `users.current_level`을 실제로 갱신하는 곳이 여기이기 때문이다.
갱신에 쓰는 값이 `level.target_level`인지 `action`인지 먼저 정하고, **하나만 정본으로 삼아라.**
권고: **`target_level`을 정본으로 쓴다**(그것이 한 단계 제약과 세 값 일치 검증을 이미 통과한 값이다).
`action`은 사람이 읽는 라벨로만 쓰고, **어긋나면 저장을 거부하지 말고 `learner_notes`의 사유에
그대로 남긴다** — 모델이 라벨을 잘못 붙인 것이 계획 전체를 버릴 이유는 아니다.
⚠️ 다만 **어긋남을 조용히 삼키지는 마라**: `logger.warning`으로 한 줄 남긴다(관측 없이 지나가면
프롬프트가 라벨을 잘못 유도하고 있다는 신호를 잃는다).
...
⛔ **이 제약은 철회됐다 (2026-09-08, `TASK-44` · `9df2369`).** `next_review_at`이 채워지므로
**복습 목록 경로로도 검증할 수 있고, 그 경로가 더 강한 검증이다** — 초점 허용 집합이
`{복습 예정} ∪ {만성}`이라 발음이 실제로 초점이 될 수 있는지는 그 경로만 잰다. AS11을 쓰는
태스크(`TASK-48`)는 두 경로를 함께 잰다: §4.4 발음 절 분리(위 Then) + 복습 목록 진입.
```
...
7. **§3.4 「남는 틈 1건」의 폴백 서술을 정정한다** (S2-8 재리뷰가 잡았다, 2026-09-05 — 직접 확인).
   설계서는 "세션을 끝내고 곧바로 다시 시작하면 계획 생성이 아직 끝나지 않아 **폴백으로 시작된다**"고
   적었지만 **구현은 그렇지 않다.** `load_prepared_plan`은 그 학습자의 **최신 계획 1행**을 읽으므로,
   직전 세션의 계획이 아직 없어도 **그 앞 세션의 계획이 있으면 폴백이 아니라 그 계획이 읽힌다.**
   같은 전제를 쓰는 §11 이월의 「연속 세션의 계획 공백 보완」 후보 ①("새 계획이 없으면 **폴백 대신**
   직전 계획의 남은 질문을 이어 쓴다")도 함께 고친다 — 그 후보는 이미 일어나는 일을 제안하고 있다.
   ⚠️ **코드 옆 서술이 참이고 설계서가 낡았다** — `services/sessions.py`의 `_PREPARED_PLAN_SQL` 위
   주석이 이 사실과 그 대가("같은 초점·같은 질문이 두 세션에 연속으로 나올 수 있다")를 소유한다.
   설계서를 고치지 않으면 실행 로그 §3 패턴 ③(설계서 → 계획서 → 코드로 같은 누락이 전달된다)이
   그대로 재현된다.
