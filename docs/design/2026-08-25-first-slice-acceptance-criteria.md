...
- **U2. 결과 화면 — 5상태** (R2 매핑 표가 단일 계약): `분석 중` / `확정`(교정 최대 2개: 원문 → 교정문 → 한 줄 이유) / `부분 실패`("분석하지 못한 발화가 있습니다" + 재시도 불가 안내 **+ 성공분 교정 병존**, §5.5 · 문구는 `TASK-57`이 고쳤다) / `연결 실패`(G2 연동, ≙AC10 후반부) / `분석 대상 없음`.
