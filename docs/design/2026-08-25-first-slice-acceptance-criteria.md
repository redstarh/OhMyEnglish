...
**현재 판정 (2026-09-09 · `TASK-47`): 전건 충족이 아니므로 선언하지 않는다.**
...
| 4 W/R/G AC 테스트의 red→green 증거 | ⛔ **미충족 — 단 경로가 열렸다.** **결정 48**(2026-09-09)이 **뮤테이션 KILL 확인을 등가 증거로 인정**했다(면제가 아니다 — 기준을 느슨하게 하지 않고 증거의 형태만 바꿨다). 그 기준으로 다시 재니 **기록된 KILL 확인이 사실상 0건**이다: `test_utterances.py`(W1·W3·W6)·`test_jobs.py`(W3·W4·W5)·`test_claude_schema.py`(W7)가 0건이고 다른 파일의 히트는 **테스트 이름**이거나 **SURVIVED 기록**이다. 붙이는 작업은 `TASK-73` 이 소유한다 |
