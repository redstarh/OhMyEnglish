...
**「Phase 1 AC 충족」과 「발음 기능 동작」은 다른 축이고 후자는 아직 아니다.** 발음 코칭이 지금
어떤 프롬프트 조건에서도 일어나지 않는다(실물 왕복 28회 · 조건 넷 전부 0 —
`tests/harness/runs/2026-09-10-task81-pronunciation-focus.md`). 원인은 `SYSTEM_PROMPT` 규칙 9·11
이고 그것을 푸는 작업은 `TASK-75` 가 소유한다. **이 구별 없이 이 선언을 인용하면 「선언했는데
발음이 안 된다」가 된다.**
