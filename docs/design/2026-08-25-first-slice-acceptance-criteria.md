...
- **W1. 턴 단위 비동기 분석** (≙AC3): ⛔ **2026-09-09 정정 — 「확정 전사문 저장과 `analyze_utterance` 등록이 한 트랜잭션이고」를 지웠다.** I-1(발화 조각 묶음)이 그 짝을 깼다: 저장 시점에는 이 발화가 턴의 마지막 조각인지 알 수 없으므로 `save_final_transcript` 는 job 을 걸지 않고, 등록은 **턴이 닫힐 때** `flush_pending_analysis` 가 묶음 단위로 한다. 코드가 그 사실을 이미 적어 뒀다 — *"분석 job은 여기서 걸지 않는다"* · *"I-1에서 enqueue가 빠지며 「두 write의 원자성」이라는 원래 목적은 사라졌다"*. 그래서 **지금 요구는 이것이다**: ⑴ 전사문 저장은 그 자체로 원자적이고 `sequence_no` 경쟁에서 전사문을 잃지 않는다 ⑵ 턴이 닫히면 그 묶음의 **마지막 발화**에 job 1건이 걸린다 ⑶ 그 등록이 실패해도 묶음을 잃지 않는다 — 다음 flush 가 다시 찾고, 세션이 끝난 뒤에는 `flush_ended_sessions` 가, 종료 기록 전에 죽은 세션은 `reap_orphan_sessions` → 스윕 순서가 덮는다. ⚠️ **낡은 문면이 공허한 테스트를 살려 두고 있었다** — 그 테스트는 프로덕션이 금지하는 호출 패턴(호출자 트랜잭션)에서만 참이었고 `TASK-76` 이 지웠다. 그리고 워커가 claim해 분석한 뒤 `error_patterns` upsert + `error_occurrences` insert + job 완료를 한 트랜잭션으로 커밋한다. **세션이 `status='active'`인 동안 해당 job이 `done`에 도달할 수 있다** (세션 종료 대기 없음의 관측형). **오류 0건 결과(픽스처 발화 3)도 occurrence 0행·패턴 미생성으로 정상 `done` 처리된다** — W7의 스키마 거부 대상과 구분한다 (SVG T2 empty).
...
### 선언 자리 — 여기에 적는다 (`TASK-47` 이 정함)

**Phase 1 완료 선언은 이 절에 적는다.** 규칙을 소유한 문서가 판정도 소유한다 — 둘이 떨어져 있으면
한쪽만 읽고 결론을 내게 된다. ⛔ **원장에는 상태를 두 벌 쓰지 않는다**: 원장은 후속 태스크의 상태만
갖고, 6항목 대조의 근거는 `docs/design/2026-09-09-phase1-completion-audit.md` 가 소유한다.

## ✅ Phase 1 완료 — **선언한다 (2026-09-10 · 사용자 승인)**

**아래 표의 여섯 항목이 전건 충족됐고, 사용자가 그 근거와 한계를 보고 선언을 승인했다.**
최초 대조(2026-09-09 `TASK-47`)에서 미충족이던 **항목 4 는 `TASK-73` 이, 항목 6 은 `TASK-70` 이**
닫았다. 각 항목의 근거와 한계는 표 안에 있다.

⛔ **이 선언이 뜻하지 않는 것 — 두 축을 혼동하지 않는다.**
**「Phase 1 AC 충족」과 「발음 기능 동작」은 다른 축이다.** 선언 시점(2026-09-10)에 후자는 아직
아니었다: 발음 코칭이 **어떤 프롬프트 조건에서도 일어나지 않았다**(실물 왕복 28회 · 조건 넷 전부
0 — `tests/harness/runs/2026-09-10-task81-pronunciation-focus.md`). 원인은 `SYSTEM_PROMPT` 규칙
9·11 이었고 그것을 푸는 작업은 `TASK-75` 가 소유했다.

⛔ **그 뒤에 바뀌었다 — 2026-09-19 실측으로 위 서술이 반증됐다**(`TASK-244` · 결함 `TASK-238`).
`TASK-75` 는 **2026-09-10 에 Done** 이고, 그것이 규칙 9·11 문제를 **발음 전용 모드 신설**로 풀었다
(설계 정본 `docs/design/2026-09-12-pronunciation-mode-design.md`). 실측: `mode=pronunciation`
세션 1회에서 `pronunciation` 프레임이 **1/1** 도착하고, 코치가 소리를 지목해 문장을 다시 읽고
따라 말하기를 요청했으며, `pronunciation_attempts` **1행**(`signal_source=nova_tool` ·
`target_sound=th_as_t` · `sound_check=matched`)과 `error_patterns` **1행**(`pronunciation_th_as_t`)이
생겼다(회차 기록 `tests/agent/runs/2026-09-19-b6/result.md` §7-1·§7-4).

⛔ **28회 왕복 관측을 지우지 않는 이유**: 그 시점에 참이었고, 무엇이 언제 바뀌었는지는 그 관측이
남아 있어야만 읽을 수 있다. ⚠️ **위 문장을 「발음은 미동작」으로 인용하면 지금은 거짓이다** —
2026-09-19 통합테스트가 실제로 그렇게 인용해 발음 영역을 「실패를 예상하는 자리」로 계획했고,
관측은 반대로 나왔다. **이 구별 없이 이 선언을 인용하면 어느 방향으로든 오독이 난다.**

⚠️ **판정 없이 선언하지 않는다는 규칙은 지켜졌다** — 항목 6 이 ✅ 인 것은 재확인이 **approve** 로
왔기 때문이고, 「HIGH 를 고쳤다」가 근거가 된 것이 아니다.

| 항목 | 판정 |
|---|---|
| 1 F·W·R·G·U 전 항목 pass | ✅ 충족 |
| 2 W-live 1회 | ✅ 충족 — 2026-09-09 직접 실행 `PASS: 12/12` |
| 3 E2E-S 1회 | ✅ 충족 — 스텝 1(스텁에서 구조적 관측 불가)은 미충족으로 세지 않는다. 근거는 대조 문서 §3 |
| 4 W/R/G AC 테스트의 red→green 증거 | ✅ **충족 (2026-09-10 · `TASK-73`)** — **결정 48**이 인정한 등가 증거(뮤테이션 KILL 확인)를 **AC 항목 열넷 전부**(W1~W7 · R1~R3 · G1~G4)에 붙였다. 각 항목의 대상 테스트에 **어떤 변이를 어떻게 걸어 무엇이 FAIL 했는지**를 적었고, 변이는 전부 되돌려 게이트를 다시 쟀다. ⚠️ **이전 판정(「기록된 KILL 확인이 사실상 0건」)은 파일 단위로 grep 한 결과였고, 지금 판정은 항목 단위다** — 항목마다 그것을 담는 단정 하나를 골라 무력화하는 것이 결정 48 이 요구한 형태다. ⛔ **SURVIVED 를 KILL 로 위장하지 않았다**: 어느 이웃 테스트가 같은 변이를 통과했는지 각 기록에 함께 적었다. 근거 커밋은 `2642e1b`(R) · `ba8350f`(G) · `c67f70a`(W)다 |
| 5 `/simplify` 후 재검증 | ✅ 충족 — `b915b09`(2026-08-25) |
| 6 code-reviewer Approve | ✅ **충족 (2026-09-10 · `TASK-70`)** — 리뷰어는 **codex**(`codex:codex-rescue`)다. 승인 기준은 `~/.claude/rules/common/code-review.md` 의 *Approve = CRITICAL·HIGH 0건*이고 **지금 열린 CRITICAL·HIGH 가 0건**이다. ⚠️ **이것은 「전체 범위를 한 번에 본 Approve」가 아니라 라운드 누적이다** — 그 한계를 적어 둔다: 1·2차가 Phase 1 범위를 보고 HIGH 셋을 냈고 고침 뒤 재리뷰가 **해소로 인정**했으며 그때 남은 새 지적이 `TASK-79`·`TASK-80` 이었다. 3차는 그 둘이 건드린 자리로 **범위를 좁혀**(사용자 지시) 돌렸고 `TASK-79` 는 지적 없음, `TASK-80` 에 HIGH 1건이 나왔다. 그것을 `f31ce3c` 로 고친 뒤 4차 재확인이 **approve** 했다(반례 재현 + 조합 넷 검증 + 게이트가 조합에 무관하다는 논증). ⛔ **범위를 좁힌 이유는 1·2차 판정이 나머지를 덮었기 때문이고, 전체 재리뷰를 다시 돌리는 것이 더 정확하다는 판단이 서면 이 줄을 뒤집어야 한다** |

⚠️ **캡틴 결정 B-7 이 지목했던 미충족 셋(W-live·E2E-S·실물 마이크)은 전부 닫혔다** — 항목 4·6 은
그 목록에 없었고 이 대조에서 처음 드러났다. **B-7 이 닫힌 것과 6항목 전건 충족은 다르다.**
