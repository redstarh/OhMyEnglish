...
- 작성: 2026-08-25 / 개정 1회 (critic CHALLENGE 반영)
- 상태: critic 재검증 대기
...
스텁은 §1 질문 3개에 대해 고정 답변 3건을 발행하며, 그중 2건은 같은 `article` 오류를 포함한다:
...
- **W1. 턴 단위 비동기 분석** (≙AC3): 확정 전사문 저장과 `analyze_utterance` 등록이 한 트랜잭션이고, 워커가 claim해 분석한 뒤 `error_patterns` upsert + `error_occurrences` insert + job 완료를 한 트랜잭션으로 커밋한다. **세션이 `status='active'`인 동안 해당 job이 `done`에 도달할 수 있다** (세션 종료 대기 없음의 관측형).
...
- **R2. 상태 판정** (≙AC6): 결과 API의 상태는 다음 규칙으로만 정한다 —
  - `analyze_utterance` job이 **1건 이상이고 전부 terminal**일 때만 `확정` (교정 목록 포함)
  - non-terminal이 하나라도 있으면 `분석 중` — 이때 교정 목록을 **응답에 포함하지 않는다** (잠정 목록 노출 금지, §5.5)
  - job 0건 또는 `learning_sessions.status='failed'`면 확정이 아닌 **별도 상태**(`연결 실패`/`분석 대상 없음`)를 반환한다 — 빈 결과를 확정처럼 주는 경로 차단 (SVG T2)
...
- **U1. 세션 화면**: 학습 시작/종료, 질문 3개 진행, 전사문 표시 — 부분=회색·확정=일반 (`voice-architecture.md:41`). 부분 전사문의 발생원은 G3 스텁이다.
- **U2. 결과 화면 — 4상태**: `분석 중` / `확정`(교정 최대 2개: 원문 → 교정문 → 한 줄 이유) / `부분 실패`("일부 발화는 분석하지 못했다" + 재시도 불가 안내, §5.5) / `연결 실패`(G2 연동, ≙AC10 후반부).
...
2. 공통 픽스처로 3문항 완주 — 부분 전사문(회색)→확정 전사문(일반) 전환 확인 (U1)
3. **워커를 일시 정지한 상태**에서 세션 종료 → 결과 화면 진입 → `분석 중` 확인 (결정적 절차 — 경합 회피) → 워커 재개
4. 실제 Claude 분석 완료 후 `확정` 상태에서 교정 최대 2개 확인 (공통 픽스처라 기대값 결정적)
5. **job 1건을 강제 실패**(DB에서 `failed`로 직접 갱신) → 결과 화면에서 `부분 실패` 문구 확인 (U2·R3)
...
| **Claude 클라이언트 자격증명 SigV4 전환** | §2·§4.1 "bearer 금지"로부터의 Phase 1 임시 이탈 해소 (F5의 설정 교체로 완료) |
