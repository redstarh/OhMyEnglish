...
⛔⛔ **그 상수의 «순서»가 제품 동작이다** (결정 76 · 아래 두 표는 UUID 순으로 읽기 쉽게 적은 것이고
**배열 순서가 아니다**). 신규 선택이 `created_at` 이 가장 이른 후보를 집고 그 컬럼은 배열의 삽입
순서이므로, **배열 끝에 붙인 상황은 「가장 늦게 나오는 상황」이 된다.**
⚠️ 이 문장은 실측이 불렀다 — 업무 6종을 배열 뒤에 몰아 둔 첫 판이 dev DB 실제 앱 경로 12회에서
**업무 0회**를 냈다(`tests/harness/runs/2026-09-12-task4-rotation-app-path.md` §1). 그래서 지금 배열은
`…101`·`…102`·`…103` 뒤부터 **업무·일상을 번갈아** 놓는다. 실제 배열 순서의 정본은 그 상수이고
`test_seed_interleaves_business_stages_early` 가 계약을 지킨다.
...
⛔ **위 일곱 가운데 「무엇이 먼저 오는가」를 재는 것이 없었고 그 구멍으로 결함이 지나갔다**(결정 76).
비율과 고갈이 초록인데 업무 상황이 한 번도 안 나왔다 — `test_seed_interleaves_business_stages_early`
(시드 배열 순서)와 `tests/harness/runs/2026-09-12-task4-rotation-app-path.md`(실제 앱 경로 30회)가
그 자리를 메운다. ⚠️ **비율이 맞는다는 것과 순서가 맞는다는 것은 다른 단정이다.**
