...
3. **`scenario_context`가 과거 행에서 드리프트한다.** 지금 값은 재계산 시점의 **최신** occurrence
   ...
   ⚠️ **한 칸 더 있다** (코드 리뷰 LOW-2 · 구현 후 관측): 동결 조건이 **기존 행의** 상태를 보므로
   (`when review_tasks.status = 'pending'`), `superseded`·`abandoned` 행이 재분석으로 되살아나
   `pending`이 되는 턴에는 문맥이 **옛 값으로 남고 다음 재계산에서야 갱신된다.** 즉 학습자가 그
   사이에 보는 것은 한 번 낡은 연습 문구다. 지금 고치지 않는다 — 비용이 작고, 고치려면 동결
   판정을 "새 상태"로 옮겨야 해서 `done` 행이 다시 덮이는 경로가 열린다(이 약점이 막으려던 것).
...
5. ~~**발음 패턴은 여전히 이 히스토리에 들어오지 않는다.**~~ ⛔ **전제가 무너졌다 (2026-09-08,
   `TASK-44` · 커밋 `9df2369`).** 발음 패턴은 이제 `next_review_at`을 받고 **`review_tasks` 행을
   얻는다** — `task_type='rephrase'`로, 즉 이 설계가 다루는 바로 그 표에 들어온다. 그래서
   **약점이 아니라 요구사항이다**: 이 설계의 히스토리 구조는 발음 복습 과제도 담아야 하고,
   `pattern_id → category`로 그것이 발음임을 유도한다(자매 설계 §5.4가 정한 규약 — `review_tasks`에
   새 컬럼을 두지 않는다). 정본은 `docs/design/2026-09-08-pronunciation-review-cycle-design.md`.
   ⚠️ **이 줄이 낡은 채로 §8 캡틴 질문 5를 지탱하고 있었다** — 그 질문의 정정은 §8이 갖는다.
...
| 5 | ~~**히스토리 화면에 발음 복습이 안 보이는 것을 수용하는가?**~~ ⛔ **이 질문은 철회한다 — 전제가 거짓이 됐다** (2026-09-08, `TASK-44` · `9df2369`). 발음 패턴이 이제 `review_tasks` 행을 얻으므로 "안 보인다"가 성립하지 않는다. ⚠️ **묻지 마라 — 이 질문을 그대로 올리면 이미 철회된 축소가 결정으로 되살아난다.** 대신 남는 것은 **구현 요구**다: 히스토리 구조가 발음 복습 과제도 담고, 화면이 그것을 문법과 구별해 보이는지는 `TASK-43` 구현이 판정한다 | ~~이월된 축소를 푸는 것은 자매 설계와의 접합면을 넓히는 별개 판단이다~~ → **접합면은 이미 넓어졌다** |
