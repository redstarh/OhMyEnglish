...
### 3.3 명세 — `010_shadowing_items.sql`
...
- **008은 `TASK-26`(주간 리포트) 예약**이므로 쓰지 않는다.
- `009`는 적용됨. → **다음 빈 번호는 `010`이다.**
- ⚠️ **`TASK-12`(복습 이력)도 지금 번호를 지정하는 중이다.** `010`을 골랐음을 보고하고
  충돌 조정은 팀리드에게 맡긴다.

**008이 나중에 들어올 때의 순서 갈라짐** — `009_drill_turns.sql`의 정정 상자가 소유하는 기전이
이 파일에도 그대로 적용된다. 이 DB는 `009 → 010 → (나중에) 008` 순으로 적용하고 새 DB는
`008 → 009 → 010` 순으로 적용한다. **무해할 조건을 여기 못 박는다**:

> **`008`은 `learning_sessions`에 `shadowing_item_id`라는 이름의 컬럼을 더하지 않아야 하고,
> `shadowing_items`·그 FK·위 CHECK에 의존하지 않아야 한다.**

주간 리포트가 새 표만 만든다면 이 조건은 자동으로 만족된다. 어기게 되면 **008이** 먼저 그
순서 문제를 해결한다(009의 정정 상자와 같은 규칙).
...
```sql
-- 010 되돌리기. ⚠️ shadowing_recording 행이 이미 있으면 값역 축소가 실패한다 —
...
- **적용 순서**: `010`은 `001`(`utterances`·`learning_sessions`)에만 의존한다. `009`와 독립이고
...
**네 번** 냈다(값을 저장하는데 읽는 코드가 0곳). → **`010_shadowing_items.sql`은 아래 둘이
갖춰진 뒤에 쓴다:**
...
**번호 `010`은 지금 예약하고 파일은 그때 쓴다.** 예약만 하면 §3.5의 순서 갈라짐이 하나 더
