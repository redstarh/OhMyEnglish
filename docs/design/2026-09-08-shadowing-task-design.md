...
- **008은 `TASK-26`(주간 리포트) 예약**이므로 쓰지 않는다. `009`는 적용됨.
...
- ⚠️ **`011`도 확정이 아니다.** `TASK-12`가 **008 예약을 011로 옮기자**는 안을 캡틴에게 올려 두었다
  (그쪽 문서 §5.2·§8 질문 1). **그 안이 승인되면 이 설계는 `012`가 된다.** 번호의 최종 조정은
  **팀리드가** 한다 — 여기서 두 태스크가 각자 다시 고르면 같은 충돌을 반복한다.
...
순서 문제를 해결한다(009의 정정 상자와 같은 규칙). ⚠️ `TASK-12`는 **자기 조건이 위험하다고
판정**했다(주간 리포트가 `review_tasks`를 읽을 개연성이 높다) — 그것이 그쪽이 008 이동을 캡틴에게
올린 이유다. **이 설계의 조건은 그보다 안전하다**: 주간 리포트가 `utterances`의 값역이나
쉐도잉 클립에 의존할 이유가 없다.
