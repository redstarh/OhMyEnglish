...
async def fail_terminally(
    ...
    ⛔ **어디에 쓰고 어디에 쓰지 않는가 — 19자리를 전수 분류했다** (`TASK-260` · 2026-09-20).
    판정은 질문 둘로 한다: ⑴ 재시도 사이에 입력이 **바뀔 수 있는가** ⑵ 그 실패가 모델 호출
    **뒤**인가(뒤면 재시도마다 돈이 나간다).

    * **쓴다** — 입력이 바뀔 수 없고 호출 뒤인 자리. 지금 그것은 `ScenarioNoStage` 하나이고
      그 자리에서 유료 호출 4건이 낭비되던 것을 실측했다. 전사문이 빈 경우도 같은 함수 안에 있어
      함께 종결한다(호출 전이라 비용은 0이지만 한 함수에서 같은 성질을 다르게 다루지 않는다).
    * **쓰지 않는다 ①** — 모델 출력이 계약을 어긴 경우 전부(`AnalysisValidationError` ·
      `SummaryValidationError` · `WeeklyValidationError` · `plan contract violated` ·
      `ScenarioValidationError`). **다른 표본이 계약을 지킬 수 있다**는 전제가 참이다.
    * **쓰지 않는다 ②** — `PLAN_NO_FOCUS_CANDIDATES`(콜드스타트). 호출 전이라 비용이 0이고
      **입력이 실제로 바뀐다**: 같은 몇 분 안에 다른 분석 job 이 끝나면 복습·만성 후보가 생긴다.
      ⇒ 여기서 종결시키면 재시도가 값을 하던 경로를 죽인다.
    * **쓰지 않는다 ③** — 대상이 없는 job(`has no ... target`) · 사라진 세션·발화 · 라우팅 누락.
      전부 호출 전이라 비용이 0이고, **CHECK 와 FK 가 구조적으로 막고 있어** 도달 자체가 거의
      불가능하다(`analysis_jobs_target_matches_job_type` 이 종류별 대상을 전수 열거하고, 세션·발화
      삭제는 job 을 cascade 로 함께 지운다). 바꿔도 재는 값이 없다.
    ⚠️ **그래서 이 함수의 호출자를 늘리는 것이 목표가 아니다** — 「종결이 맞는가」의 판정 근거가
    여기 있는 것이 목표다. 늘리려면 위 질문 둘에 수치로 답한 뒤에 한다.
    """
    updated = await conn.fetchval(
        """
        update analysis_jobs
