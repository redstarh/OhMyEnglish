...
* **시작이 읽을 계획**은 `load_prepared_plan`이 돌려준다 — 직전 세션이 만들어 둔
  `session_plans` 최신 1행을 **조회만** 한다(§3.4). ⚠️ **아직 세션 시작 경로에
  배선되지 않았다**: `api/ws.py`는 이 함수를 부르지 않고, 계획을 대화 상대에게
  넘기는 것은 Task 10이 소유한다. 그래서 AS4("계획이 없어도 세션이 열린다")는
  이 함수의 계약으로만 성립하고 아직 관통 경로로 성립하지 않는다.
