...
- "title": 화면에 보일 한 줄 라벨. **영어로** 써라. 한 문장, 물음표 없이.
...
# ⛔ **`title` 의 언어를 명시하는 것이 사용자 결정 84 다** (`TASK-133` · 2026-09-13).
#
# ⚠️ **이 줄이 언어에 침묵했고 그것이 실측된 결함이었다** — 실물 Claude 가 만든 첫 무대의 제목이
# **한국어**였고 시드 30행은 전부 영어다. 그러면 학습 현황 목록(`services/scenario_progress`)이 두
# 언어를 한 목록에 낸다. 회차: `runs/2026-09-13-task102-intake-pipeline/README.md` §4.
#
# ⛔ **파서에 언어 검사를 두지 «않는» 것이 이 판단의 나머지 절반이다**(그 태스크 AC#3 이 함께
# 정하라고 요구했다). 이유 둘: ⑴ 관측된 결함의 원인은 **문면의 침묵**이고 그것이 가장 싼 첫
#    번째 방어다
# ⑵ 한글 코드포인트로 거부하면 고유명사가 든 정상 영어 제목까지 막고, 거부는 재시도 비용을 만든다
# (`TASK-122` 가 같은 저울질을 하고 같은 결론을 냈다 — 문면으로 막고 발생률을 본다).
# ⚠️ **되살릴 조건을 적어 둔다**: 이 문면을 넣은 «뒤에도» 한국어 제목이 관측되면 파서 거부로 간다.
#
# ⛔ **`prompt_template` 은 이 결정의 대상이 아니다** — 그것은 코치에게 주는 지시문이므로 영어가
# 아니면 수업이 한국어로 돈다. 그래서 위 규칙에서 그 키에는 언어를 적지 않는다(이미 `You are ...` 로
# 시작하라고 못박혀 영어가 강제된다).
# `test_asks_for_an_english_title_and_leaves_the_stage_line_alone` 이 그 «적지 않음»까지
# 함께 잰다 — 결정 84 의 범위가 조용히 넓어지는 것을 막는다.
