...
async def flush_ended_sessions(conn: asyncpg.Connection) -> list[UUID]:
    ...
    ⚠️ **이 함수 혼자서는 종료 기록 전에 프로세스가 죽은 세션을 덮지 못한다.** 그러면
    `end_session`이 돌지 않아 세션이 `active`로 남고, 이 스윕은 정의상 `active`를
    건너뛴다. **그 경로는 리퍼와 짝을 이뤄 덮는다** — `sessions.reap_orphan_sessions`가
    유예를 넘겨 조용한 `active` 세션을 `failed`로 닫으면 그 세션이 이 스윕의 대상이 되므로,
    워커는 **리퍼 → 스윕** 순서로 부른다(`workers/analysis_worker.py`, I-4). 리퍼가 없던
    동안에는 그 묶음이 영구히 걷히지 않았다 — 코드 리뷰가 잡은 과대 주장의 실체였다.
