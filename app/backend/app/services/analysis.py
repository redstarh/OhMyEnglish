...
# ⛔ **「항목마다 키를 다시 짓지 마라」가 실측으로 필요해진 줄이다** (`TASK-257` · 2026-09-20).
# 아래 형식은 `findings` 를 **한 항목**으로만 보여 주므로, 항목이 둘 이상일 때 두 번째부터
# 키 이름이 흔들린다 — 실물 응답이 같은 응답 안에서 `findings[0].pattern_key` 는 옳게 쓰고
# `findings[1]` 에는 `pattern_form` 을 적었다(회차 증거
# `tests/harness/runs/2026-09-20-task256-worker-rerun/responses.jsonl` call 1). 파서가 거부하고
# 재시도가 성공했으므로 계약은 지켜졌지만 유료 호출 한 건이 더 났다.
# ⚠️ **틀린 키 이름을 프롬프트에 «적지 않는다»** — 그 낱말을 보여 주면 오히려 그것이 후보로
# 살아난다. 그래서 금지가 아니라 「글자 그대로 쓴다」는 긍정 지시로 적는다.
# ⚠️ 형식 예시를 두 항목으로 늘리지 않은 이유: 그러면 모델이 **항목 수를 둘로 맞추려는** 쪽으로
# 기울어 「개수 제한 없이 전부」라는 위 줄과 싸운다.
# ⛔ **이 줄을 「빈도를 낮춘 장치」로 읽지 마라** (`TASK-258` 이 나중에 재서 정정했다). 수정 «전»
# 프롬프트로 항목 296개를 재도 위반이 0건이었다 — 그 위반은 항목 628개에 1건(0.16%)으로 드물고,
# 주된 방어는 파서의 거부와 job 재시도다. 이 줄의 근거는 **모호함을 없앤다** 하나다. 그러므로
# 지워도 당장 아무 일도 일어나지 않는다 — 그 사실을 알고 지워라.
_OUTPUT_RULES = """\
[출력]
