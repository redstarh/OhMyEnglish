...
class Settings(BaseSettings):
    ...
    # ⛔ **앞 판의 서술을 정정했다** (2026-09-18 · `TASK-185`). 그 주석은 *"코치 지시문이 이 값을
    # 「각 문장을 N회 따라 말하게 한다」로 싣는다"* 고 적었으나 **그런 지시문이 없다** — 닿는 곳은
    # `api/ws.py` → `as_event_payload()` → 화면뿐이고 프롬프트 조립에 닿는 경로가 **0건**이다
    # (grep 확인). 계획 단계의 의도가 주석에 남고 구현이 다르게 간 자리다.
    #
    # **이 값이 정하는 것 둘** (결정 129): ① 클립을 몇 번 들려주는가 ② **낭독을 몇 번 읽게 하는가.**
    # ⚠️ **한 값으로 두는 것이 판단이다** — 값역이 같고(1~10) 「듣고 따라 읽기」가 짝이라 회수가 갈릴
    # 이유가 없다. 갈릴 이유가 생기면 그때 나눈다(지금 나누면 근거 없는 두 번째 노브가 된다).
    # ⛔ **내리면 실제 연습량이 함께 줄어든다** — 그것이 의도가 아니면 내리지 마라.
    # ⚠️ **기본값 1 을 올리지 않았다**: 값역의 항등원이라는 위 근거가 살아 있다. 그래서 화면은 목표가
    # **2 이상일 때만** 진행도를 보인다 — 1 이면 「1/1」이 뜻이 없고 기본 동작도 바뀌지 않는다.
