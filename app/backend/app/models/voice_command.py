...
# `TASK-61.3` 회차가 관측한다.
#
# **결정 114 (`TASK-61.17`) — 짧은 표지를 더했다.** 앱 이름 표지가 영어에서 2회 중 1회
...
#
# **결정 116 (`TASK-61.18`) — 「Hello」·「헬로」를 다시 뺐다.** 실측이 그 대가를 확인했다:
# `Hello, my name is Jin and I work at a bank.` 가 `voice_command` 로 저장되어 오류 분석에서
# 빠졌다(1/1 · `runs/2026-09-16-task61-16-divergence-surface` §3-②). 자기소개·인사의 첫마디로
# 가장 흔한 형태라 그 손실이 반복된다.
# ⚠️ **대가가 줄었을 뿐 사라지지 않았다** — 「Hey, how are you?」 는 여전히 표지를 얻는다. 단정이
# 그 사실을 지킨다(`test_a_hey_greeting_still_counts_as_a_marker_and_that_cost_remains`).
