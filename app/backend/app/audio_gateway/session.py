...
class SessionRunner:
    ...
    async def _close_and_record(self, status: SessionEndStatus) -> None:
        ...
        **종료 기록과 발음 시도 수렴은 한 트랜잭션이다** (설계서 §3.2). 갈라 두면 그 사이
        ...
        ⛔ **종료 시간을 소유하는 것은 이 함수다** (`TASK-215` · AC3). 예산 셋이 두 모듈에 흩어져
        있어 「종료가 왜 이만큼 걸리나」의 답이 어디에도 없었다 — 셋은 **차례로** 쓰이고 합이 이
        함수의 상한이다:

        | 순서 | 예산 | 소유 | 무엇을 기다리나 |
        |---|---|---|---|
        | ① | `DRAIN_TIMEOUT` | 이 모듈 | 비행 중인 전사문 저장 (`_await_pending_saves`) |
        | ② | `nova._FINAL_USAGE_DRAIN_SECONDS` | 어댑터 | 종료 뒤 오는 마지막 `usageEvent` |
        | ③ | `nova.CLOSE_TIMEOUT_SECONDS` | 어댑터 | Nova 스트림 정리 |

        ⇒ 셋의 합이 최악 상한이고 지금 **8.0초**다. ②③ 은 `self._adapter.close()` **안에서**
        차례로 쓰이므로 이 함수의 본문에는 보이지 않는다 — 그것이 이 표가 필요한 이유다.
        ⛔ **값을 개별로 적지 않고 합 하나만 적는다** — 셋을 여기 복제하면 상수와 갈라진다. 그 합이
        낡지 않는 것은 `test_session_close_budget` 이 지킨다(바꾸면 그 테스트가 여기를 가리키며
        깨진다).
        ⚠️ **셋 다 상한이고 정상 경로의 소요 시간이 아니다.** 정상 세션은 ① 이 즉시 끝나고(비행 중인
        저장이 없다) ② 는 마지막 `usageEvent` 가 오는 즉시 풀린다.
        ⚠️ **①의 상한은 「자식을 취소하는 기한」이지 이 함수가 걸리는 시간의 상한이 아니다** —
        그 성질은 `_await_pending_saves` 의 docstring 이 소유한다. 즉 8.0초는 **근사치이고 하한이
        아니다.**
        """
    ...
    async def _await_pending_saves(self) -> None:
        ...
        기한은 주입값이 아니라 상수 `DRAIN_TIMEOUT`이다: 저장 하나는 짧은 트랜잭션
        하나라 테스트가 이 값을 줄일 이유가 없고(드레인 데드라인과 달리 대기가
        시나리오의 일부가 아니다), 줄이면 "드레인 마감 직후 저장 중"인 상태를
        재현할 수 없다.

        ⚠️ **`DRAIN_TIMEOUT`은 자식을 취소하는 기한이고, 이 함수가 걸리는 시간의
        상한이 아니다.** `wait_for`가 타임아웃에서 `_GatheringFuture`를 취소하면
        `Task.cancel()`은 `_fut_waiter.cancel()`이 True인 동안 그대로 돌아가므로,
        이 코루틴은 **gather가 완료될 때까지 계속 매달린다** — gather는 자식이 전부
        끝난 뒤에만 완료된다. 그래서 취소는 저장을 "끊지" 못하고, 이 함수는 자식의
        cleanup만큼 기한을 넘겨서라도 **반드시 회수한다**.

        **그 성질이 계약이다**: 반환 시점에 비행 중인 저장은 없다. 실측(2026-09-02,
        `DRAIN_TIMEOUT=0.2`로 축소): 취소 시 즉시 롤백하는 자식은 0.20초에, cleanup에서
        await하는 자식과 **취소를 삼키고 커밋까지 하는 자식은 0.50초**에 반환했고 세
        경우 모두 반환 시점에 자식이 `done`이었다. 뒤따르는 `_flush_analysis`와
        `end_session`(`_close_and_record`)이 이 계약에 기댄다 — 세션이 `completed`로
        보이는 순간 그 세션의 발화가 전부 커밋돼 있어야 회복 스윕이 아직 마지막이 아닌
        발화에 job을 걸지 않는다 (`TASKS.md` I-1의 철회된 엣지).
        """
    ...
    async def _save_final(self, event: TranscriptEvent) -> None:
        ...
        if self._paused and event.speaker == "user" and utterance_type == LEARNING_UTTERANCE_TYPE:
            ...
            # ⛔ **`warning` 이다** (함정 `H-Z` · 표지 게이트와 같은 규약). 처음에 `info` 로 뒀더니
            # 회차에서 **그 줄이 한 건도 보이지 않았고**(2026-09-16 실측: 발화는 실제로 버려졌는데
            # 로그는 0건) 그러면 「정지 중에 학습자가 얼마나 말하는가」를 셀 수 없다. 그 크기가
            # 이 기능의 대가이므로 세야 한다.
            # ⚠️ 전사문을 로그에 넣지 않는다 — 버린 이유가 「기록하지 않는다」인데 로그에 남기면
            # 같은 말을 다른 곳에 적는 것이 된다. 길이만 남긴다.
            logger.warning(
