...
def build_system_prompt(
    ...
    **조립 재료**(놓친 소리 · 계획 · 질문 · 무대)를 센 것이고 그 결정이 답한 질문은 *"`questions`가
    ...
    `drill_turns_min`을 인자로 받는 것은 결정 15 위반이 아니라 **그 정신(데이터를 받는다)을 지키는
    쪽**이다: 여기서 `get_settings()`를 부르면 G-3(`factory.py`가 소유한 *"팩토리는 데이터를 받아
    조립한다"*)가 깨지고, **테스트가 값을 주입할 자리가 사라진다** — 설계서 §6이 요구한
    「기대값 경계 4건」과 H-5 tripwire가 값 주입에 의존한다. 값의 소유자는 `Settings` 하나이고
    ...
    **오늘의 계획**(§5.2 가변부): 계획을 문장으로 바꾸는 것은 **읽는 쪽의 일이다** —
    ...
    적지 않으면 두 지시가 함께 실려 모순된다. 같은 축으로 보이는 후보는 **셋**이고
    **그중 둘만 대체한다**(S2-10 리뷰 2라운드에서 확정 — 처음에는 힌트 시점만 적었다).
    ⚠️ **"같은 축이다"가 곧 "계획이 이긴다"는 뜻이 아니다.** 축마다 근거가 다르다:
