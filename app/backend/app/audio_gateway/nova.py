...
이 파일의 근거다. 그 실측에서 나온, **문서만 읽고는 알 수 없었던 것**을 아래에 열거한다
(⛔ **개수를 적지 않는다** — 항목이 늘 때 그 숫자를 함께 고치지 않는 것이 이 리포의 지배
실패 모드이고, 실제로 `b31227a`가 5번을 더하면서 「넷」을 남겼다. `TASK-33`이 그것을 이렇게
닫았다: 세지 않는 서술은 낡지 않는다):
...
Voice control:
12. Speech is only a command when it starts with a wake word - "Hey" or the app name
    "Oh My English" (Korean learners may say "헤이" or "오 마이 잉글리시"). Anything else is
...
# 돌려 **전부 0**을 얻었고 계획 블록이 없는 기준 팔도 0이었다. 남은 층이 이 두 규칙이고 **겹치지
# 않는 두 경로**로 막는다: 규칙 9 는 문법 교정이 **없는** 턴에서도 `leave pronunciation alone` 으로,
# 규칙 11 은 문법 교정이 **있는** 턴에서 상호배제로. 하나만 풀면 다른 하나가 남는다.
#
# ⛔ **one-per-turn 예산을 늘리지 않는다** (`TASK-75` AC#3). 규칙 11 을 「같은 턴에 둘 다」로 풀면
# 그 계약이 깨진다 — 대신 **그 턴의 한 교정을 발음이 차지한다.** 순서만 뒤집고 예산은 그대로다.
#
# ⚠️ **대체 범위가 소리 줄이 있는 세션으로 한정된다.** 계획이 발음 초점을 지정하지 않으면 이 줄이
# 아예 없고 고정부의 규칙 9·11 이 그대로 산다 — 평소 대화의 `Grammar first` 는 유지된다. 그것이
...
def build_system_prompt(
    ...
    **인자에 기본값을 두지 않는다** (설계서 §2.1의 C-1). 두면 호출부가 재료를 빠뜨려도 조용히
    ...
    ⚠️ **왜 드릴 설정값도 인자인가** (캡틴 결정 19). 캡틴 결정 15의 「인자 4개」는
    ...
    **오늘의 계획**(§5.2 가변부): 계획을 문장으로 바꾸는 것은 **읽는 쪽의 일이다** —
    ...
    적지 않으면 두 지시가 함께 실려 모순된다. **대체하는 축은 아래 목록에서 셋이다 —
    「힌트 시점」·「목표 수준」·「오늘의 소리」.** 나머지는 전부 비대체이고 축마다 근거가 다르다.
    ⚠️ **셋째(오늘의 소리)는 2026-09-10 에 늘었다** (`TASK-75` · 사용자 승인). 이 문단이
    *"늘어나려면 고정 규칙을 덮는다는 판단이 새로 필요하고 그것은 캡틴 결정을 요구한다"*고 적어
    뒀고 실제로 그 순서를 지켰다 — **세는 서술이 안전하다는 근거가 이번에 검증됐다.**
    ⚠️ **"같은 축이다"가 곧 "계획이 이긴다"는 뜻이 아니다.**

    ⚠️ **후보의 개수를 세지 않는다.** 이 문장은 한때 *"같은 축으로 보이는 후보는 **셋**이고 그중
    둘만 대체한다"*였는데, 그 배치가 축을 **여럿** 더하면서(무대 · 질문↔규칙2 · 질문↔규칙3 ·
    드릴 반복 · 우선순위) **그 수를 안 고쳐** 리뷰 2라운드에 걸렸다 — 블록이 늘 때마다 함께
    밀리는 수라서 구조적으로 낡는다.
    ⛔ **그리고 이 정정문 자신이 같은 병에 걸려 있었다** — *"축을 **셋** 더하면서"*라고 적었는데
    바로 뒤 괄호가 **다섯**을 열거한다(`TASK-33`이 고쳤다). 개수가 왜 낡는지 설명하는 문장
    안에서 개수가 낡은 것이라, **세지 않는 서술만이 이 부류를 닫는다**는 증거로 남긴다.
    **대체하는 쪽은 세어도 안전하다**: 늘어나려면 고정 규칙을 덮는다는 판단이
    새로 필요하고 그것은 캡틴 결정을 요구한다. 그리고 명시한 세 이름이 있으므로 읽는 사람이
    목록에서 `대체한다`를 훑어 **직접 검산할 수 있다.**
    ⛔ **축을 더할 때 이 문단을 함께 읽어라** — 새 축이 대체 쪽이면 위 세 이름을 고쳐야 한다.
    ...
    * **오늘의 소리**(`_SOUND_INSTRUCTION`) ↔ 규칙 9(`Grammar first`)와 규칙 4·11(교정 1건/턴 ·
      발음 교정도 교정) — **대체한다** (`TASK-75` · 사용자 승인 2026-09-10). 근거는 실물이다:
      `TASK-81` 이 초점 자리를 내주고 소리 줄까지 넣었는데도 왕복 28회가 조건 넷에서
      **전부 0**이었고
      계획 블록이 없는 기준 팔도 0이었다. 즉 남은 층이 이 두 규칙이며 **겹치지 않는 두 경로**로
      막는다 — 규칙 9 는 문법 교정이 **없는** 턴에서도, 규칙 11 은 **있는** 턴에서 상호배제로.
      ⛔ **예산은 늘리지 않는다**: 규칙 4 의 한 교정을 발음이 **차지**할 뿐이다(순서만 뒤집는다).
      그래서 규칙 11 의 「같은 턴에 얹지 마라」는 계약이 그대로 산다.
      ⚠️ **대체 범위가 이 줄이 있는 세션으로 한정된다** — 계획이 발음 초점을 지정하지 않으면 줄이
      아예 없고 규칙 9·11 이 그대로다. 조건을 프롬프트가 아니라 **계획 데이터**가 정한다.
