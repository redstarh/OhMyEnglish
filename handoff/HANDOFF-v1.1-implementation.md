...
2. ~~**`agent_reprompt` 보조 신호를 유지할 가치가 있는가.**~~ → ✅ **결정됨(2026-08-28): 만든다.**
   캡틴 지시. 설계서 §10 미결 2의 기본안("기록 전용, 5차수 후 판정")을 **뒤집는 결정**이다 —
   근거는 R10-4의 절반을 미충족으로 남기지 않는 것이고, 세 감지 경로가 서로 다른 구간을
   덮는다는 것이다(Nova 직접=가장 정확하나 놓칠 수 있음 / 한글 전사=확실하나 아주 심한
   경우만 / 되묻기=그 중간).
   ⚠️ **취약점은 그대로다** — Nova가 무엇이라 되묻는지는 지시문에 달렸고 Task 5가 지시문을
   규칙 6개→10개로 늘렸다. 그래서 ① 문구 목록을 **한 곳(상수)에 모으고** ② 그 목록의 근거를
   실측으로 적고 ③ **5차수에서 실제 Nova 응답과 대조**한다. 추측한 문구를 흩뿌리지 않는다.
   기존 실측 근거: 4차수 `p1k` 응답 `"Sorry, that part sounds a bit unclear. Could you
   please repeat…"` (구 지시문 시점 — 그래서 대조가 필요하다).

2-b. ✅ **결정됨(2026-08-28): 대답 없이 세션을 끝낸 시도는 `incorrect`다 — 단 패턴은 만들지 않는다.**
   캡틴 지시. 설계서 §3.2의 "남은 `pending`은 `unclear`로 수렴"을 **뒤집는 결정**이다.
   근거: 학습자 관점에서 "대답을 못 한 것"은 못 한 것이다.
   **단 약점 목록(`error_patterns`)에는 넣지 않는다** — 넣으면 시도조차 안 한 발음이 약점으로
   쌓이고, 세션을 급하게 닫으면 그날 시범 전부가 약점이 돼 "진짜 자주 틀리는 발음"이 흐려진다.
   구현 규약 2개:
   - `link_pattern`(Task 7)은 **판정 경로(`record_attempt`)에서만** 부른다. `resolve_dangling`은
     절대 부르지 않는다 — 구조적으로 불가능하게 둔다.
   - 수렴 시 `spoken_form`을 **비운다(null)**. 실제로 재발화를 못 들었으므로 "들린 발음"이 없는
     것이 맞고, 그러면 `outcome='incorrect' and spoken_form is null`이 **"시도 안 함"의
     식별자**가 된다(스키마 변경 없이). §11 학습 코치가 나중에 둘을 구분할 수 있어야 한다.
