...
**차단 결정은 없다** — `TASKS.md` B-4(발음 `pattern_key` 값역)는 2026-08-28 **§5.6 규약 재사용**
으로 결정됐다. Nova의 `target_sound`를 키로 직접 쓰지 않는다.
...
| 언제 | 이전 → 다음 | 4개 지표 대조 |
|---|---|---|
| 2026-08-28 | `claude_air_3-14` → **같은 창의 새 컨텍스트** (3-15는 쓰이지 않았다) | ✅ **4/4 일치** (1번은 아래 사유로 실질 일치) |

기준값은 `8a0834e` / Task 7 / 312 passed·clean / 필수 3건이었고, 새 컨텍스트가 **직접 돌려** 얻은
값은 **`c443109`** / Task 7 / **312 passed** · ruff·format(27파일)·ty clean / **3건**이다.

**HEAD 차이의 사유**: `8a0834e..HEAD`는 커밋 1개(`c443109`, handoff 기준값 정정)뿐이고
`git merge-base --is-ancestor 8a0834e HEAD`가 참이다. 이전 세션이 이 표를 쓴 **뒤에** handoff를
docs-only로 커밋해 생긴 차이라 코드 상태는 같다(게이트 3항목 동일).
→ **교훈**: 인계표에 HEAD를 적을 때는 **그 표를 담은 커밋 자신**이 HEAD를 옮긴다. 다음부터
기준값은 `HEAD ≥ <해시>` 형태로만 적는다(머리말은 이미 그렇게 적혀 있었다).

**tmux 정리**: 계획대로 3-15로 넘어가지 **않았다** — 캡틴이 같은 창(3-14)에서 새 컨텍스트를
시작했고, 3-15 pane은 빈 셸이었다(`capture-pane` 빈 줄). 정리 시점에 3-15는 이미 사라져 있었고
(`kill-session` → `can't find session`) 남은 세션은 3-14 하나다.
