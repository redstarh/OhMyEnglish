...
| `428883e` | handoff 갱신 (Task 5 마감) |
| `1764e92` | 캡틴 결정 3건 기록 (일부는 이후 뒤집혔다 — §2.1) |
| `0025301` | **발음 플로우 일반화** (계획 Task 6) — 005 제약 + 규칙 소유 경계 재정리 |
| `9b25efb` | **설계서·계획서 일관화** — 구현 결정을 설계에 반영 |
...
## 2.1 계획에서 벗어난 5건 — **계획 본문 코드를 그대로 베끼면 안 된다**

**정본 목록은 계획서 머리말의 "구현 후 정정" 표로 옮겼다** (`9b25efb`) — 계획을 여는 사람이
가장 먼저 보는 자리이기 때문이다. 설계 계약의 정본은 설계서 **§3.1a·§3.2·§6.1**이다.
아래는 그중 다음 태스크가 **바로 쓸** 두 가지만 남긴 것이다.

**(1) 서비스의 실제 시그니처** — 계획 본문의 서술을 이것으로 대체한다:
...
### 3.2 Task 6이 끝난 지금 실제로 무엇이 되고 무엇이 안 되나 (실측)

| 단계 | 상태 |
|---|---|
| Nova가 발음을 지적하고 문장을 다시 읽어준다 | ✅ 지시문 규칙 7~10 (⚠️ 실물 미검증 — §3.1) |
| `toolUse`가 온다 → `PronunciationEvent`로 번역된다 | ✅ `toolConfiguration` 전송 · `_on_tool_use` |
| 서버가 `{"type":"pronunciation"}` 프레임을 방송한다 | ✅ |
| **DB에 저장된다** | ✅ `record_attempt` — 시범 시점에 열고 판정에 닫는다 |
| 한글 전사 보조 신호 | ✅ `note_transcript` — 한글이 섞이면 `signal_source='korean_transcript'` 행 1건 |
| 세션 종료 시 수렴 | ✅ `resolve_dangling` — 남은 대답 기다림을 **`incorrect`**로, `spoken_form`은 비운다. 종료 기록과 **한 트랜잭션** |
| 되묻기 문구 감지 | ❌ **만들지 않는다** (캡틴 결정) — R10-4 절반은 의도적 미충족 |
| **프론트가 배지를 띄운다** | ❌ **Task 8.** `app/frontend/lib/ws.ts`의 `ServerEvent`에 `pronunciation`이 **없고** `page.tsx` switch에 `case`도 **없다**(실측 0곳) — 프레임이 조용히 무시된다(크래시 없음) |
| 약점 패턴으로 연결된다 | ❌ **Task 7** — 설계서 §3.2가 **경로 불문**을 요구한다(수렴된 행도 포함) |
| 결과 화면 발음 카드 | ❌ Task 8 |

즉 **지금 `VOICE_ADAPTER=nova`로 돌리면 Nova가 발음을 교정해 주고 기록도 남는다.**
남은 것은 그 기록을 약점 패턴으로 잇는 것(Task 7)과 화면에 보이는 것(Task 8)이다.

⚠️ **배지 없음은 정상이다.** nova로 띄웠는데 배지가 없다고 **정상인** 어댑터·세션을
디버깅하지 마라 — 프론트는 아직 그 프레임을 모른다.
...
그래서 시도를 **2단계 생명주기**로 모델링한다: `pending` INSERT → 판정 UPDATE → **세션 종료 시 남은 `pending`을 `incorrect`로 수렴**(2026-08-28 결정으로 `unclear`에서 바뀌었다 — §6 2-b). 마지막 규칙이 핵심 방어다 — `pending`을 영구히 남기면 미판정 시도가 숙련도를 왜곡한다.
...
**⑧ 시도의 순서는 DB가 강제한다 — 앱 규약이 아니다** (004 `attempt_seq`). 상세와 버린 대안은 계획서 머리말 정정표 #1. 요지: 벽시계는 순서의 대리일 뿐이고, 함수 하나의 규약은 표를 직접 쓰는 다른 writer를 구속하지 못한다.

**⑨ 보조 신호와 Nova 생명주기는 다른 함수다** (`record_signal` vs `record_attempt`). 상세는 계획서 머리말 정정표 #2. 요지: 인자값이 생명주기 동작을 바꾸면 호출부에서 안 보인다. 설계서 §6.1이 `signal_source`를 서술 컬럼으로 정의한 것과 맞춘다.

**⑩ 규칙은 서비스가 소유하고 세션은 배선만 한다** (설계서 §3.1a). 감지기 진입점이
`note_transcript` 하나라 감지기를 늘리거나 줄여도 `session.py`가 바뀌지 않는다. 계획은 한글
정규식을 `session.py`에 두라고 했는데(계획 :1162) 그러면 다음 감지기가 또 거기 박힌다.

**⑪ 상태 조합은 표가 강제한다** (005 `pending ⟹ nova_tool`). 앱에서 지키면 같은 규칙이
값역 좁히기·판정 SQL 필터·그 필터를 보는 테스트 셋으로 흩어진다. 제약 한 줄이 그 셋을
대체하고 raw INSERT로도 우회할 수 없다 — "케이스를 막는 코드"가 아니라 "케이스가 존재할 수
없게" 만드는 쪽을 택한다.
...
2. ~~**`agent_reprompt` 보조 신호를 유지할 가치가 있는가.**~~ → ✅ **최종 결정(2026-08-28): 만들지 않는다.**
   ⚠️ **이 항목은 같은 날 두 번 결정됐다.** 처음에 "만든다"로 정해 커밋 `1764e92`에 기록했으나
   캡틴이 곧 **뒤집었다** — "너무 복잡해지고 케이스가 많아져". 근거는 문구 매칭이라 케이스가
   계속 불어난다는 것이고, 특히 Task 5의 지시문 규칙이 매 발음 시범마다 "repeat"를 만들어
   "시범 vs 되묻기"를 가르는 규칙이 자란다.
   **결과: R10-4의 절반은 의도적 미충족이다.** `signal_source` 값역에는 예약값으로 남긴다
   (지우려면 마이그레이션이 필요한데 데이터 이득이 없다). 미충족 구간("Nova가 놓쳤고 전사문도
   한글이 아닌" 경우)이 실제로 얼마나 되는지는 **5차수 관측 대상**이다 — 크면 판단을 다시 본다.

2-b. ✅ **최종 결정(2026-08-28): 대답 없이 끝낸 시도는 `incorrect`고, 이후 학습도 그냥 틀림으로 본다.**
   설계서 §3.2의 원래 수렴값 `unclear`를 뒤집는다. 근거: 학습자 관점에서 "대답을 못 한 것"은
   못 한 것이다.
   ⚠️ **이 항목도 두 번 결정됐다.** 처음에 "단 약점 목록에는 넣지 않는다"는 예외를 붙여
   `1764e92`에 기록했으나 캡틴이 **철회했다** — "이후 학습에 그냥 틀림으로 일관성을 가지자,
   플로우 복잡성이 너무 높아지고 있어". 그래서 **경로별 예외가 없다**: 판정으로 `incorrect`가
   됐든 수렴으로 됐든 같게 처리한다(설계서 §3.2). **Task 7은 수렴된 행도 패턴을 만들어야 한다.**
   구현: `resolve_dangling`이 `outcome='incorrect'`, `spoken_form=null`을 쓴다. `spoken_form`을
   비우는 이유는 예외 식별이 아니라 **데이터 정합**이다 — 재발화를 못 들었으므로 "들린 발음"에
   남을 값이 없고, 비우지 않으면 Nova의 placeholder가 결과 화면에 학습자 발음으로 표시된다.

2-c. ✅ **결정됨(2026-08-28, 내 판단): 세션 종료 기록과 수렴을 한 트랜잭션으로 묶는다.**
   캡틴이 배관은 내게 맡겼다. `services/sessions.py:49-54`의 `mark_session_ended(pool, …)`를
   `conn`을 받게 바꾸고 `_close_and_record`가 트랜잭션을 연다. 호출처가 `session.py:148`
   한 곳뿐이라 변경이 작고, **Task 7이 어차피 같은 원자성을 요구해** 두 번 고치는 것보다 싸다.
   수렴이 이제 학습자에게 보이는 값(`incorrect`)을 쓰므로 견고함의 값이 올라간 것도 근거다.
   ⚠️ 세션 종료 경로는 3차수 N5가 검증한 자리다 — 기존 테스트 무회귀를 반드시 확인한다.
...
6. 계획 `docs/design/2026-08-27-pronunciation-echo-plan.md`의 **Task 7**부터 TDD로 진행한다.
   - **착수 전에 §6 "내 작업" 첫 항목**(`record_attempt`가 자기 트랜잭션을 열어야 한다)을 처리한다. Task 7이 그 원자성을 요구한다.
   - **패턴 생성은 경로 불문이다** — 판정으로 `incorrect`가 된 행뿐 아니라 **수렴으로 `incorrect`가 된 행도** 패턴을 만든다(설계서 §3.2). 경로별로 다르게 처리하지 않는다.
   - `error_patterns.target_form`이 **not null**이라 계획에 쓴 upsert가 그대로면 실패한다.
   - 테스트 헬퍼는 파일을 열어 확인하고 재사용한다 — 계획이 추측한 이름은 Task 5·6에서 매번 실제로 없었다.
