...
> **가리키기만** 함. 최종 갱신 **2026-09-10** · 브랜치 `design/first-vertical-slice`
...
## 다음 한 걸음 — **`TASK-79` → `TASK-80` → 좁힌 재리뷰**. 그 뒤 `TASK-73` → 그러면 선언 가능

> **`TASK-70` 은 열려 있음** — 리뷰를 두 번 돌렸고 **둘 다 Not Approve** 임. ✅ **1차의 HIGH 3건은
> 해소로 인정됐음**(재리뷰가 *"어댑터 오류 수정과 원자성 테스트 삭제는 현재 설계에 부합"* 이라
> 적었음). 막고 있는 것은 **새 지적 둘**이고 태스크로 등록했음 — `TASK-79`(P1 · `no_utterances`
> 재확인 상한이 워커 지연을 못 버팀) · `TASK-80`(P2 · SigV4 쌍이 원자적으로 선택되지 않음).
> ⛔ **`TASK-79` 는 상수를 키워서 닫지 않음** — 스윕 시점을 보장하는 **계약이 없어** 어떤 상수도
> 맞을 수 없음. 지금 계측도 지연 시나리오를 못 잡음(리뷰가 그것까지 지적했음).
> ⚠️ **다음 재리뷰는 범위를 좁혀 돌림** — 사용자가 *"리뷰가 너무 길어지는 것 같아"* 를 지적했음
> (2026-09-10). 전체 Phase 1 모듈이 아니라 **`TASK-79`·`TASK-80` 이 건드린 자리와 이웃만.**
> 1·2차 판정이 나머지를 이미 덮었음.
>
> ⛔ **`TASK-73` 착수 조건**: 뮤테이션은 **codex 리뷰가 도는 동안 걸지 않음.** 리뷰어가 리포를
> 읽으므로 변이된 코드를 리뷰하게 되어 판정이 무효가 됨(2026-09-10 에 알아채고 미뤘음). 항목별
> 대상 단정은 그 태스크 노트가 줄 번호로 가짐 — **W1 은 이미 완료**(변이 둘로 확인).
>
> ⚠️ **`TASK-78` 은 다른 세션(`ohmyenglish-15`) 소유임** — 건드리지 않음. 남은 것은 AC#3(복습 시계
> 종단 · 워커 필요)·AC#4(사용자 재결정)이고 **내 ① 다음 순서**임(아래 결정 50).

> ⛔ **발음 판정의 현재 판단은 결정 50 이 소유함**(대장). 요지 셋: ⑴ **tool 배선은 문제가 아님**
> — 「문장 되말하기 ⟺ tool」 **11/11 회차** · `target_sound` 는 tool 이벤트 전건 ⑵ **그런데 지금 앱
> 설정에서는 코칭 자체가 안 일어남** — 원인은 **계획 블록**(`Focus on:`·무대·힌트가 전부 문법을
> 가리킴) ⑶ **그래서 지금 우회로를 걷어내면 tool 도 우회로도 0 이 됨.** 순서는 ① 계획이 발음에
> 자리를 내주게 고침 → ② 도착률 재측정 → ③ 기준 넘으면 걷어냄. 소유자 `TASK-78`(다른 세션).
> ⛔ **`--app-prompt` 는 앱 프롬프트가 아니고 「기반 프롬프트」임**(2,339자). 앱은
> `build_system_prompt`(인자 6개)로 계획·무대를 붙여 보냄 — **이 구별을 놓치면 통제 대조가 무효임.**
> ⚠️ **재결정 전에는 `toolChoice` 강제를 제품 경로에 넣지 않음.** 강제하면 발화가 사라짐(직접 관측).
> ⚠️ 스파이크에 팔 셋이 있음 — `--prompt-file` · `--tool-choice` · `--wav a,b`(다중 턴). **기본값이
> 없어 기본 팔은 기반 프롬프트 그대로임.**

> ⛔ **Phase 1 완료를 선언하지 않았음** — `TASK-47` 이 6항목을 대조했고 **4항목 충족 · 2항목 미충족**임.
> 판정 정본은 `docs/design/2026-09-09-phase1-completion-audit.md` 이고 **선언 자리는 AC 문서**임.
> 남은 둘: **항목 6**(리뷰 Approve — `TASK-70`) · **항목 4**(뮤테이션 등가 증거 — `TASK-73`).
> ⚠️ 캡틴 결정 B-7 이 지목했던 미충족 셋은 전부 닫혔음 — **그것과 6항목 전건 충족은 다름.**
> ⛔ **첨부된 출력이 현재 코드를 증명하는지 먼저 봄** — W-live 기록의 `11/11` 이 낡아 있었고(스크립트
> 단정이 12개로 늘었음) 다시 돌려 `PASS: 12/12` 를 얻었음.

> ✅ **결과 화면은 정리됐음** — `TASK-55`·`56`·`57`·`64`·`34`·`77` `Done`. C3 회차 **단정 94건 전건
> 통과**. ⛔ **결과 화면은 수를 그리지 않음이 게이트임**(`A3-3`) — 재는 방식은 「드릴 두 수가 없다」가
> 아니라 **「API 가 준 학습자 문구에 없던 숫자가 화면에 없다」**임. ⛔ 상태 코드는 분류용이고 학습자
> 문구는 화면이 소유함(`failureNotice`). 상세는 `runs/2026-09-09-task55-56-57-*.md` 와
> `runs/2026-09-10-task77-*.md` 가 가짐.
>
> ⚠️ **백엔드 어댑터 상태를 알고 시작함**: pid **58241** · `VOICE_ADAPTER=stub_unresponsive` ·
...
|--:|---|---|
| 1 | `git rev-parse --short HEAD` · `git status` | **`911d593` 이상**(등호를 요구하지 않음 — `H-P`) · 추적 미커밋 **0건** · `origin` 과 동기. ⚠️ **다른 세션이 동시에 원장을 쓰고 있으면 미커밋 0건이 아닐 수 있음** — 그 세션의 파일인지 먼저 보고 내 것만 커밋함(`H-AM`) |
| 2 | 원장 — `grep -h "^status:" backlog/tasks/*.md \| sort \| uniq -c` | 전체 **83** · Done **60** · To Do **20** · In Progress **3** · Awaiting Decision **0**. ⚠️ In Progress 셋의 소유가 갈림 — `TASK-70`·`TASK-73` 은 내 것이고 **`TASK-78` 은 다른 세션 것**임. ⛔ `backlog task list --plain \| grep -c "^  TASK-"` 로 세지 않음 — 우선순위 라벨이 붙으면 빠짐 |
...
| 4 | DB (읽기만) | `schema_migrations` **9** · `learning_sessions` **13** · `utterances` **120** · `error_patterns` **9** · `error_occurrences` **24** · `review_tasks` **15** · `session_plans` **2** · `pronunciation_attempts` **4** · `analysis_jobs` **49** · **보존 세션 6개 전건 생존**. ⚠️ **수치에 표 이름을 붙여 적음** — 이름 없는 묶음은 읽는 사람이 엉뚱한 표에 대응시킴. ⚠️ **2026-09-10 에 다시 쟀음** — 다른 세션이 앱 경로 회차를 돌리고 정리했으므로 이전 값을 그대로 옮기지 않았음. `pronunciation_attempts` 4행은 **전부 `signal_source='nova_tool'` 이고 `target_sound` 가 4행 전부에 있음**(실물 마이크 세션 둘에서 왔음) |
