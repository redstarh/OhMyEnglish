...
**차단 결정은 없다** — B-1(판정↔시범 상관키)은 2026-08-29 **(b) 최신순 수용**으로 결정됐다.
코드 변경 0이고 근거는 설계서 §8 D5-5. 어긋난 쌍이 남을 수 있음을 **알고** 렌더한다.
착수 시 챙길 2건(`signal_source`를 응답에 함께 싣기 · `incorrect`+`spoken_form` null은
"대답 없이 끝난 시도")은 **`TASKS.md` A-2**에 있다 — 여기 복사하지 않는다.
