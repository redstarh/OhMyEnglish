...
| **4차수 P·M (2026-08-26 완료)** | ✅ P1~P6 · M0 · M1 — **`p1` 쌍 한정**(`p2` 쌍 미실행). **신규 앱 결함 0건**, 최우선 위험 P5 clean. ⚠️ **P7은 증거 불충분으로 판정 보류**(스크린샷 2장이 바이트 동일). 기록 `runs/2026-08-26-run-4.md` |
| **5차수에 남은 것** | N6~N13 · B1~B4 전량 · 라이트/다크 5상태 · 회귀(A1·A2·E4·E5·E6·D1·D2 + `tsc --noEmit`) · P8 · **P계층 `p2` 쌍(`p2a`/`p2m`/`p2k`) — 4차수는 `p1` 쌍만 돌렸다** · **P7 재캡처**(세션 식별자가 보이게) · **M계층 재실행 시 agent 발화 덤프** · `tests/harness/**` ruff 6건 |
...
| `runs/2026-08-26-run-4.md` | 4차수 — **P계층(발음)·M계층(학습 반영)**, P5 clean, 신규결함 0. 스크린샷 3장 |
...
~~단 P5·P6에서 결함이 나오면 그때 왕복을 연다~~ → **4차수에서 P5는 clean, P6은 오탐 0건이라
분기가 미충족됐다. 왕복을 열지 않았다** (분기 조건은 `runs/ROUNDS.md` §4차수 범위 확장).
...
| # | 이번에 확인할 것 |
|---|---|
| **M0** | `inject_errors.py --scenario E1`로 패턴을 쌓고(문장 5개 주입 → **관측 2회 모두 신규 7개**였다: 1차수 `run-1.md:32`, 4차수. baseline 1 포함 **합계 8**. 검출은 Claude 판정이라 고정이 보장되지는 않으니 **단정은 델타로 한다**) **새 세션**을 연다 → ① `scenario_id`가 이전 세션과 동일(고정 첫 행, `sessions.py:29` `limit 1`) ② agent 첫 발화가 축적 패턴을 언급하지 않음 ③ `review_tasks` 0행 ④ `next_review_at` 전부 null |
| **M1** | `u1` → 새 세션 → `u2`(같은 `article` 패턴) → `frequency`가 **+2 오르지만**(델타로 단정 — H-4) 두 세션의 agent 발화가 구별되지 않음. **두 세션 발화를 나란히 기록**해 대조 증거로 남긴다. **M0의 새 세션이 M1의 첫 다리다** — 세션은 2개다 |
...
**4차수에서는 새 결함이 0건이라 이 묶음이 발동하지 않았다.** 5차수에서 새 결함이 나오면
O-1(선행 개행)을 함께 넣어 수정 지시를 만든다.
...
| # | 내용 | 상태 |
|---|---|---|
| **O-1** (LOW) | agent 전사문에 **선행 개행**이 들어간다. Nova가 `'\nWhat time do you…'`를 보내고 `save_final_transcript`가 그대로 저장한다. agent 발화라 job이 없어 분석에는 안 들어가고 화면 렌더도 정상 — 지금은 무해. **4차수에서 재현됨**(p1k·u1 두 세션) | **5차수에 묶어 전달** — 4차수는 신규 결함 0건이라 전달하지 않았다 |
...
| **음성 픽스처 배치** | 3차수는 `u1.wav` 1개, 4차수는 9개를 배치했으나 **실제 사용은 5개**(`u1`·`u2`·`p1a`·`p1m`·`p1k`). `u3`·`p2*`는 미사용이다. 5차수는 `p2` 쌍·N7(3턴)까지 돌므로 9개가 다 필요하다 — 매번 배치/제거하거나 캡틴이 상주 배치를 승인할 수 있다 | 캡틴 판단(사전승인 범위) |
