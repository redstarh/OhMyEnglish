...
## ② 다음 한 걸음 — **`TASK-99` AC#1**(`frequency` 를 두 writer 가 덮는 결함의 **도달성** 관측)

`backlog task view TASK-99 --plain` 을 먼저 돌린다. **사용자가 2026-09-11 에 이것을 골랐다** —
이 갈래 소유의 `--ready` 가 `TASK-74` 로 비었고, 그 자리에서 후보 셋(`TASK-99`·`TASK-48`·동료 돕기)을
올려 받은 답이다.

**AC#1 만 이 갈래 몫이다** — 「모델이 `pronunciation_` 접두 `pattern_key` 를 지어내는지 **실물 분석
회차**로 관측한다」이고, ⛔ **도달하지 않으면 잠재 결함으로 등급을 내리고 근거를 적는 것**까지가
그 AC 다. 수정 층 선택(AC#2)·`last_seen_at`(AC#3)·낡은 주석 정정(AC#4)은 **관측 결과를 보고 다시
정한다** — 앱 코드라 이 갈래가 단독으로 정하지 않는다.

**재료가 이미 있다**: 개수를 가른 조건에서 `frequency` 가 2 → 1 로 내려가고 `last_seen_at` 이 과거로
이동하는 것을 **독립 2회 재현**했다(정본은 그 태스크 노트). ⛔ **재현된 것은 증상이고 도달성은 아직
재지 않았다** — 그 구분이 이 AC 의 전부다.
...
| # | 지표 | 2026-09-11 `TASK-74` 를 닫은 직후에 직접 돌려 얻은 값 |
|--:|---|---|
| 1 | 기준 커밋 | **`08b4d04` 이상**(등호를 요구하지 않는다 — 다른 세션이 커밋한다) · `origin` 보다 **ahead 3**(아무도 push 하지 않았다) · 내 미커밋 **0건** |
| 2 | 다음 걸음 | ⚠️ **이 갈래 소유의 `--ready` 태스크가 0건이다** — `TASK-74` 가 그 마지막이었다. 아래 ② 를 읽는다 |
| 3 | 게이트 | `pytest` **940 passed**(12.91s) · `ruff check` 안·밖 exit 0 · format **unformatted 0** · `ty` exit 0 · 프론트 `tsc`·`eslint` exit 0 |
| 4 | 착수 전 필수 | **7개**(위 ③). 원장: `Done` **88** · `In Progress` **4**(`TASK-78`·`81`·`86`·`97`) · `Awaiting Decision` **0** · `To Do` **18**(그중 `--ready` **17**) |
...
⚠️ **`TASK-86` 이 `Awaiting Decision` → `In Progress` 로 옮겨졌다**(동료 세션 · 결정 57 을 받았다).
