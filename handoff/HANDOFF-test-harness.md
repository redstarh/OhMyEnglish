...
최종 갱신 **2026-09-11 · `TASK-74` 를 닫은 직후** · 브랜치 `design/first-vertical-slice` · 세션 `ohmyenglish-40`

---

## ① 이번 세션이 마무리한 것

**닫은 태스크 여섯**: `TASK-94`(P8) · `TASK-79`(AC#2) · `TASK-101` · `TASK-92` · `TASK-97`(AC#1·#2·#3) ·
**`TASK-74`**(`08b4d04` · 사용자 판정 → 결정 **59**). 그 판정은 **보조 신호가 발음 복습 단계를
전진시키지 않게** 이력 쿼리에 허용 목록을 걸고 설계서 §3.2·PRD §10.4 에 명문화한 것이다 —
근거와 「닫지 않은 것 하나」는 `backlog task view TASK-74 --plain` 이 갖는다.
...
## ② 다음 한 걸음 — **`TASK-99` AC#2 의 사용자 판정을 받는다**(`Awaiting Decision`)

`backlog task view TASK-99 --plain` 을 먼저 돌린다. **AC#1·#4 는 닫았다**(`7c85f3b`) — 도달성을
실물 18회로 재고 **잠재 결함으로 등급을 내렸고**, 낡은 주석 세 곳을 정정했다. 정본은
`runs/2026-09-11-task99-reachability/` 다.

⛔ **AC#2 가 판정으로 성질이 바뀌었다** — 등급이 잠재로 내려가서 「어느 층을 고치나」 앞에
**「고칠지 자체」**가 왔다. 갈리는 축은 한 문장이다: `ErrorFinding.category` 가 발음 카테고리를
값역에 갖고 **금지가 프롬프트 문구뿐**인 것을 코드로 닫을지, 닫으면 그 발화의 교정 전체를
실패시킬지 그 finding 만 버릴지(이 모듈에 **버리는 선례**가 있다 — `attempts` 의 미매치 키).

⛔ **이 회차를 「G-8 필터를 지워도 된다」의 근거로 인용하면 안 된다** — 양성 대조로 둔 팔 B 도 0 이라
필터의 **필요성은 입증되지 않았다.** 판별력은 별도 시험 4/4 로만 확보했다(`--self-check`).
...
`TASK-88`·`TASK-100`·`TASK-103`·`TASK-104`(앱 코드).
...
| # | 지표 | 2026-09-11 `TASK-99` AC#1·#4 를 닫은 직후에 직접 돌려 얻은 값 |
|--:|---|---|
| 1 | 기준 커밋 | **`7c85f3b` 이상**(등호를 요구하지 않는다 — 다른 세션이 커밋한다) · 아무도 `origin` 에 push 하지 않았다 · 내 미커밋 **0건** |
| 2 | 다음 걸음 | **`TASK-99` AC#2** — `Awaiting Decision` 이고 **사용자 판정으로 열린다.** 아래 ② 를 읽는다 |
| 3 | 게이트 | `pytest` **940 passed**(12.13s) · `ruff check` 안·밖 exit 0 · format **unformatted 0** · `ty` **All checks passed** · 프론트 `tsc`·`eslint` exit 0 |
| 4 | 착수 전 필수 | **7개**(위 ③). 원장: 파일 **112** · `Done` **88** · `In Progress` **4**(`TASK-78`·`81`·`86`·`97`) · `Awaiting Decision` **1**(`TASK-99`) · `To Do` **19**(그중 `--ready` **18**) |

⚠️ **지표 4 의 세는 기준을 밝혀 둔다** — 이전 판의 「착수 가능 20」은 `--ready` **출력 행 수**였고
그 출력에는 `In Progress`·`Awaiting Decision` 행이 섞인다. 위 표는 **상태별**로 센 값이다
(`grep -h '^status:' backlog/tasks/*.md | sort | uniq -c`).
⚠️ **원장이 이 세션 중에 110 → 112 로 늘었다** — 동료 세션이 `TASK-108`·`TASK-109`(계획 파서가
계획 전체를 버리는 두 결함)를 등록했고 `TASK-86` 을 `Awaiting Decision` → `In Progress` 로 옮겼다.
⚠️ **`ty` 가 `tests/harness/runs/**` 까지 본다**(이 세션 실측) — 회차 스크립트의 `Settings()` 에
