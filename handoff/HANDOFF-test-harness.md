...
**닫은 태스크 다섯**: **`TASK-74`**(`08b4d04`) · **`TASK-99` 전체**(`7c85f3b`·`681e793`) ·
**`TASK-48`**(`37e8b61`) · **`TASK-110`**(`8256219` · 이 세션이 신설하고 닫았다 — **스모크 16/16
PASS**) · **`TASK-72`**(`bdbb878`). 계획서 Task 12 의 Step 1·2·3 이 이로써 전부 닫혔다.
**신설하고 남긴 것 하나**: **`TASK-113`**(아래 ②).
...
## ② 다음 한 걸음 — **`TASK-113`**(`database-schema.md` 를 줄 번호로 가리키는 인용 39자리가 낡았다)

`backlog task view TASK-113 --plain` 을 먼저 돌린다. **`TASK-72` 가 부수로 발견해 이 세션이 신설했다.**
확인한 셋이 전부 다른 내용을 가리켰다 — `:120` → 실제 **196~197** · `:139-140` → **183~184** ·
`:377` → **530**. ⛔ **위험한 이유는 조용하다는 것이다**: 인용이 «존재하는» 줄을 가리키므로 따라간
사람이 엉뚱한 문단을 근거로 읽는다.

⛔ **번호를 갱신하지 말고 절 이름으로 바꿔 없앤다**(`H-H`) — 갱신하면 다음 삽입에서 다시 낡는다.
앱 코드·테스트에도 3자리가 있으니 함께 고친다. ⚠️ **`docs/backup/**` 는 손대지 않는다** — 그 시점의
기록이므로 고치면 증거가 바뀐다.
...
| # | 지표 | 2026-09-12 `TASK-72` 를 닫은 직후에 직접 돌려 얻은 값 |
|--:|---|---|
| 1 | 기준 커밋 | **`bdbb878` 이상**(등호를 요구하지 않는다 — 다른 세션이 커밋한다) · `origin` 과 **동기**(`0 0`) · 내 미커밋 **0건** |
| 2 | 다음 걸음 | **`TASK-113`** — 「`database-schema.md` 인용 39자리를 절 이름으로 바꾼다」 · `To Do` · 이 세션이 신설했다 |
| 3 | 게이트 | `pytest` **956 passed**(12.82s — 동료의 결정 64 구현이 들어와 943 에서 늘었다) · `ruff check` 안·밖 exit 0 · **unformatted 0** · `ty` **All checks passed** |
| 4 | 착수 전 필수 | **7개**(위 ③). 원장: 파일 **118** · `Done` **94** · `In Progress` **4**(`TASK-78`·`81`·`86`·`97`) · `Awaiting Decision` **0** · `To Do` **20**(그중 `--ready` **19**) |
