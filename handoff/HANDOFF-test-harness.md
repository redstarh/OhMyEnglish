...
**닫은 태스크 셋**: **`TASK-74`**(`08b4d04`) · **`TASK-99` 전체**(`7c85f3b`·`681e793`) ·
**`TASK-48`**(`37e8b61`). **신설 하나**: **`TASK-110`**(계획서 Task 12 Step 3 이 소유자가 없었다).
...
## ② 다음 한 걸음 — **`TASK-110`**(슬라이스 2 산출물을 보는 스모크 단정 4건)

`backlog task view TASK-110 --plain` 을 먼저 돌린다. **이 세션이 신설했다** — 계획서 Task 12 Step 3 이
`TASK-48` 의 AC 밖이라 **소유자가 없었고**, 그 태스크를 닫으면 원장에서 사라지는 넷이었다.

⛔ **AC#5 를 먼저 한다** — 손대기 전에 **하네스가 살아 있는지 확인**하고 그 결과를 적는다.
`git log -1 -- tests/harness/` 와 최근 앱 변경 날짜를 대조한다. 이유는 함정 `H-AA` 다: **스모크가
이틀간 죽어 있었던 전례가 있고, 죽은 스모크에 단정을 더하면 통과가 무의미하다.**

단정 넷: ① 세션 종료 후 계획 job 이 걸린다 ② `session_plans` 1행 ③ `learner_notes` 1행
④ 다음 세션 지시문에 `Today's plan:` 이 있다.

⛔ **`TASK-99` 회차를 「G-8 필터를 지워도 된다」의 근거로 인용하면 안 된다** — 양성 대조 팔도 0이라
필터의 **필요성은 입증되지 않았다.** 그 필터의 값은 비대칭에 있다.
...
| # | 지표 | 2026-09-11 `TASK-48` 을 닫은 직후에 직접 돌려 얻은 값 |
|--:|---|---|
| 1 | 기준 커밋 | **`37e8b61` 이상**(등호를 요구하지 않는다 — 다른 세션이 커밋한다) · 아무도 `origin` 에 push 하지 않았다 · 내 미커밋 **0건** |
| 2 | 다음 걸음 | **`TASK-110`** — 「슬라이스 2 산출물을 보는 스모크 단정 4건」 · `To Do` · 이 세션이 신설했다 |
| 3 | 게이트 | `pytest` **943 passed**(12.04s) · `ruff check` 안·밖 exit 0 · **unformatted 0** · `ty` **All checks passed** · 프론트 `tsc`·`eslint` exit 0 |
| 4 | 착수 전 필수 | **7개**(위 ③). 원장: 파일 **113** · `Done` **90** · `In Progress` **4** · `Awaiting Decision` **0** · `To Do` **19**(그중 `--ready` **18**) |
