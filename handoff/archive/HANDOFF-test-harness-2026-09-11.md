...
**`TASK-91` 완료 — P2·P3·P7 을 닫았다.** P2·P3 은 기존 원자료로 실물 호출 0회에 판정했고
(`runs/2026-09-10-task91-p2-p3-p7.md`), **P7 은 분석 job 을 처리한 뒤 화면을 캡처해 닫았다**
(`runs/2026-09-11-task91-p7.md`). ⛔ **결과: 화면은 다르고 피드백은 있으나 방향이 틀렸다** —
학습자가 `report` 를 옳게 말했는데 교정 카드가 「짧고 분명한 업무 명사를 쓰라」로 **낱말 교체**를
처방한다(문면은 「분명하지 않게 **들려요**」로 청취를 가리킨다). `TASK-84` 결함의 학습자 관점 증거다.
✅ **그 회차에서 `TASK-83` 절차가 첫 실전 검증을 통과했다** — `H-AY` 의 3점 세트가 재현됐고
회차 전 스냅샷 덕에 **어긋난 행 0** 으로 전건 복원됐다(앞 회차는 복원 불가였다).
