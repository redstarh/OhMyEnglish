...
**착수 전에 그 설계서에서 읽을 절은 셋이다**: **§6**(변경 대상 8곳 + 테스트 목록) ·
**§7**(**내 유도 7건** — 한 줄씩 뒤집을 수 있게 모아 뒀다) · **§5**(약점).
...
⏸ **critic 판정 **미수신** — 이것이 유일한 미결이다.** 설계 확정은 SVG가 「되돌리기 어려운 결론」으로
분류해 반증 위임(opus)을 요구하고, 걸어 뒀지만 이 세션이 끝날 때까지 **오지 않았다.**
⚠️ **`idle`은 「끝났다」가 아니다** — 미수신을 **미결로 남기고** 태스크를 `Done`으로 올리지 않는다.
다시 걸 때 공격 대상으로 줄 것: 설계서 **§7 유도 7건**이 캡틴 결정을 우회한 것이 아닌지 ·
**§2.3의 `count(*) where speaker='agent'`가 정말 「닫힌 사용자 턴 수」와 같은지** ·
`summary`에 CHECK가 없는 것.

⚠️ **판정 미수신이 구현을 막지는 않는다** — 막는 것은 `Done` 전환이다. 설계서 §6의 TDD로 진행하고
판정이 오면 반영한다.
...
| 4 | 착수 전 필수 | **3건** — ① **캡틴 결정 9·10·11·12를 다시 묻지 않는다**(`captain-decisions.md` §5) ② ⏸ **critic 판정 미수신이 유일한 미결이다** — 구현은 막지 않지만 **`Done` 전환을 막는다** ③ **워커를 켜지 않는다**(비용 + §9의 `C3e`가 깨진다) |
