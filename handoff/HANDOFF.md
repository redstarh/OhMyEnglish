...
2. ⛔ **발음 tool 의 현재 사실**: 배선은 문제가 **아니고**(문장 되말하기 ⟺ tool) **지금 앱 설정에서
   ...
   지금 우회로를 걷어내면 **둘 다 0** 이 된다. 순서는 **`TASK-81`**(계획이 발음에 자리를 내주게
   고친다) → 도착률 재측정 → 그때 우회로 제거이고, 뒤 둘의 소유자는 `TASK-78`(**다른 세션**)로
   **`TASK-81` 에 의존을 걸어 뒀다.** ⚠️ `TASK-81` 은 2026-09-10 에 등록했다 — **그전까지 그
   첫 걸음을 소유한 태스크가 없었다.** ⛔ **`TASK-75` 와 다른 층이다**(그쪽은 규칙 9 게이트).
