...
⛔ **정본은 `docs/design/2026-09-18-read-aloud-judgment-design.md` 와 결정 131 이고 착수 전에 읽음.**
요지 셋: ⑴ 낭독 오디오는 **의도적으로** Nova 에서 돌려져 파일로만 남음 ⑵ ⛔ **새 job 종류를 만들지 않고
요청할 때 계산함** ⑶ ⛔ **판정에 모델을 부르지 않음**(낱말 일치로 답이 정해짐). ⚠️ 복습 시계 연결은 v1
범위 밖임 — 낱말 차이를 발음 키로 옮기는 규칙이 없어 지금 연결하면 `TASK-116` 의 오염을 되살림.
