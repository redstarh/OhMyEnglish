...
### §11 슬라이스 1 — 006 마이그레이션 + `suggested_contexts` 저장

**착수 전 필수 0건** (D-5·D-6는 2026-09-03에 해소했다). 먼저 볼 것 3개:

1. **`suggested_contexts`·`impact_score`는 컬럼이 없다** — 006에서 신설해야 한다. 설계서를 믿고
   그대로 구현하면 그 자리에서 깨진다(D-6에서 설계서에 못 박아 뒀다).
2. 함정 **H-S** — 복습 주기·`next_review_at`·일일 계획이 **달력 날짜**에 걸린다. `current_date`로
   구하지 않고 `AT TIME ZONE`으로 변환하며, tz의 SoT는 `users.timezone` 컬럼이다.
3. 우선순위 근거: `suggested_contexts`는 모델이 산출해도 지금 **버려지고 소급이 불가능하다** —
   늦어질수록 잃는 것이 쌓인다(`TASKS.md` §11 순서 권고).
