...
| 2 | 다음 한 걸음 | **`TASK-193`** — 하네스 회차가 앱 큐에 job 을 남기지 않게 함(`H-CD` 가 근거). ⚠️ `TASK-199`~`TASK-203` 은 **2026-09-18 에 닫혔음**(규칙 셋의 중복 제거·영어화 · 상시 로드 토큰 24,438 → 16,457) |
...
| 4 | 착수 전 필수 | 전체 **264** · 완료 **262** · 열린 것 **둘**(`TASK-189`·`TASK-193`) · **미충족 AC 0건**(둘 다 착수 전이라 AC 가 비어 있음) |
...
| ID | 무엇 | 상태 |
|---|---|---|
| **`TASK-193`** | 하네스 회차가 앱 큐에 job 을 남기지 않게 함 | 착수 전 (`H-CD` 가 근거) · 다음 걸음 |
| `TASK-189` | 낭독 판정의 범위와 경로 설계 | 착수 전 · ⚠️ 미결 하나가 사람의 결정을 기다림 |

⛔ **`TASK-189` 의 사전 조사가 «태스크 노트에» 있음 — 착수 전에 그것을 읽음.**
`TASK-189` 는 셋을 확정해 뒀음 — **PRD §7 이 「녹음 비교」를 이미 요구함** ·
