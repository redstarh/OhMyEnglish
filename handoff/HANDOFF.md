...
> ✅ **`TASK-67`(HIGH)이 닫혔음** — 원인 둘을 확정하고 후보 둘을 배제했으며 **결정 49** 가 발음
> 판정을 **보조 신호로 축소**했음. ⛔ **프롬프트 문구를 더 고치지 않음**(다섯 가지 전부 0건) ·
> ⛔ **`toolChoice` 강제를 제품 경로에 넣지 않음.** 기제는 실재함 — Nova 가 그것을 거부하지 않고
> 앱 프롬프트 그대로 `toolUse` 를 내며 `target_sound` 까지 실음. **그런데 같은 회차에서
> `audioOutput` 이 0이라 코칭 발화가 사라짐.** 그 관측을 지우지 않음(구조를 바꿀 때의 근거임).
> ⚠️ **결정 49 는 복습 시계를 도달 불가로 남김** — `note_transcript` 가 `target_sound=None` 만
> 내고 복습 SQL 은 그것이 빈 행을 안 봄(`review.py:207~216` 이 그 사실과 함정 둘을 이미 가짐).
> 판정은 `TASK-74`, 규칙 9 라우팅 결함은 `TASK-75` 가 소유함.
