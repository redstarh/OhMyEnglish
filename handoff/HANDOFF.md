...
마이크 1회는 여전히 필요하니 그 한 번에 함께 본다. 근거와 폐기 사유는 `TASKS.md` A-5.

**그래서 순서가 정해졌다**: G-1·G-3(지시문 확정) → **A-5 배지 구현** → G-4 마이크 1회에서
배지 · 판정 품질 · 픽스처를 한 번에 관측. **Task 8은 그때까지 열어둔다.**
따라서 **바로 다음 코드 작업은 A-5가 아니라 G-1·G-3(지시문)**이다.
...
| 2 | 다음 한 걸음 | **G-1·G-3 — Nova 지시문 정리** (그 뒤 A-5 배지 → G-4 마이크 1회로 함께 검증). A-5를 먼저 손대지 마라 |
| 3 | 게이트 | **333 passed** · ruff·format(27파일)·ty clean · `tests/**` 베이스라인 **6 errors·4 files** · 프론트 `tsc --noEmit`·`eslint` clean |
| 4 | 착수 전 필수 | **0건** — A-2 후단 2건은 `9b7985f`로, A-5 검증 수단은 캡틴 결정으로 해소됐다. 단 **G-1은 G-5와 같은 통로(§11 지시문 가변부)를 쓰므로 따로 만들지 마라** |
