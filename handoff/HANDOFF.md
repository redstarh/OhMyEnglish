...
| 하네스 테스트↔수정 루프 | **3/10 차수 사용.** 미해결 앱 결함 **0건**. 4차수는 커버리지 확장 |
| Phase 1 완료 선언 | ⏸ **캡틴 게이트** — 아래 |
| 3단계 학습 코치 Agent | 📄 설계서 초안(2026-08-25 18:49) — **검토 전** |
| **발음 교정 루틴** | ❌ **없다**(문법 교정만 있다). 4차수 P계층에서 검증 + 수정 범위 분기 |
| **오류 → 이후 학습 반영** | ⚠️ **저장은 되고 활용은 안 된다.** 4차수 M계층에서 부재를 증거로 고정 |
...
1. **E2E-S 스텝 1 "마이크 실프레임"** — 3차수 N5로 **거의 닫혔다**. 브라우저 캡처 경로가
   ...
   대체). 캡틴이 실물 세션 1회를 돌리거나 **이월 수용을 명시**하면 이 항목은 닫힌다.
2. **발음 교정의 범위** — `scenarios-P-pronunciation.md` §5의 ②(프롬프트 수준 교정 지시 추가)를
   할지, ③(발음 패턴 기억 = **PRD:39 비범위**)까지 갈지. ②는 작지만 기능 신설이라 승인이 필요하다.
3. **M계층 반영 단위** — 다음 세션의 *시나리오 선택*을 바꾸는가, *Nova 지시문*에 약점을 주입하는가
   (`port.start()` 시그니처 변경). `scenarios-E-agent-learning.md` M계층 §캡틴 결정 3건.
...
| 묶음 | 내용 | 정본 문서 |
|:--:|---|---|
| **P** | **발음 교정 루틴 검증(신설)** — 정확/오류 발음 쌍으로 P1~P8. 최우선은 **P5**(한글 전사문이 분석 워커에 들어갔을 때 job이 `failed`로 수렴하는지) | `tests/harness/scenarios-P-pronunciation.md` |
| **M** | **오류의 이후 학습 반영 검증(신설)** — M0(반영 부재 실증)·M1(2세션 연속 대조). 지금 실행 가능 | `tests/harness/scenarios-E-agent-learning.md` M계층 |
| ① | 미실행 Nova 시나리오 — N6(barge-in)·N7(3턴)·N8(실발화 병합)·N11(권한거부)·N12(포맷불일치)·N13(endpointing) | `tests/harness/scenarios-N-real-voice.md` §4 |
| ② | 3차수가 대체 커버한 B1~B4 전량 재확인 + 라이트/다크 5상태 | `runs/2026-08-26-run-3.md` §회귀 |
| ③ | 회귀 — A1·A2·E4·E5·E6·D1·D2 + 프론트 `npx tsc --noEmit` | — |
| ④ | 결함이 나오면 O-1(agent 전사문 선행 개행, LOW)을 묶어 수정 왕복을 연다 | — |
...
1. **하네스 4차수 실행** — 위 §4차수 범위. P5(한글 전사문 분석)와 M0(반영 부재 실증)이 새 핵심
