...
### ⚠️ 첫 걸음 — `TASK-31`의 **AC 16건이 전부 닫혔다.** 남은 것은 **코드리뷰 수신** 하나다

**Done으로 올리지 않았다** — `instrument.js` 수정(AC #14)에 대한 **독립 코드리뷰가 미수신**이다
(9단계: Approve 전 Done 전환 금지). ⚠️ **리뷰가 안 돌아올 수 있다** — 미수신이면 그 사실을 원장에
미결로 남기고 **✅로 올리지 마라.** 리뷰가 `Approve`면 `Done` → **쉬지 말고 `TASK-21`로 넘어간다**
...
✅ **크론 "무발동" 미결이 닫혔다 — 발동 실패가 아니라 오배송이었다** (2026-09-06 관측).
감사 잡 **`82bc95e7`**(슬롯 `:07`)의 프롬프트가 **작업 세션 창에 사용자 턴으로 떨어졌다.**
`CronList`가 보여주는 그 잡의 프롬프트 첫 줄이 받은 것과 **축자 일치**하고, 감사 세션도 독립적으로
같은 판정을 냈다. 발동 시점은 **작업 세션이 idle이 된 직후** — H-AD의 서술과 그대로 맞는다.
⚠️ **오배송된 감사 지시를 그대로 수행하면 자기감사가 된다. 받으면 감사 세션에 전달한다.**
판별법·관측자 효과·`durable` 생존은 **`docs/ops/pitfalls.md` H-AD가 소유한다 — 폴링하지 마라.**
