...
| # | 지표 | 값 |
|--:|---|---|
| 1 | 기준 커밋 | 게이트를 돌린 시점은 **`fd5b105`** 이고 그 뒤 `TASK-199` 노트 커밋이 하나 있음 · 작업트리 clean · `origin` 과 **0/0**. ⛔ 이 handoff 의 마감 커밋이 더 뒤에 오므로 **차이가 원장·handoff 뿐이면 정상**임 |
| 2 | 다음 한 걸음 | **`TASK-205`** — 029 마이그레이션(`utterances.readback_transcript` 컬럼 하나 · 결정 131). ⚠️ 앞 원장이 **전부 닫혔음** — `TASK-199`~`203`(규칙 중복 제거·영어화 · 상시 로드 토큰 24,438 → 16,457) · `TASK-193`(회차 teardown) · `TASK-189`(낭독 판정 설계). ⛔ 새 축의 정본은 `docs/design/2026-09-18-read-aloud-judgment-design.md` 임 |
| 3 | 게이트 | `pytest` **1361 passed · exit 0** 은 **2026-09-18 에 직접 다시 돌린 값**임. 나머지 일곱(`ruff` 0 · `ruff format` 296 files · `ty` 0 · `tsc` 0 · `eslint` 0 · `next build` 0)은 `fd5b105` 시점 값이고 **그 뒤 앱 코드 변경이 0건**임(바뀐 것은 원장·`docs/ops/pitfalls.md`·`~/.claude` 뿐) |
| 4 | 착수 전 필수 | 전체 **272** · 완료 **264** · 열린 것 **여덟**(`TASK-204`~`TASK-211`) · `In Progress` **0건** · 착수 가능은 `--ready` 로 **셋**(`TASK-204`·`TASK-205`·`TASK-207`) |
...
## ④ 열린 태스크 여덟 — 낭독 판정 축 하나와 결함 후보 하나

| ID | 무엇 | 상태 |
|---|---|---|
| **`TASK-205`** | 029 — `utterances.readback_transcript` 컬럼 하나 | 착수 가능 · 다음 걸음 |
| `TASK-206`~`210` | 전사 함수 → 낱말 대조 → 엔드포인트 → 화면 → 브라우저 검증 | `205`·`207` 뒤 순차 |
| `TASK-211` | 복습 시계 연결 — ⛔ 「낱말 → 소리」 규칙을 **먼저** 정함 | `210` 뒤 |
| `TASK-204` | 결함 후보: Nova 사용량 4행이 날짜가 달라도 값이 같음 | 착수 가능 · 비용 산정이 이것에 걸림 |

⛔ **이 축의 정본은 설계서 `docs/design/2026-09-18-read-aloud-judgment-design.md` 와 결정 131 임 —
착수 전에 그것을 읽음.** 요지 셋: ⑴ 낭독 오디오는 **의도적으로** Nova 에서 돌려져 파일로만 남음
⑵ ⛔ **새 job 종류를 만들지 않고 요청할 때 계산함** ⑶ ⛔ **판정에 모델을 부르지 않음**(낱말 일치로 답이
정해짐). ⚠️ 복습 시계 연결은 v1 범위 밖임 — 낱말 차이를 발음 키로 옮기는 규칙이 없어 지금 연결하면
`TASK-116` 계열이 고친 오염을 되살림.
