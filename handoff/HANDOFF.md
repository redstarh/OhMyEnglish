...
| # | 지표 | 값 |
|--:|---|---|
| 1 | 기준 커밋 | **`47c7b5e`** · `origin` 과 **0/0** · 미커밋은 미추적 `paseo.json` 하나(내 것 아님 · 3바이트 `{}`) |
...
**닫은 태스크 6건**: `TASK-139`(dev DB 에 024 적용) · `TASK-61`(부모) · `61.1`·`61.2`·`61.3`·`61.4`.
그리고 `TASK-78.1` AC#2 는 닫고 태스크는 파킹으로 되돌렸음.
**새로 등록 7건**: `TASK-139` · `61.1`·`61.2`·`61.3`·`61.4`·`61.5`·`61.6`.
**결정 등재 6건**: **101**(위임분 99·100 을 사용자가 유지로 확정) · **102**(음성 명령 첫 조각의 제품
계약 넷) · **103**(이 세션의 역할 경계를 넓힘) · **104**(D3 을 프롬프트로 · D5 는 앱이 표지를 요구) ·
**105**(표지를 `ohmyenglish` 와 한국어로 둠) · **106**(한국어 명령의 침묵을 그대로 둠).

**회차 6건** (`tests/harness/runs/`): `2026-09-15-task78-1-defenses-in-use` ·
...
**실물 사용 합**: Nova **16세션** · Claude **3회**. ⚠️ `llm_calls` 로는 셀 수 없음(하네스가
`usage_sink` 를 주입하지 않음) — 회차 기록에서 합한 값임.

**리포에 더한 것**: 픽스처 여덟(`tests/harness/fixtures/voice/vc0*.wav` · macOS TTS · 생성 명령은
...
- ⛔ **구멍 ②**: **한국어 «명령» 턴에서만 코치가 침묵함**(0/4). 프롬프트 후보와 「한국어 입력 자체」
  후보를 **둘 다 반증**했음 — 한국어 학습 발화에는 코치가 말함(1/1 · `audio` 150). 조건은 「한국어 +
  tool 호출」 조합이고, 사용자가 결정 106 으로 **그대로 두기로** 정했음(`TASK-61.5` 파킹).
