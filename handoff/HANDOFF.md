...
✅ **크론 "무발동" 미결이 닫혔다 — 발동 실패가 아니라 「밀림 + 합산 발동 + 오배송」이었다.**
**오배송은 2회 연속 관측이라 우연이 아니라 상시 결함이다**(14:22 · 17:53). 감사 잡 `82bc95e7`의
프롬프트가 두 번 다 **작업 세션 창에 사용자 턴으로** 떨어졌다 — `CronList`의 프롬프트 첫 줄이 받은
것과 축자 일치하고, 기제는 `.claude/scheduled_tasks.json`이 **리포 단위 공유**라는 것이다.
⚠️ **`lastFiredAt`으로 정시성을 논하지 마라** — 직접 읽은 값이 두 잡 모두 **밀리초까지 동일**
(`1788684837028` = KST `17:53:57.028`)이고 각자의 슬롯 대비 **46분57초 / 30분57초** 늦었다. 그것은
슬롯 시각이 아니라 **"세션이 한가해진 시각"**이고, 밀린 슬롯들이 그 순간 **한 턴에 합쳐진다.**
