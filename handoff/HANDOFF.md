...
| 2 | 다음 한 걸음 | **`TASK-199`** — rule·skill·CLAUDE.md 의 중복 제거와 영어화 판단. ⚠️ **조사가 절반 진행된 상태로 넘어감** — 중간 결과가 그 태스크 노트에 있음 |
| 3 | 게이트 | **여덟 다 exit 0** — `pytest` **1361 passed** · `ruff` 0 · `ruff format` **296 files** · `ty` 0 · `tsc` 0 · `eslint` 0 · **`next build` 0**(`/` 가 `○` Static 유지) |
| 4 | 착수 전 필수 | 전체 **260** · 완료 **257** · 열린 것 **셋**(`TASK-189`·`TASK-193`·`TASK-199`) · **미충족 AC 0건**(열린 셋은 착수 전이라 AC 가 비어 있음) |
...
| ID | 무엇 | 상태 |
|---|---|---|
| **`TASK-199`** | rule·skill·CLAUDE.md 의 중복 제거·영어화 판단 | ⚠️ **조사 절반** · 다음 걸음 |
| `TASK-189` | 낭독 판정의 범위와 경로 설계 | 착수 전 |
| `TASK-193` | 하네스 회차가 앱 큐에 job 을 남기지 않게 함 | 착수 전 (`H-CD` 가 근거) |

⛔ **`TASK-199`·`TASK-189` 의 사전 조사가 «태스크 노트에» 있음 — 착수 전에 그것을 읽음.**
`TASK-199` 는 규모(상시 로드 약 **748줄**)와 **실재하는 중복**(훅 설계 근거 셋이 두 규칙 문서 양쪽에
있음)을 확정해 뒀음. `TASK-189` 는 셋을 확정해 뒀음 — **PRD §7 이 「녹음 비교」를 이미 요구함** ·
`signal_source` 값역에 값을 더하면 **새 표 없이** 기존 발음 경로를 탐 · **남은 미결은 「낭독을 무엇으로
전사하나」** 하나임(낭독은 설계상 Nova ASR 을 타지 않음).

## ⑤ 착수 전 반드시 읽을 것

- **결정 128·129·130**(지시 대장 끝) — 기각한 대안과 되돌리는 조건을 각 결정이 가짐.
- **`TASK-197` 노트** — `/simplify` 네 각도의 처분과 유지한 것의 근거(설정 과부하 판정 · 참조 표 대안).
- **`bedrock-credentials` skill** — 자격증명이 필요할 때 이것을 먼저 부름(사용자 지시).
