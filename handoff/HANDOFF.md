...
✅ **해소 방향 확정 (2026-09-01, 캡틴): (가) 턴 경계까지 모아서 분석한다.**
화면 표시는 그대로 두고 **분석 입력만** 합친다 — agent가 응답을 시작하면 그 직전까지의 사용자
final을 이어 job 1건을 건다. **고칠 자리는 `services/utterances.py:140` 한 곳**이고 검증
시나리오는 하네스 **N8**이 이미 예정했다. 즉 **바로 착수할 수 있다.**

⚠️ **5차수를 이 수정보다 먼저 열지 않는다** — 오탐이 섞인 데이터로 판정하면 그 판정이 오염된다.
⚠️ **오탐 패턴 2개가 dev DB에 살아 있다** — 지울지는 캡틴 결정으로 남아 있다(두면 다음 세션
프롬프트·복습 큐에 계속 실린다).

📌 **테스트 기록의 정본은 `tests/harness/runs/2026-09-01-mic-1.md`다** — 다음 마이크 세션이
그대로 쓸 절차·판별 3개·관측할 것 5개가 거기 있다. `ROUNDS.md`에 차수 밖 기록으로 등재했다.
