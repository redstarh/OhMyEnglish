...
>
> 최종 갱신 **2026-08-30** · 기준 커밋 **HEAD ≥ `9b7985f`** · 브랜치 `design/first-vertical-slice`
...
끝나고 세션 화면 배지가 남았다(A-5). §11(추천 학습 루틴)은 설계만 끝났고 코드가 0줄이다.
...
결과 화면 카드는 **끝났다**(`9b7985f`). 남은 것은 대화 중 배지다 — 백엔드가 이미
`{"type":"pronunciation", …}`을 방송하는데(`app/audio_gateway/session.py:254-261`) 프론트
`ServerEvent` union과 `page.tsx`의 `switch`에 그 타입이 없다. 고칠 자리·시각 규약은
`TASKS.md` **A-5**.

⚠️ **착수 전에 검증 수단을 정해야 한다.** 스텁은 이 프레임을 내지 않고, 프론트에 테스트
프레임워크가 없고, 하네스는 클라이언트로 붙어 서버→클라이언트 프레임을 주입할 수 없다.
후보 3개와 근거는 A-5에 적었다 — **실물 Nova 1회(G-4와 묶기)가 가장 값싸다.**
...
| 항목 | 값 |
|---|---|
| 테스트 | **333 passed**, skip/xfail 0 (v1.1 착수 전 241 → 312 → 327 → 333) |
...
| # | 지표 | 값 |
|--:|---|---|
| 1 | HEAD | **`HEAD ≥ 9b7985f`** — 반드시 `≥`로 읽는다(함정 H-P). `--amend`로 기준 해시를 잃지 않도록 마감 커밋에 `--amend` 폴백을 쓰지 않는다(함정 H-R) |
| 2 | 다음 한 걸음 | **A절 A-5 — 세션 화면 발음 배지** (Task 8 남은 절반). 착수 전 검증 수단 결정 필요 |
| 3 | 게이트 | **333 passed** · ruff·format(27파일)·ty clean · `tests/**` 베이스라인 **6 errors·4 files** · 프론트 `tsc --noEmit`·`eslint` clean |
| 4 | 착수 전 필수 | **1건** — A-5의 검증 수단을 고르는 것(후보 3개는 `TASKS.md` A-5). A-2 후단 2건은 `9b7985f`로 해소됐다 |
