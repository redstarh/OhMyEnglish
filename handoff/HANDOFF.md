...
## 다음 한 걸음 — **`TASK-70`**(Phase 1 최종 코드 리뷰). 그 뒤 `TASK-73` → 그러면 선언 가능

> ⛔ **결정 49 의 사실 전제 둘이 반증됐음 — 캡틴 재결정 대기.** **빠진 변수는 「다중 턴」이었음**:
> `TASK-67` 의 팔 열 개가 **전부 단일 발화**였고 앱은 다중 턴으로 돎. 앱 프롬프트 · `toolChoice`
> **강제 없음** · 발화 둘로 돌리면 **`toolUse` 가 오고 `target_sound` 가 실리며 `audioOutput` 도
> 0이 아님**(직접 재현: `th_as_s` · 197). DB 도 같은 방향임 — `pronunciation_attempts` 4행 전부
> `nova_tool` 이고 `target_sound` 가 다 있으며 출처가 실물 마이크 세션 둘임.
> **즉 「앱 프롬프트로는 안 불린다」와 「턴마다 둘 중 하나만」이 둘 다 거짓임.**
> ⛔ **재는 단위를 앱이 도는 단위와 맞추지 않은 것이 원인임** — 단일 발화로 재고 다중 턴을
> 결론했음. 후속은 `TASK-78` 이 소유함. 재결정 전에는 `toolChoice` 강제를 제품 경로에 넣지 않음.
> ⚠️ **그래도 유효한 것**: 단일 발화 범위의 관측(규칙 9 게이트가 문법으로 라우팅 — `TASK-75`) ·
> 보조 신호가 **입력을 받은 적이 없음**(`user` 발화 51건 중 한글 0건 — 직접 쿼리). `TASK-74`.
> ⚠️ **스파이크에 팔 둘이 늘었음**(`--prompt-file`·`--tool-choice`). 기본값이 없어 기본 팔은 앱과
> 글자 그대로 같음 — 그것이 통제 대조의 전제임.

> ⛔ **Phase 1 완료를 선언하지 않았음** — `TASK-47` 로 6항목을 대조했고 **4항목 충족 · 2항목은
> 「충족 판정 불가 — 증거 부재」**임. 판정과 근거는
> `docs/design/2026-09-09-phase1-completion-audit.md` 가 소유하고 **선언 자리는 AC 문서**
> (`2026-08-25-first-slice-acceptance-criteria.md` 「선언 자리」 절)로 정했음.
> ⚠️ **캡틴 결정 B-7 이 지목했던 미충족 셋(W-live·E2E-S·실물 마이크)은 전부 닫혔음** — 항목 4·6 은
> 그 목록에 없었고 이 대조에서 처음 드러났음. **B-7 이 닫힌 것과 6항목 전건 충족은 다름.**
> 남은 둘: **`TASK-70`**(최종 코드 리뷰 — 사후에 돌려도 같은 판정이라 **작업**) ·
> **`TASK-71`**(red→green 증거 부재 — 사후에 만들 수 없어 **`Awaiting Decision`**).
> ⛔ **W-live 는 다시 돌려야 했음** — 기존 기록의 `11/11` 이 낡았음(그 뒤 스크립트 단정이 12개로
> 늘었음). 직접 돌려 **`PASS: 12/12`** 를 얻었음. **첨부된 출력이 현재 코드를 증명하는지 먼저 봄.**
