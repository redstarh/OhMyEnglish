...
`TASK-124` · `TASK-126` · `TASK-127`. 부분 진행 하나: `TASK-39`(AC#1 닫음 — 재시도·타임아웃 명시 · AC#2 는
운영 관찰 항목).
...
`68`(Nova 는 토큰 열 넷으로 speech·text 를 나눠 담음) · `69`(금액은 내지 않음 — 단가의 자리를
두지 않음).
정본은 `docs/ops/captain-instruction-register.md` 임.

## ② 다음 한 걸음 — ⛔ 내 갈래에 열린 태스크가 없음

비용 관측이 한 바퀴 닫혔음 — 적기(Claude·Nova) → 읽기(집계) → 보기(CLI) → 금액 결정(결정 69:
내지 않음). `cd app/backend && .venv/bin/python ../../scripts/usage_report.py` 로 언제든 봄.
⛔ **금액을 내지 않는 것이 결정임** — 다시 열 조건은 결정 69 가 가짐.

다음에 이 갈래로 올 사람이 고를 수 있는 것: 화면·API 를 붙일지(아직 정해지지 않았고 CLI 가 그
자리를 메움) · `TASK-122`(빈 이름 키 · `low`) · `TASK-41`(⛔ 캡틴 결정 31 이 「모든 구현 뒤」로
미뤄 뒀음). ⚠️ **동료 갈래의 결정 대기 셋 가운데 둘을 이 세션이 받아 대장에 남겼음** — 결정 70(`TASK-123`
키 강제를 뺌) · 결정 71(`TASK-118` 전용 모드에서만 구절을 버림). 구현은 `nova.py` 를 소유한
갈래의 몫이라 그 세션에 전달했음. 남은 하나는 `TASK-97` AC#4(결정 49 유지·뒤집기)임.
...
| 2 | 다음 걸음 | ⛔ **내 갈래에 열린 태스크가 없음**(위 ②) — `In Progress` 0건 · `Awaiting Decision` 0건 |
