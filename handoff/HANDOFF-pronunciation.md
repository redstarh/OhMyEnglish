...
최종 갱신 **2026-09-11 11:2x KST** · 브랜치 `design/first-vertical-slice`
...
히스토리 요구사항 · PRD §15 신설 — 구현은 `TASK-107`).
...
**그 사이에 진행할 갈래**: `TASK-107`(연속 학습일 계산·히스토리 화면 — `TASK-3` 설계의 실행) ·
`TASK-102`(시나리오 사전 생성 — 사용자가 UI 로 등록한 것) · `TASK-104`(pending 14건 소화 — 비용과
시점을 먼저 적어야 함).
...
| # | 지표 | 이 마감 시점 값 |
|--:|---|---|
| 1 | 기준 커밋 | **`02cc960` 이상**(등호를 요구하지 않음 — 동료 세션이 커밋·푸시함) |
| 2 | 다음 걸음 | **`TASK-86`**(`Awaiting Decision` — Slack 으로 물었음) · 그 사이 갈래는 `TASK-107` |
...
| 4 | 착수 전 필수 | **8개**(위 ③). `TASK-107` 의 `dependencies` **0건** · `TASK-86` 은 미충족 AC **2건** |
