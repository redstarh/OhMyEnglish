...
| **8** | **결과 화면 발음 카드 (백엔드+프론트)** | **🔄 절반** — 결과 카드 ✅ `9b7985f` / **세션 화면 배지 ⏭** | `9b7985f` | A-2 후단 2건 **해소**. 남은 절반은 A-5 |
...
발명해야 한다). 실물 왕복 0회라 **관측된 키가 0건**이어서, 표시 사전도 `_as_` 분해 규칙도
지금은 발명값이다. 반복 오류 강조는 G-4·Task 9로 값역을 관측한 뒤 §10 미결 3과 함께 다시 본다.
...
⚠️ **검증 수단이 없다.** 스텁 어댑터는 이 프레임을 내지 않고(PS8: 스텁 무손상), 프론트에
테스트 프레임워크가 없고(`package.json`에 test 스크립트 0개), 하네스 `ws_session.py`는
**클라이언트**로 붙으므로 서버→클라이언트 프레임을 주입할 수 없다. 착수 전에 검증 방법을
정해야 한다 — 후보: 실물 Nova 1회(G-4와 묶기) · 스텁에 프레임 주입 스위치 신설 ·
프론트 테스트 프레임워크 도입(새 의존성).
