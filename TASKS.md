...
| 슬라이스 1 — 데이터 유실 정지 | 🔨 | **Task 4/7 완료** (006 → 연습 상황 저장 → 재시도 판정 계약 → **복습 상태 재계산**(`services/review.py`). 게이트 **428 passed**). 남은 것: Task 5(배선) · 6(만성 지표) · 7(문서). **태스크 분해와 순서는 위 계획서가 소유한다** — 여기서 다시 적지 않는다 |
