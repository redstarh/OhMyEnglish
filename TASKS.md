...
| 슬라이스 1 — 데이터 유실 정지 | 🔨 | **Task 6/7 완료** — 실제 대화 1턴이 복습 예정일을 만들고(`next_review_at` 최초 사용 = 슬라이스 1 완료 판정) 만성 지표 쿼리(`services/chronic.py`)까지. 게이트 **439 passed** · 프론트 exit 0. 남은 것: **Task 7(문서 정합화)** + 실물 검증 L1·L2(캡틴 승인). **태스크 분해와 순서는 위 계획서가 소유한다** — 여기서 다시 적지 않는다 |
