...
B-1~B-10이 모두 결정됐다. **결정은 여기, 결정에서 생긴 작업은 D절**이다.
...
| B-2 | **교정 예산 범위** (발음 개입이 문법 교정과 예산을 다툰다) → **✅ "문법 교정을 우선으로 하여 결정."** 설계서 §3.3의 원안이 확정됐다 — 일반 세션에서 발음 개입은 **보조 신호가 떴을 때만**이고 그 밖에는 문법이 우선이다. 지금 `SYSTEM_PROMPT` 규칙 8이 무조건이라 코드가 결정과 어긋난다 → **D-1** | 종결 |
| B-3 | **교정 상한 문구 불일치** (`agent-system-prompt.md:19` "one **or two**" ↔ `nova.py` 규칙 4 "At most **one**") → **✅ "일상 회화 우선으로 결정."** _해석_: 교정이 잦으면 회화가 끊기므로 **한 턴 최대 1개**(코드 쪽)를 정본으로 삼고 요구사항 문서를 코드에 맞춘다 → **D-2** | 종결 |
| B-4 | **발음 `pattern_key` 값역** (§5.6 키 주입 기제가 Nova 경로에 없다) → **✅ "Nova 경로를 필요한 곳에 모두 사용가능하도록 설정. 설계, 코드에 반영."** 즉 문법 워커에만 있는 기존 키 주입을 **Nova 지시문에도** 만든다 → **D-3** | 종결 |
| B-5 | **실물 마이크 1회 + 발음 픽스처 보정** → **✅ "요청하면 진행하겠슴."** 캡틴 대기가 아니라 **내가 요청할 차례다** — 절차·픽스처·확인 항목을 준비해 한 세션에 둘을 함께 닫도록 요청한다 → **D-4** | 종결 |
| B-6 | **M계층 반영 단위** (오류가 대화에 닿는 경로가 0개) → **✅ "오류가 발생할 시나리오를 가정하여 설계 구성."** A(시나리오 선택) 갈래를 채택한다 — 학습자의 약점 패턴이 실제로 튀어나올 상황을 시나리오로 구성해 넣는다 → **D-5** | 종결 |
| B-7 | **Phase 1 완료 선언** → **✅ "미완료 항목은 수정하거나, 추가 테스트 수행."** 선언을 미루지 않고 **미충족 3건(W-live 1회 · E2E-S 1회 · 실물 마이크)을 채운다** → **D-6** | 종결 |
| B-8 | `pending_learning_utterances` 앱 미사용 → **✅ "죽은 코드는 삭제."** → **D-7** | 종결 |
| B-9 | `.claude/settings.json` 커밋 여부 → **✅ "커밋하지 마."** untracked로 둔다. **지우지도 않는다** — 하네스의 워크트리 금지가 이 파일에 의존한다. 후속 작업 없음 | ✅ 완결 |
| B-10 | **문법 프롬프트에 발음 패턴이 새어 들어가 `frequency`를 두 writer가 번갈아 덮는다** → **✅ "두번 틀리면 두번 입력 가능하도록 SQL 생성, 필요시 반복 컬럼 생성, 단순한 모델링으로 해."** _해석_: 한 컬럼을 두 경로가 다투게 두지 말고, **재발을 있는 그대로 세는** 단순한 모델로 바꾼다(필요하면 컬럼을 나눈다) → **D-8**. ⚠️ 스키마가 걸리므로 D-8은 **착수 전 이 해석을 캡틴에게 확인**한다 | 종결 |
...
## D. 결정에서 생긴 후속 작업 (2026-08-30 신설)

B절 결정이 만든 작업이다. **어느 것도 아직 착수하지 않았다.**

| # | 무엇 | 어디를 고치나 | 트랙 | 선행 |
|--:|---|---|:--:|---|
| **D-1** | 발음 개입을 **보조 신호가 떴을 때만**으로 좁힌다 (B-2). 지금 `SYSTEM_PROMPT` 규칙 8이 무조건이라 코드가 결정과 어긋난다 | `audio_gateway/nova.py` 지시문 + 가변부 자리(§11 학습 코치 설계 §5.2) | A | D-5와 같은 통로를 쓴다 |
| **D-2** | 교정 상한을 **한 턴 1개**로 통일 (B-3). 요구사항 문서를 코드에 맞춘다 | `docs/agent-system-prompt.md:19` 한 줄 → 한 줄 (⚠️ 이 파일은 줄 단위 인용 대상 — 함정 H-H) | C | — |
| **D-3** | Nova 지시문에 **학습자의 기존 `target_sound` 주입** (B-4). §5.6 재사용 규약을 발음 경로에서도 실제로 동작시킨다 | `services/analysis.load_existing_patterns`를 발음 카테고리로도 쓰거나 대응 조회 신설 → `nova.py` 지시문 조립. 설계서 §4.3에 기제 명시 | A | — |
| **D-4** | **실물 마이크 세션을 캡틴에게 요청** (B-5). 절차·픽스처·확인 항목을 미리 준비해 한 세션에 마이크 1회 + 픽스처 보정을 함께 닫는다 | `tests/harness/scenarios-P-pronunciation.md`에 요청용 체크리스트 | D | D-1·D-3 이후가 효율적(지시문 확정 후 1회로 검증) |
| **D-5** | §11 M계층을 **시나리오 선택** 갈래로 설계 (B-6). 약점 패턴이 실제로 튀어나올 상황을 시나리오로 구성한다 | `docs/design/2026-08-25-learning-coach-agent-design.md`에 결정 반영 → 구현 계획 | A | — |
| **D-6** | Phase 1 미충족 3건을 채운다 (B-7) — **W-live 1회 · E2E-S 1회 · 실물 마이크** | AC 문서 `:145-154`의 6항목 대조 | A/D | D-4 |
| **D-7** | `pending_learning_utterances` **삭제** (B-8). 참조가 없는지 확인 후 함수와 그 테스트를 함께 제거 | `services/utterances.py:145` | B | — |
| **D-8** | `frequency` 이중 writer 해소 (B-10) — 재발을 있는 그대로 세는 단순 모델로. 필요하면 컬럼을 나눈다 | `error_patterns` 스키마(006) · `services/analysis.py` · `services/pronunciation.py` | A | ⚠️ **착수 전 해석 확인** (B-10 행) |

### D-8 착수 전 확인할 것 — 지금 무엇이 문제인가
