...
| S2-6 | 계획 프롬프트 조립 | 🔨 **고침 라운드 2 반영**(`1aff1ff`) · **재검증 대기** |
...
메시지가 소유한다** (progress.md는 git 무시 대상이라 `git clean` 한 번에 사라진다).
...
"one or two items"를 더한 뒤로 `- focus:`의 개수 제약이 무보호가 됐다). 판별법은
`tests/unit/test_plan.py` 머리말이 소유한다 — **프롬프트·규격 전체를 대상으로 문구를 찾지 않는다.**
