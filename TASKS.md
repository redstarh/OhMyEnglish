...
| 슬라이스 1 — 데이터 유실 정지 | 🔨 | **Task 1~7 코드·문서 완료 · ⏸ 코드 리뷰 대기**. 실제 대화 1턴이 복습 예정일을 만든다(`next_review_at` 최초 사용 = 슬라이스 1 완료 판정) · 만성 지표 쿼리까지. 게이트 **439 passed** · 프론트 exit 0. **리뷰 승인 전 ✅로 올리지 않는다**(`common/code-review.md`). 남은 것: 리뷰 반영 + 실물 검증 **L1**(dev DB에 006 적용) · **L2**(실물 Claude가 `suggested_contexts`·`attempts`를 실제로 내는지) — 둘 다 캡틴 승인 필요. **태스크 분해와 순서는 위 계획서가 소유한다** — 여기서 다시 적지 않는다 |
