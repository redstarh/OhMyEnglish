...
| S2-7 | 계획·노트·수준을 한 트랜잭션에 저장 | 🔨 **구현·커밋**(`45dbe34`) · **독립 리뷰 미착수** |
...
작업자가 스스로 올린 미결 6건 중 **메인이 값어치를 높게 본 둘**:

1. ⚠️ **허용 집합과 프롬프트가 어긋나는 것을 잡는 테스트가 없다.** 작업자의 테스트가 전부
   `due_reviews`의 id 하나만 쓴다 → **`chronic`에서만 고른 초점이 통과하는지**를 아무도 재지 않는다.
   이것은 **S2-6의 Critical과 같은 부류**다(프롬프트가 지목한 출처와 가드의 집합이 갈라지는 것).
   집합과 프롬프트가 같은 `data` 객체에서 나오는 것은 **구조적 보장이 아니라 우연한 일치**다.
2. ⚠️ **`_PoolProbingClaude`의 차별력 뮤테이션을 돌리지 않았다**(작업자 자진 신고). 트랜잭션 밖
   호출 검증이 `pool.get_idle_size()`(asyncpg **내부 API**)에 의존해 비행동적으로 깨질 수 있다.
