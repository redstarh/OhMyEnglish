...
| **G-4** | **실물 마이크 세션을 캡틴에게 요청** (B-5). 절차·픽스처·확인 항목을 미리 준비해 한 세션에 마이크 1회 + 픽스처 보정 + **A-5 실시간 배지**를 함께 닫는다 (2026-08-30 결정 — A-5의 유일한 검증 경로다) | `tests/harness/scenarios-P-pronunciation.md`에 요청용 체크리스트 | D | G-1·G-3 (지시문 확정) → **A-5 구현** → G-4 |
