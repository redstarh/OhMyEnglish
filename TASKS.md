...
>
> 최종 갱신 **2026-09-08** (원장 마이그레이션 `TASK-28` — B·F·H·관련문서지도 절을 Backlog/
> `docs/design/`·`docs/ops/`·`README.md`로 이관·삭제. **이 파일은 더 이상 태스크 원장의
> 유일한 정본이 아니다** — 열린 작업은 `backlog task list --plain` / `backlog board`를 본다.
> 남은 A·C·D·E·G·I·J절은 여전히 이 파일이 소유한다 — 다른 설계 문서가 절 제목으로 인용한다)
> · 브랜치 `design/first-vertical-slice`
>
> 갈래: **A**(§10 발음) · **B**(캡틴 결정 → `docs/design/2026-08-30-captain-decisions-phase1.md`) ·
> **C**(§11 학습 코치) · **D**(하네스) · **E**(비차단) · **G**(결정 후속) ·
> **H**(En-Coach 공유 DB → `docs/ops/shared-database-guide.md`) · **I**(실물 마이크 결함) ·
> **J**(프론트 검증 자산)
...
**원장 마이그레이션(`TASK-28`)으로 이관했다** — 전문(B-1~B-10, 근거 원문 보존)은
`docs/design/2026-08-30-captain-decisions-phase1.md`가 소유한다. 요지: 전건 종결,
후속 작업은 G절(이 파일의 git 이력)이었고 대부분 완료됐다(G-1~G-4·G-7·G-8). **G-5·G-6은
상태가 불명확하다** — 새 문서가 그 캡틴 확인 필요를 명시한다.
...
**원장 마이그레이션(`TASK-28`)으로 축약했다** — F절(전역 규약 정리, 범위 `~/.claude/**`)은
이 리포 밖 내용이라 **삭제**했다(조치는 이미 `~/.claude`에 완료돼 있다). H절의 실측값은
`docs/ops/shared-database-guide.md` §4.2-1로 이관했다.

규칙·접속 정본: **`docs/ops/shared-database-naming-rules.md`** · 배경·구축·실측:
`docs/ops/shared-database-guide.md`. 비대칭 해소(우리 표를 `public`→전용 스키마)는
**`TASK-41`**이 소유한다.

⚠️ **캡틴 결정 대기 — `TASK-42`**: `shared-database-naming-rules.md`에 En-Coach 역할의
평문 비밀번호가 있어(커밋 `7a5bfce`) 이 리포의 GitHub 공개 push가 차단돼 있다. 회전/비공개
전환/이력 재작성 중 하나를 캡틴이 확정해야 한다.
...
**원장 마이그레이션(`TASK-28`)으로 이관했다** — `README.md` §관련 문서 지도가 최신판을
소유한다. "무엇이 남았나" 행은 이제 이 파일이 아니라 `backlog board` · `backlog task list
--plain`을 가리킨다.
