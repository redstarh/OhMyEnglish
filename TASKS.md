...
| S2-6 | 계획 프롬프트 조립 | 🔨 **고침 라운드 3 반영**(`dc05ad3`) · **최종 판정 대기** |
...
`dc05ad3`(라운드 3, 9건) 메시지가 소유한다** (progress.md는 git 무시 대상이라 `git clean` 한 번에
사라진다). 라운드 2 재검증에서 **사양 준수 ✅ · Critical 0**이 됐고, 라운드 3이 남은 블로킹
Important 1건과 Minor 6건을 닫았다.
...
→ 교훈: **창을 좁히는 것으로는 부족하다. 문구를 그 주어에 묶어야 한다**(`_bullet_lead`).
그리고 **경계를 재는 값이 재려는 대상에서 파생되면 안 된다**(`_section_header`).
