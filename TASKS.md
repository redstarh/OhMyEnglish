...
**코드 리뷰 — 독립 리뷰를 못 받았다. 대신 자기 리뷰를 했고 그 한계를 명시한다.**
opus 리뷰어를 두 번 위임했으나 **둘 다 완주하지 못했다**(첫 리뷰어는 diff를 읽던 중간에 기록이
끊겼고 — 그 시각 이 머신이 약 5시간 절전에 들어갔다 —, 두 번째는 출력 파일이 사라졌다). SVG의
**작성자 ≠ 검증자**는 따라서 **미충족**이다. 아래는 구현자 자신의 검증이므로 독립 게이트를 대체하지
않는다 — 다음 세션이 `code-reviewer`에 다시 위임할 대상이다.

자기 리뷰에서 가설 3개를 코드로 직접 확인했다.

| 가설 | 결과 | 근거 |
|---|---|---|
| live 가드가 진행 중 세션을 지키는가 | **지킨다**(가정 2개 명시) | 등록은 세션 행 생성 직후이고, 그 사이 await 지점(`pool.acquire()` 해제)이 있어도 **60초 유예가 그 창을 구조적으로 덮는다**(갓 만든 세션의 나이 ≈ 0초). 해제는 `runner.run()`이 끝난 뒤 `api/ws.py`의 `finally`에서만 일어나므로 **드레인·종료 기록 구간까지 보호**된다 |
| 리퍼와 정상 종료가 경합하면 | **어느 순서든 안전** | 리퍼가 먼저면 `_END_SESSION_SQL`이 status 필터 없이 덮어써 `completed` + 실제 종료 시각으로 수렴한다. 종료가 먼저면 리퍼 WHERE의 `s.status = 'active'`가 재평가되어(READ COMMITTED) 건너뛴다 — 이 필터를 무력화하는 뮤테이션이 red였던 것과 같은 자리다 |
| 무활동 계산이 맞는가 | **맞지만 live 가드에 전적으로 의존한다** | agent final은 **이벤트가 도착할 때** 저장된다(`audio_gateway/session.py` `_save_final`). 즉 긴 agent 턴이나 사용자 침묵 60초는 활동 시계를 갱신하지 못해 **진행 중 세션도 유예를 넘긴다** — 막는 것은 오직 live 가드다. docstring의 "이것이 안전성 전부다"는 과장이 아니라 사실이다 |

**전제 2개를 못 박는다** (깨지면 진행 중 세션이 닫힌다): ① **단일 프로세스** — 다른 프로세스의
리퍼는 이 집합을 모른다. 같은 DB에 백엔드를 둘 띄우면 서로의 세션을 닫는다. ② **세션 생성이
`api/ws.py`를 통한다** — 앱 밖에서 만든 세션은 가드 밖이다.

**자기 리뷰가 찾은 것 (MEDIUM 1건, 문서로 조치)**: ②의 실제 사례가 리포 안에 있다 —
`tests/harness/inject_errors.py`가 앱 프로세스 밖에서 세션을 만들고 `--wait`(기본 **180초**) 동안
발화 없이 기다린다. 그 사이 리퍼가 60초에 그 세션을 `failed`로 닫는다. **관측 가능한 피해는 없다**:
스크립트 끝의 `mark_session_ended(…, "completed")`가 덮어써 최종 상태가 수렴하고 하네스는 status를
단정하지 않는다. 그래서 **동작을 바꾸지 않고** 그 모듈 docstring에 ⚠️로 못 박았다 — Task 9(하네스
5차수)에서 대기 중 `failed`를 보고 회귀로 오인하는 것을 막는 것이 이 조치의 목적이다. 하네스 흐름을
Task 9 직전에 바꾸는 것은 이득 없이 위험만 늘린다고 판정했다.
