...
| S2-6 | 계획 프롬프트 조립 | 🔨 **고침 라운드 1 반영 완료**(`fdfd83f`) · **재리뷰 대기** |
...
⚠️ **S2-6 고침 라운드 1의 내용·근거·red 관측 7건은 커밋 `fdfd83f` 메시지가 소유한다**
(progress.md는 git 무시 대상이라 `git clean` 한 번에 사라진다). 원리뷰 지적 11건 중 **Minor 1~3의
원문은 소실됐다** — 닫힌 세션에 있었고 progress.md에는 요약만 남았다. 부분 패치의 주석에서 복원해
반영했으니 재리뷰가 그 복원이 맞는지 함께 판정한다. 고침 중에 **항진명제 1건을 더 찾아 교체**했고
(부분 패치가 넣은 `assert "\n\n\n" not in prompt` — 고침 전 코드에서도 통과했다)
**출력 규격이 못 덮은 검증 제약 2건**(`extra="forbid"` · 모든 문자열 `min_length=1`)을 넣었다.
