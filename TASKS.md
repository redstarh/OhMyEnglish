...
⚠️ **아직 미수신 3건**(4차 요청, 짧게 쪼개 달라고 했다): **§3 ③**((a) docstring 일치 · (b) `instruction=`
분리 테스트가 컬럼/jsonb 구분을 실제로 재는가) · **§5 재검증이 확인하지 못한 것** · 잘린 "사실 하나만
바로잡는다" 항목. **판정에는 영향이 없다** — (b)는 **M5·M7의 단독 red가 이미 닫았고**(뮤테이션 표),
(a)는 Important-1·Minor-4로 닫혔다. ⚠️ 그래도 **§5(미확인 목록)는 받아 두는 값이 있다** — 재검증이 무엇을
못 봤는지가 다음 사람의 착수 전 점검이 된다. 보고가 **같은 자리에서 세 번 잘렸다**(긴 표를 한 메시지에
담으면 잘린다 — 다음엔 처음부터 쪼개 요청한다).
