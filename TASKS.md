...
| R10-5 · R10-8 교정 예산 | **프롬프트로만.** 코드 강제 없음(설계서 §8 D5-1) — 이 축소는 유지된다. ⚠️ **뒤 문장의 `⏸ B-2`는 낡았다(2026-09-05 정정)**: "지시문 규칙 8이 §3.3보다 넓다"는 어긋남은 **B-2 결정(2026-08-30, 문법 교정 우선)** 으로 닫히고 **G-1(`efc6264`)이 문구로 좁혀 구현**했다. 이 행에 남은 미결은 **코드 강제 부재 하나뿐**이고 그것은 결정된 축소다 |
