...
⚠️ **착수 조건 — 새 지적 N-1~N-4를 T1·T5a 범위에 명세로 못 박은 뒤 T1을 시작한다.** 넷이 **전부
C2 한 행**에 몰려 있고 재결정이 아니라 추가 명세다:
...
| **N-4** | **§7-3의 C2 행이 이제 측정 그 자체라 독립 대조가 없다** — 대조("`partial`만 주입하고 `final`을 막으면 muted 줄만 남아야 한다")가 §7-6 2단계 측정의 1단계와 같다 | 별개 대조를 §7-3에 세운다. ⚠️ **보고 뒷부분이 잘려 처방 전문을 받지 못했다** — T1에서 critic 보고(이 세션 로그)를 다시 확인하거나 같은 관점으로 재도출해라 |
