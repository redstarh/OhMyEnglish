...
**착수 전 필수 — critic 재검증**: 초판이 **CHALLENGE**를 받아 차단 4건 + 흡수 7건을 보완했으나
**그 보완분의 재검증을 받지 못한 채 세션이 마감됐다.** 초판 판정의 최종 답이 "**아니오 — 4개를
보완한 뒤에는 예**"였으므로, 재검증 없이 T1을 시작하면 그 판정을 우회하는 것이 된다.
