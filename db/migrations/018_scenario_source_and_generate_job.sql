...
-- ⚠️ **이 줄은 낡았다 — 정정한다 (2026-09-14 · `TASK-136`)**: `008` 은 「예약 자리」가 아니라
-- **영구 결번**이다(결정 27). 주간 리포트는 착수 턴에 다음 번호를 새로 발급했고 그것이 `023` 이다.
-- ⛔ 번호를 미리 예약하지 않는다 — `sorted(glob("*.sql"))` 순서 때문에 뒤늦게 낮은 번호를 만들면
--    신규 DB 와 기존 DB 의 적용 순서가 갈라진다(그 실측이 결정 27 의 근거다).
--
