...
⛔ **금액을 계산하지 않는다** — 사용자 결정 69(2026-09-12)가 「금액은 내지 않는다」로 정했다.
단가는 공개 문서 값이라 이 리포가 관측할 수 없고, 값의 이력 없이 곱하면 지난달 토큰에 이달 단가를
곱하게 된다. 다시 열 조건은 그 결정이 소유한다.
...
async def main(days: int) -> None:
    ...
    print("⛔ 금액은 내지 않는다 — 사용자 결정 69. 필요하면 이 토큰 수에 단가를 곱한다.")
