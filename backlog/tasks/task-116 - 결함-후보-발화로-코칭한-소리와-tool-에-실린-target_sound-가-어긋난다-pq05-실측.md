...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 재현률을 먼저 센다 — 같은 픽스처로 몇 회 중 몇 건이 어긋나는지(1회 관측을 비율로 읽지 않는다)
...
닫음 2026-09-15 (세션 ohmyenglish-65) — AC#1(재현률)을 새 회차 없이 기존 실측을 모아 닫았음. ⛔ 1회 관측을 비율로 읽지 않았고, 오디오에 없는 소리를 심은 «판별 조건»의 회차만 셌음.

전용 모드: TASK-120 3/3 · TASK-123 ARM-B 3/3 · TASK-128.3 ARM-B 3/3 = 9/9.
일반 세션(WS 레그): TASK-129 3/3.
브라우저 레그: TASK-81 ARM-3 1/1.
합 13/13 — 즉 어긋남은 «거의 항상» 나고 모드·레그가 바뀌어도 같음.

⇒ 어긋남 자체는 남아 있음(결정 82 가 프롬프트를 더 고치지 않기로 정했음). 이 태스크가 닫히는 근거는 «오염 경로»가 막혔다는 것임: 116.1(복습 전진 배제) · 116.3(일반 세션에서 검사가 발화하게 함 · 결정 98) · 116.4(증거 갈림에서 정상 기록 보호) · 116.2(화면이 그 사실을 말함 · 결정 95). 자식 넷 다 Done 임.

⛔ 남은 구멍은 묻지 않고 태스크로 남겼음 — TASK-116.5(문장 전체 인용은 미판정). 그것이 이 축에서 아직 오염이 통과하는 유일한 알려진 모양임.
<!-- SECTION:NOTES:END -->
