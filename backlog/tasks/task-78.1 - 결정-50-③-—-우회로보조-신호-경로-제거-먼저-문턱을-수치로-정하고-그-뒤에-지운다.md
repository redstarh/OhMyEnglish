...
## 2026-09-16 — 결정 120: 문턱 없이 지금 지움 (AC#1 닫힘)

사용자가 선택지 넷 중 「기준 없이 지금 지움」을 골랐음. 결정 50 ① 과 결정 101 을 뒤집었고
그 경위는 캡틴 지시 대장 결정 120 이 소유함. ⇒ AC#1 은 「문턱을 세우지 않는다」로 닫힘.

지운 것: note_transcript(한글 전사 감지기) · record_signal(보조 신호 writer) ·
AssistSignal·AssistOutcome 값역 · 게이트웨이 호출 자리 · 그 경로만 재던 테스트 넷.
⛔ 읽는 쪽은 남김 — signal_source 값역과 결과 화면 구분 렌더. dev DB 에 korean_transcript 행이
3건 실재함(이 턴 실측)이라 값역을 좁히면 그 행을 읽는 경로가 깨짐.

RED 를 먼저 봤음: 한글 전사문에 발음 행이 0 건이라는 단정이 기존 코드에서 실패했음(1건 생성).
게이트: pytest 1262 passed(테스트 넷 제거 반영) · ruff · ruff format 49 files · ty ·
