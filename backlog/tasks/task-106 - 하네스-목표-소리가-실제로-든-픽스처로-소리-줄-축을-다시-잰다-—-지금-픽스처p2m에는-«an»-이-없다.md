...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 계획이 지목하는 소리(an_as_a)가 실제로 든 wav 를 만들고 그 오류가 전사에 어떻게 나타나는지 먼저 적는다 — 전사에 흔적이 0이면 그 사실 자체가 관측값이다
- [x] #2 초점 교란을 없앤다 — 문법 초점이 같은 낱말을 요구하지 않는 구성으로 두거나 그 팔을 «하네스 팔» 로 이름 붙여 제품 팔과 섞지 않는다
- [x] #3 그 픽스처로 팔 A 를 돌려 코칭 발생 · tool 도착 · target_sound 실림을 세고 대조군으로 판별력을 «먼저» 확인한다
- [x] #4 ⛔ 결과가 0 이어도 「소리 줄이 무효」로 단정하지 않는다 — 그 회차가 무엇을 배제했고 무엇을 배제하지 못했는지 갈라 적는다
<!-- AC:END -->
