...
AC#4 회차 2026-09-15 (세션 ohmyenglish-65) — 정본은 tests/harness/runs/2026-09-15-task81-app-leg/README.md 임. ⛔ AC#4 를 닫지 않았음.

브라우저 레그 2회(전용 Chrome :9333 · 프론트 사본 :3001 · 래퍼 백엔드 :8012 · 검증 전용 DB 둘)에서 코칭 0 · pronunciation 프레임 0 이었음. 두 팔 모두 계획이 발음을 초점으로 잡은 것을 session_plans 로 확인했고 ARM-2 는 힌트에 조건절도 없었음.

⛔ 그런데 「앱 경로에서 코칭이 안 난다」로 읽지 않음 — 드라이버(p_app_path.py)가 픽스처 «하나»만 첫 발화로 흘려 왕복이 1회뿐이고, WS 레그(TASK-129 회차 · 코칭 3/3)에서 코칭이 난 자리는 코치의 «둘째 턴» 이었음. 즉 원인 후보가 「브라우저 경로」와 「왕복 수」 둘이고 이 회차가 가르지 못함.
