...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 TDD — 전용 프롬프트에 단수 지목이 «없다» 와 후보 블록이 «있다» 를 반대 방향 단정 둘로 먼저 쓴다
- [x] #2 결정 56·75·71 의 단정 셋이 그대로 통과하는 것을 확인한다 — 줄을 지운 것이 조용히 통과하지 않게 음성 대조를 붙인다
- [x] #3 바이트 게이트가 깨지는 것을 확인하고 «회차 전에 갱신하지 않는다» (순서: 문면 → 회차 → 판정 → 게이트)
<!-- AC:END -->
