...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 스파이크 직결(spike_nova_protocol.py --tools --app-prompt)로 케이스별 실물 왕복을 돌려 Nova ASR 전사가 오류 음소를 남기는지 관측한다 — 원자료는 회차마다 다른 이름으로 남긴다
- [ ] #2 케이스별로 ① 발음 코칭 발화 여부 ② toolUse 도착 여부를 세고 기준선 6.1% 와 대조한다
- [ ] #3 코칭률이 픽스처 축에서 달라지는지 판정한다 — 표본이 작으면 「구별되지 않음」으로 적고 단정하지 않는다
- [ ] #4 ⛔ 워커를 켜지 않고 보존 세션 둘을 파괴하지 않는다 — DB 는 SELECT 만 하고 그 사실을 회차 기록에 적는다
