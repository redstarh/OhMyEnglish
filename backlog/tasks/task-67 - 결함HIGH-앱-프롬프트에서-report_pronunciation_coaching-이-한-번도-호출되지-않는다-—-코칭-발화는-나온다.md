...
⛔ 정정 (2026-09-11 · 다른 세션이 지적하고 팀리드가 코드로 확인) — 위에 적은 「스파이크의 --app-prompt 는 기반 프롬프트만 싣는다」는 참이지만 그 함의가 반쪽이었음.

build_system_prompt docstring(nova.py:245~246)을 직접 읽었음: 「넷 중 아무것도 없으면 결과는 SYSTEM_PROMPT 그 자체다 — 계획 없이 시작하는 경로(AS4)에는 이 함수가 아무것도 덧붙이지 않는다.」

⇒ 즉 --app-prompt 팔이 앱 경로와 다른 것은 「계획이 실린 세션」에서만 참임. AS4(계획 없이 시작하는 경로)에서는 그 팔이 실제 앱 프롬프트와 «글자 그대로 같음». 그래서 그 팔의 관측은 무관한 것이 아니라 제품 경로의 한 갈래를 정확히 잰 것임.

⚠️ 위 노트가 「내가 확인한 것은 프롬프트 길이와 import 경로임」으로 범위를 좁혀 둔 것은 유효함. 낡은 것은 그 사실이 아니라 「그래서 그 팔은 앱 경로가 아니다」로 읽히는 함의임 — H-AL 이 경고한 형태(완료 태스크의 노트가 낡아 다른 세션이 그 위에 판단을 세움)라 여기에 정정을 남김.

⚠️ 그 지적을 준 회차의 정본은 그 세션 소유임(TASK-93 회차 기록 §2.0). 여기서 그쪽 수치를 옮겨 적지 않음.
<!-- SECTION:NOTES:END -->
