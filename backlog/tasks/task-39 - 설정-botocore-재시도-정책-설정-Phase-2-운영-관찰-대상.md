...
- [x] #2 스로틀 재현 또는 관찰로 중복 과금이 사라지는지 확인한다
<!-- AC:END -->
...
**스로틀 분기를 재현해 유추를 측정으로 바꿨음.** 선행 회차(2026-09-12)는 스로틀 재현을 **기각**하고
시간초과 분기에서 유추했음 — 기각 이유가 실물 한도를 넘기는 비용이었는데, 그 비용을 치르지 않고
분기를 탈 수 있음: **로컬 서버가 `429 ThrottlingException` 을 돌려주면** botocore 의 판정 경로만
타고 Bedrock 을 부르지 않음. 실행체는 `runs/2026-09-17-task39-throttle-branch/` 이고 API 비용 0.

| 설정 | 전송 |
|---|--:|
| 제품 정책(`standard` · `total_max_attempts=2`) | **2** |
| 정책 전(botocore 기본 = legacy) | **5** |
| 대조 `total_max_attempts=1` · `3` | 1 · 3 (값과 정확히 일치) |

선행 회차의 유추가 맞았음 — 스로틀 분기도 시간초과 분기와 같은 상한을 따르고 `max_attempts` 의
1 어긋남이 이 분기에서도 없음. 대조 둘을 넣은 이유는 판별력임(제품 정책만 재면 「2회」가 정책 때문인지
서버 구성 때문인지 가르지 못함).

⛔ **AC#2 의 물음에 대한 답은 「사라진다」가 아님** — 중복 과금은 **사라지지 않고 상한이 내려감**.
SDK 시도 5→2, job 재시도(`MAX_ATTEMPTS=5`)와 곱해 최대 청구 횟수 **25→10**. 재시도를 0 으로 만드는
쪽은 선행 회차가 이미 기각했음(계획 job 이 그대로 실패하면 그 세션의 계획이 안 생김). 즉 이 태스크가
정한 것은 「끄는가」가 아니라 「몇 번까지 허용하는가」임.

⚠️ **세우지 못한 것을 회차 기록 §0 에 먼저 적었음**: 실물 한도의 백오프·재시도 예산 소진은 재지
않았고, 「서비스가 첫 시도를 청구했는가」는 클라이언트에서 관측 불가임. 그 둘은 Phase 2 운영 관찰
항목이고 이 태스크에 다시 담지 않음 — 담으면 태스크가 영구히 열림.

⚠️ **게이트에는 설정값만 남김** — `tests/unit/test_config.py` 가 `total_max_attempts: 2` 와
`max_attempts` 키의 부재를 단정함. 전송 수를 스위트에 넣으면 우리 계약이 아니라 botocore 내부 동작에
게이트를 걸게 되어 라이브러리를 올릴 때 엉뚱한 자리가 깨짐.
<!-- SECTION:NOTES:END -->
