...
## ⛔ 정정 (2026-09-18 · `TASK-210` 의 실물 회차에서 드러났음)

**「네 행의 216/0 은 이른 snapshot 이었다」를 단정하지 않는다.** 실물 Nova 를 한 번 태웠는데
전사·응답이 0 이던 회차가 **216 · 0 · in_speech 150 을 그대로 재현**했음 ⇒ 그 값은 「아무것도
일어나지 않은 세션의 정직한 총계」일 수 있음. `B0-r2.json` 의 네 번째 이벤트와 값이 같은 것은
**우연히 같은 지점**일 수 있고, 그 일치를 근거로 세운 인과가 필요 없었음.

⛔ **고침 자체는 그대로 옳다** — 종료 뒤에 오는 이벤트를 못 보면 코치가 말한 사용량을 놓치고, 그것은
`B0-r2.json` 이 직접 보여 줌(output 이 0→270 으로 자람). 그리고 침묵을 붙인 둘째 회차에서 기록이
**input 1,853 · output 15** 로 나왔음 — 216/0 을 벗어났고 output 이 0 이 아님.
⚠️ 즉 **확정된 것은 「고침이 값을 크게 만든다」이고, 확정되지 않은 것은 「앞 네 행의 원인」**임.
<!-- SECTION:NOTES:END -->
