...
- [x] #2 A5-1 의 음성 대조 ①(주입 전 배지 요소 부재)을 오염 없이 평가한다 — 같은 문서에서 재시도를 합성 클릭하지 않는다
- [x] #3 판별력을 확인한다 — 배지 문구를 어긋나게 만든 변형에서 실행체가 exit 1 을 내는 것을 관측하고 그 출력을 첨부한다
