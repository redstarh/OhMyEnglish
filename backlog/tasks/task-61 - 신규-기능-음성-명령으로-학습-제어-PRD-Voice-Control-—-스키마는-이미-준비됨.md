...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 명령 인식 수단을 정한다 — Nova tool 호출로 할지 전사문 패턴으로 할지 근거와 함께 고른다. ⛔ tool 은 이미 작동이 실증됐고 전사문 패턴은 오인식에 약하다
- [x] #2 command_confirmation 을 쓴다 — 스키마가 그것을 두고 있는 이유가 오인식 방어다. 되돌릴 수 없는 명령(종료)은 확인을 거친다
- [x] #3 지원 명령의 최소 집합을 정해 구현한다 — PRD 가 열거한 열 가지를 한 번에 다 하지 않고 시작·종료·반복부터 한다
- [x] #4 한국어와 영어 명령을 모두 받는다 — PRD 가 그 예시를 든다
- [x] #5 판별력을 게이트로 고정한다 — 명령이 아닌 학습 발화가 명령으로 오인되지 않는 음성 대조를 넣는다
<!-- AC:END -->
...
2026-09-15 제품 판단 넷을 사용자와 확정했음 (세션 ohmyenglish-65 · AskUserQuestion). 정본은 docs/ops/captain-instruction-register.md 결정 102 임.

정한 것: ① 명령 표지는 「Oh My English」 접두어 하나(명령 버튼·둘 다 기각) ② 첫 조각의 명령은 「종료」 하나 ③ 확인은 음성 한 번이고 답을 command_confirmation 으로 남김 ④ 한국어·영어 둘 다 받음.

AC#1 을 닫았음 — 수단은 결정 46 이 Nova tool 호출로 확정했고 결정 102 가 그 위의 계약을 채웠음.
⛔ AC#2·#3·#4·#5 는 구현이 남았으므로 닫지 않았음. 서브태스크 셋으로 나눠 등록했음 — TASK-61.1(어댑터 경로와 확인 · 개발 세션 몫) · TASK-61.2(판별력 게이트 · 개발 세션 몫) · TASK-61.3(실물 브라우저 회차 · 이 세션 몫 · 61.1 에 의존).

⚠️ 이 세션의 역할이 통합 테스트라 설계서와 구현을 쓰지 않았음. 개발 세션이 TASK-61.1 을 열 때 결정 102 와 PRD Voice Control 절을 함께 읽어야 함.

2026-09-15 첫 조각이 끝났음 (세션 ohmyenglish-65). 서브태스크 61.1(구현) · 61.2(판별력 게이트) · 61.3(실물 회차) · 61.4(결함 다섯 고침)이 Done 이고, 남은 것 둘을 서브태스크로 등록했음 — 61.5(한국어에서 코치가 침묵함) · 61.6(PRD 의 남은 명령 일곱).

AC 판정 근거:
AC#2(command_confirmation) — 실물에서 확인 답이 그 유형으로 한 행만 저장되는 것을 관측했음(runs/2026-09-15-task61-4-refix-verify §6-3).
AC#3(최소 집합) — 결정 102 ②가 「종료 하나」로 정했고 구현·관측이 끝났음. 반복·천천히·힌트는 대화 규칙 5 로 이미 되고, 「학습 시작」은 범위 밖임(연결이 곧 세션 시작).
AC#4(한국어·영어) — 앱이 둘 다 받음. ⚠️ 실물 인식은 갈림: 한국어 표지 성공, 영어 표지는 합성 음성에서 6회 중 1회. 사용자가 결정 105 로 그 대가를 받아들였음.
AC#5(판별력 게이트) — 61.2 가 양방향 단정과 무력화 여섯으로 고정했음.

⚠️ 이 태스크를 닫는 것은 «첫 조각»의 완료이고 PRD 요구 전체의 완료가 아님 — 그 구분을 61.6 이 갖는다.
<!-- SECTION:NOTES:END -->
