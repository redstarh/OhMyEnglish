...
사용자 결정 넷은 **결정 89** 가 정본임(`docs/ops/captain-instruction-register.md` · 기각한 대안까지 그 표가 가짐) — 여기서 재서술하지 않음. 그리고 결정 87 이 이 태스크를 다음 기능으로 정했음. ⚠️ 그 넷은 처음에 이 노트에만 적혀 대장이 비어 있었고 `TASK-135` 가 뒤늦게 등재했음 — 번호가 시간 순서와 어긋나는 이유임.
