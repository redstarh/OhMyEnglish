...
접힘의 대가에 대한 관측 2026-09-14 — 발음 축이 «새로 돌리지 않고» TASK-128.3 회차 원본에서 다시 셌음.

세션 7건의 pronunciation 프레임: A1~A4 각 2건 · B1 1건 · B2·B3 각 2건. ⛔ 프레임 2건을 「pending 두 개」로 읽으면 안 됨 — 규칙 10 이 한 사건에 두 번 부르므로 2건 = 한 사건의 pending + 판정임. 그 회차 DB 조회도 세션당 시도 1행이었음.

⇒ 코칭 사건이 7세션 모두 «세션당 1건»이었고 **접힘 조건(판정 없는 코칭 2회)이 0/7 로 관측되지 않았음**. 판정이 오지 않은 사건은 1/7 이지만 그 세션도 사건이 하나라 접힐 대상이 없었음.

⚠️ 그쪽이 붙인 경계를 그대로 남김 — 표본 7 · 전부 전용 모드 · 픽스처 둘로 고정이라 **비율로 읽지 않음**. 일반 세션·긴 세션에서의 빈도는 미관측임. 즉 「내가 돌린 조건에서는 그 대가가 발생하지 않았다」까지가 말할 수 있는 것임.
<!-- SECTION:NOTES:END -->
