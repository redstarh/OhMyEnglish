...
- [x] #4 AS4 팔의 기존 결과(0/8)와 대조해 «계획이 tool 도착을 바꾸는가»를 판정한다 — 표본이 작으면 구별되지 않음으로 적는다
...
2026-09-11 (세션 `ohmyenglish-7f`) — AC#4 판정을 회차 기록 §9 에 적고 닫았음. 판정은 «구별되지 않음»임.
근거 둘: `0/8`(AS4) 대 `0/4`(A') 는 Fisher `p=1.0` 이고, 더 근본적으로 **두 팔이 같은 분기**라
「계획 유무」가 tool 도착 경로에 닿지 않음(소리 줄이 없으면 규칙 9·11 이 0 을 예측함). ⛔ 표본을
늘려도 이 축은 갈리지 않으므로 다음 회차는 «소리 줄이 붙은 계획»으로 재야 함 — 그 갈래가 실재하는
것을 `TASK-100` 이 사본 DB 에서 확인했음(제품 지시문 5,181자에 소리 줄 1건). 21회 회차는 `TASK-105`
가 소유함.
<!-- SECTION:NOTES:END -->
