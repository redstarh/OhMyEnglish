...
- [x] #5 검증에 걸린 기록의 화면 표시는 사용자 승인을 받는다 — 복습 큐에서 빼는 것은 결정 82 가 이미 정했으므로 묻지 않는다
<!-- AC:END -->
...
2026-09-14 KST 세션 ohmyenglish-65 — AC#5 를 사용자 승인으로 닫았음. 사용자가 고른 것은 「검증에 걸린 시도를 표시하고 기록만 함·복습에는 쓰지 않음을 함께 말함」이고 기각된 것 둘은 「표시하지 않음」·「정상 시도와 구별 없이 표시」임. 근거 정본은 결정 95(docs/ops/captain-instruction-register.md)이고 여기서 다시 세지 않음. ⛔ 화면 반영 구현은 이 태스크 범위가 아니라 TASK-116.2 로 등록했음.
<!-- SECTION:NOTES:END -->
