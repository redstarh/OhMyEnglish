...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 화면 문구가 «전용 세션인데 후보가 없다» 를 참으로 말한다 — 폴백이라 말하지 않는다
...
확인 2026-09-15 (세션 ohmyenglish-65) — AC#1 은 코드가 이미 참을 말하고 있었고(결정 83 문구 둘) 남은 것은 화면 확인이었음. 후보 0건인 검증 전용 DB 에서 「발음 집중」으로 들어가 관측한 문구가 「발음 연습으로 시작했어요. 오늘 다룰 소리는 대화에서 듣고 고를 거예요.」였음 — 전용 세션임을 참으로 말하고 후보 부재를 폴백이라 말하지 않음. learning_sessions.mode 도 pronunciation 으로 기록됐음.

⚠️ 폴링으로는 못 잡았음(stub 어댑터에서 세션이 곧 끝나 결과 화면으로 넘어감) — 클릭 전에 MutationObserver 를 걸어 한 번이라도 그려진 문장을 모으는 방식으로 잡았음. 정본은 runs/2026-09-15-task116-2-review-excluded-card/README.md §2 임. 실물 모델 사용 0.
<!-- SECTION:NOTES:END -->
