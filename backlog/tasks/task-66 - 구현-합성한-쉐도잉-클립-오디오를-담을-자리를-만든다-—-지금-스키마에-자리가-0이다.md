...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 오디오를 어디에 두고 무엇으로 가리키는지 정한다 — 컬럼 추가인지 별도 표인지, 파일이 리포 안인지 밖인지. 발명하지 않고 기존 학습자 낭독 저장 방식과 대조해 근거를 든다
- [x] #2 R10-7 예외 범위를 스키마에 새긴다 — 011 의 CHECK 주석이 「다른 오디오가 조용히 쌓이는 길을 막는다」고 한 의도를 유지한다. 합성 클립 외의 오디오가 새 자리로 들어오지 못하게 한다
- [x] #3 시드 1건의 clip_end_sec 을 실측값으로 고친다 — 지금 30.00 을 선언했으나 그 전사문의 합성음은 16.64초다(2026-09-09 실측). 합성음에서는 시간 창이 곧 오디오 전체다
- [x] #4 재생 경로(소비자)와 같은 커밋으로 낸다 — 소비자 0곳이면 진행하지 않는다(설계서 §9)
- [x] #5 클립 생성 절차 문서의 마지막 줄을 채운다 — 파일을 어디에 두는가가 정해지면 review 문서 §7.4 가 닫힌다
<!-- AC:END -->
...
완료 2026-09-14 (세션 ohmyenglish-f4). 설계서 2026-09-14-shadowing-clip-audio-design.md · 계획서 …-plan.md 의 여덟 태스크(66.1~66.8)가 모두 끝났음. 사용자 결정 90 이 범위를 정했음.

AC#1(자리와 가리키는 것) — assets/clips/<id>.wav 를 git 이 추적하고 DB 는 파일명만 가리킴. 뿌리는 새 설정 키 shadowing_clip_audio_root 임. ⛔ 학습자 낭독 뿌리를 재사용하지 않은 근거는 스윕이 그 뿌리를 순회하며 걷는 것임.

AC#2(예외 범위를 스키마에 새김) — 022 의 CHECK 둘. 파일명을 id 로 강제해 경로 이탈을 값역에서 막고, source_url 이 있는 클립에는 오디오를 금지해 R10-7 예외를 합성 클립 하나로 좁혔음.

AC#4(소비자와 같은 커밋) — 엔드포인트 GET /api/shadowing/clips/{item_id}/audio + 최소 쉐도잉 화면을 함께 냈음. ⛔ 착수 전 조사가 「엔드포인트가 0곳」이라 적은 것보다 넓었음 — 화면 자체가 0이었고 그 사실이 범위 질문을 다시 만들었음.

AC#5(문서 마지막 줄) — 선행 검토 §7.4 에 닫힘 절을 붙였고 §8 의 미결 하나도 정정했음.

게이트 여섯(이 턴 직접 실행): pytest 1152 passed exit 0 · ruff check exit 0 · ruff format exit 0(217 files) · ty exit 0 · tsc exit 0 · eslint exit 0.
브라우저 회차: tests/harness/runs/2026-09-14-task66-clip-audio.md.

⛔ 남은 것 하나는 TASK-66.9(dev DB 에 022 적용)이고 승인 사안이라 Awaiting Decision 으로 뒀음 — dev DB 는 019 까지 적용돼 있고 020(발음 축)·021·022 셋이 미적용이라 순서와 소유가 함께 걸림. 그리고 dev DB 의 시드가 clip_end_sec=16.64 로 낡았음(리포는 17.36).

⚠️ 사람 청취는 미결임 — 브라우저가 그 파일을 디코드해 재생한 것까지만 확인했음.
<!-- SECTION:NOTES:END -->
