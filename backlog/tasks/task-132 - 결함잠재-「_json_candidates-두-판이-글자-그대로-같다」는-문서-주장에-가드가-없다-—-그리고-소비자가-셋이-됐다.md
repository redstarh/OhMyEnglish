...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 두 판을 «하나로» 할지 «둘로 두고 동일성을 테스트로 못박을지» 정하고 근거를 적는다. ⛔ 새 공개 API 를 만드는 것이 유일한 답이라고 가정하지 않는다 — plan.py 가 그 함수의 경계를 docstring 으로 소유하고 있어 옮기면 근거가 흩어진다는 것이 동료 세션의 판단이었고 그것도 재료다
...
AC#1 닫음 2026-09-13 (세션 ohmyenglish-f4) — 사용자 결정 86: 둘로 두고 동일성을 테스트로 못박음. 원문은 docs/ops/captain-instruction-register.md 가 가짐.

근거: plan.py 의 docstring 이 그 함수의 경계(관용 범위)를 소유하므로 공용 함수로 옮기면 근거가 흩어짐 — 동료 세션의 판단이 그것이었고 사용자가 그 판단을 택했음. ⚠️ AC#2·#3·#4 가 이 결정 전에 이미 그 방향으로 끝나 있었으므로 추가 작업이 0이고, 이 결정은 그것을 정본으로 «확정»하는 것임.
⛔ 되돌리려면 동일성 테스트를 먼저 지워야 함 — 합친 뒤에는 그 테스트가 같은 함수를 두 번 부르는 단정이 되어 뜻을 잃음. 그 비용을 알고 유지를 골랐음.
<!-- SECTION:NOTES:END -->
