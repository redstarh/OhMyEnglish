...
# 7차 시도 — 실물 2차(발음 유발 · 결정 39). **`pronunciation` 프레임이 또 0건**

## ⛔ 보존할 세션 — `9664afd1-3747-4bc4-aaf2-94bb60dc7cfb`

> `status=completed` · `mode=speaking` · `started_at 2026-09-08 21:50:47.839673+00` ·
> `ended_at 2026-09-08 21:52:11.260503+00`. **teardown 하지 않았음**(호출자 소유).
> 1차 실물 세션 `e0c5e580-…` 도 **건드리지 않았음**(생존 확인).
> 백엔드 pid **96473** · `VOICE_ADAPTER=nova` · `WORKER_ENABLED=false` · HEAD `23aed48`.
> 프리플라이트 통과(P5 `pid 96473 · 기동 01:58:43 전 · 소스 34건 → 통과`) · 계측 sha256
> `cbc19fb6…` 일치(`{cache:'no-store'}`) · 세션은 **정확히 한 번만** 열었음.
> 증거: `runs/2026-09-09-run-5-live/frames-9664afd1.json` · `…/transcripts-repr-9664afd1.txt`.

**흘린 WAV 3개**(브리프가 지정한 순서): `p1m.wav`(2.49 s · 음소 치환) → `p1k.wav`(3.05 s ·
강한 억양) → `p2m.wav`(3.04 s · 음소 치환). 3턴 완주(`final` 6 · `agentFinals` 3 · `userFinals` 3 ·
`session_ended` 1 · `sent.end_session` 1 · `audio` 268 수신 · `start` 271회).
⚠️ **조용 판정을 2,000 → 3,500 ms 로 늘렸는데 `interrupted` 가 여전히 2건 났음** — barge-in 은
줄지 않았음. 관측 설계 보정이 실패한 것이고 기록해 둠(판정으로 세지 않음).

## ① `TASK-13` 발음 키 값역 — **`BLOCKED`. 이번에도 표본 0**

| 관측 | 값 |
|---|--:|
| 이 세션의 `pronunciation` 프레임 | **0건** |
| 이 세션의 `pronunciation_attempts` | **0행** |
| 전체 `pronunciation_attempts` | **4 → 4** (늘지 않음) |
| 화면 배지(`p[aria-live="polite"]`) | **0개** |

**발음 오류 픽스처(`p1m`·`p1k`·`p2m`)를 흘렸는데도 도구가 불리지 않았음.** 분자·분모가 둘 다
0이므로 `target_sound` 값역을 분류할 수 없음 → **`BLOCKED`**. 기존 4행은 이 회차가 얻은 값이
아니므로 **분모에 넣지 않았음**(브리프 규약).

⛔ **원인을 단정하지 않음. 이 회차가 새로 배제한 것과 남긴 것을 가름.**
- **배제됨**: 「사용자가 지시 문장을 읽지 않아서」 — 1차의 정황이었음. 이번엔 **발음이 실제로
  틀린 음성**을 흘렸고 그래도 0건임.
- **새 정황**: agent 가 3턴 모두 **발음을 언급하지 않고 내용 질문만** 했음(아래 P3). 즉 모델이
  발음 문제를 **인지한 흔적이 프레임에 없음.**
- **남은 후보 둘**(이 회차가 가르지 못함): ⓐ ASR 이 오류를 **복원해 버려서** 모델에게 도달하는
  텍스트가 정상이다(아래 P2 가 그것을 뒷받침함) ⓑ 앱 프롬프트가 도구를 부르게 만들지 못한다.
  ⚠️ **ⓐ가 사실이면 이것은 「관측 실패」가 아니라 「제품 사실」임** — 발음 오류가 ASR 단계에서
  사라지면 모델이 고칠 대상을 못 본다. 그 판정은 호출자·캡틴 몫임.

## ② `TASK-24` 사각의 크기 — **`BLOCKED`. 분모 0**

발음 시도 **0건**이므로 「발음을 놓쳤는데 전사문이 한글도 아닌」 턴의 비율을 낼 수 없음.
**분모 0 을 그대로 적음** — 0/0 을 「0 %」로 적지 않음. 참고 사실: 사용자 전사문 3건 전부
**영어**이고 한글 0건임(그 자체가 사각의 크기는 아님).

## ③ `P2`·`P3`·`P4` — 하나는 재현, 하나는 기대대로, 하나는 **미재현**

`transcripts-repr-9664afd1.txt` 가 문자 단위 비교를 담고 있음.

| 시나리오 | 기대(픽스처 정본 §4) | 관측 | 판정 |
|---|---|---|---|
| **P2** (`p1m`) | `i think i found three very useful videos.` — **원문으로 복원** | seq1 = **정확히 같음**(`== True`) | **`PASS`** — 「앱은 발음 오류를 인지하지 못한다」가 앱 경로에서 확정됨 |
| **P2** (`p2m`) | `i finished the report and shared the results with my team.` | seq5 = `'i finished the le port and chaille de lessalts with my team.'` — **다름**(`== False`) | **`FAIL`**(기대 불일치) — 아래 |
| **P3** (`p1m`·`p2m`) | agent 가 발음을 **언급하지 않는다** | 3턴 모두 내용 질문만(`videos were about?` · `what kind of project`). 발음 언급 **0건** | **`PASS`** — 4차수 결론이 앱 경로에서 유지됨 |
| **P4** (`p1k`) | `아이싱크 아이파운드 쓰리 베리 유스풀 비디오즈` — **한글 전사** | seq3 = `'i think i found three very useful videos.'` · **한글 0자** | **`FAIL`**(미재현) |

⛔ **P2 의 `p2m` 과 P4 의 `p1k` 가 사전 실측과 어긋났음 — 고치지 않고 보고함.**
- **`p2m`**: ASR 이 복원하지 못하고 **왜곡된 채로** 전사했음(`the le port` · `chaille de lessalts`).
  즉 **`p1` 쌍은 복원되고 `p2` 쌍은 복원되지 않음** — 「ASR 이 음소 오류를 복원한다」는 성질이
  **문장에 따라 갈린다.** 이것이 `p2` 쌍을 처음 돌려 얻은 사실임(5차수 이월분).
- **`p1k`**: 같은 픽스처가 사전 실측(`runs/2026-08-26-P-preflight/`)에서는 한글이 됐는데 이번엔
  **영어로 정확히** 전사됐음. ⚠️ **「사라졌다」고 단정하지 않음** — 표본 1회이고, 사전 실측은
  **Nova 직접 왕복**이었고 이번은 **앱 경로**임(`NOVA_ENDPOINTING_SENSITIVITY=LOW` 등 설정이 다를
  수 있음). 두 경로 중 무엇이 갈랐는지는 이 회차가 가르지 못함.

## ④ `P9` 종단 — **얻지 못했음.** 입력이 없었음
...
**발음 시도가 0건이므로 `refresh_review` 가 발동할 입력이 없었음.** `outcome` 값을 적을 대상도
없음(브리프가 `pending` 이면 시계가 안 걸리는 것이 정상이라 했으나 **그 판별에 쓸 행 자체가 없음**).
⚠️ 문법 경로도 아직임 — `analyze_utterance` 3건이 `pending`(워커 꺼짐).

## ⑤ `O-1` — 1차와 **같은 형태로 재현**

| 대상 | 선행 공백 | 선행 개행 | 이중 공백 |
|---|--:|---|---|
| DB agent 3행 | **0** | **없음** | seq4·seq6 **있음** · seq2 **없음** |
| DB user 3행 | 0 | 없음 | 없음 |
| **`partial` 프레임 4건** | **1·2·1·2자** | 없음 | — |

선행 공백 partial 4건: `' For example…'` · `'  Were they …'` · `' Could you t…'` · `'  Was it a s…'`.
**1차와 같은 부류임**(선행 개행 0건 · partial 선행 공백 · final 이중 공백). ⚠️ **다만 이번에는
agent final 3건 중 1건(seq2)에 이중 공백이 없었음** — 조각 경계가 그 턴에서는 단일 공백으로
합쳐졌음. 즉 **이중 공백은 항상 생기는 것이 아니라 조각 경계에 달렸음.**

## DB — teardown 하지 않았음

| 표 (쿼리) | 실물 2차 전 | 실물 2차 후 (**그대로 둠**) |
|---|--:|--:|
| `learning_sessions` (`where user_id='0…001'`) | 13 | **14** (+1) |
| `analysis_jobs` (전체) | 49 | **53** (+4 = `analyze_utterance` 3 + `plan_next_session` 1) |
| `utterances` (전체) | 120 | **126** (+6) |
| `error_patterns` (시드) | 8 | 8 |
| `pronunciation_attempts` (전체) | 4 | 4 |
| `review_tasks` (전체) | 10 | 10 |

**§8-0 drift(§8 정의 쿼리) = 0 — 읽기만 했음.** 1차 실물 세션 `e0c5e580-…` **생존** ·
보존 세션 5개 **생존**. 프로세스: pid **96473** 동일 · 플래그 동일 · `/health` `{"status":"ok"}` ·
임시 CORS 서버 종료(8899 `000`). 픽스처는 호출자 소유라 건드리지 않았음.

## 내가 확인하지 못한 것

- **`TASK-13`·`TASK-24`** — 두 회차 연속 표본 0. **발음 오류 음성으로도 도구가 불리지 않는다**는
  사실만 새로 얻었음. 남은 후보 ⓐ(ASR 복원) · ⓑ(프롬프트)를 가르는 것은 **앱 코드·프롬프트를
  보는 일**이고 이 회차의 범위가 아님.
- **`p1k` 한글 전사가 왜 사라졌는가** — 앱 경로 대 Nova 직접 왕복의 차이인지 표본 흔들림인지
  가르지 못했음.
- **`p2` 쌍의 `p2a`·`p2k`** — `p2m` 하나만 흘렸음(브리프가 3턴까지 허용했고 그것을 썼음).
- **N8** — 워커가 꺼져 있어 job 6건(두 세션 합)이 `pending` 임.
