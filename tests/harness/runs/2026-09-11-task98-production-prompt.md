...
## 6. 하지 않은 것 — ⛔ **§1~§5 구간에 한정된다. §8 은 다르다**

**§1~§5(프롬프트 조립 + 8회 회차)**: 워커 미기동 · DB **SELECT 만** · 소스 미수정 ·
`--tool-choice` 미사용 · 보존 세션 불가침. 실물 Nova 왕복 8회.

⛔ **§8 에서는 사용자 결정(결정 53)으로 워커를 켜고 DB 를 썼다** — 그 구간의 조작과 복원은 §8.3 이
갖는다. **이 절을 그대로 읽으면 회차 전체가 SELECT 만 한 것으로 오독된다.**

**이 세션 누적 실물 호출**: Nova 왕복 **50회** + 계획 생성 Claude **1회**.
