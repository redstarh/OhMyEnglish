...
## 9. AC#4 판정 — 「계획이 tool 도착을 바꾸는가」는 **구별되지 않음** (2026-09-11 · 세션 `ohmyenglish-7f`)

대조한 두 팔은 아래다. 둘 다 **소리 줄이 없는 갈래**다.

| 팔 | 프롬프트 | 계획 | tool 도착 |
|---|---|---|--:|
| AS4(기존) | 제품 조립기 · 인자 넷 비움 = `SYSTEM_PROMPT` | 없음 | **0/8** |
| A'(§3) | 제품 프롬프트 · 계획 실림 · 소리 줄 없음 | 있음(낡은 계획) | **0/4** |

판정은 **구별되지 않음**이고 근거가 둘이다. 하나만으로 적으면 다음 세션이 표본만 늘린다.

1. **표본이 작다.** `0/8` 대 `0/4` 는 Fisher 양측 `p=1.0` 이다. 그리고 코칭 기준율 13%에서
   4회 전부 0일 확률이 **0.564** 이므로(§5) 이 팔의 0 은 아무것도 배제하지 못한다.
2. ⛔ **더 근본적으로 판별력이 없다 — 두 팔이 같은 분기에 있다.** 계획이 실려도 **발음 초점이
   없으면 소리 줄이 붙지 않고**(`nova.py:339~340`) 규칙 9·11 이 온전히 살아 0을 예측한다(§3).
   즉 「계획 유무」가 tool 도착 경로에 **닿지 않는다.** 표본을 늘려도 이 축은 갈리지 않는다.

⇒ ⛔ **이 축을 재려면 「소리 줄이 붙은 계획」이 필요하고, 그것이 실재하는 것을 이 턴에 확인했다**
(`TASK-100`). 개발 DB 의 **사본**에서 제품 경로로 계획을 다시 생성해 얻은 값:

| 확인 | 값 |
|---|---|
| 저장 | `session_plans` 2행 → **3행** · job `status=done attempts=1 last_error=null` |
| 초점 | `[pronunciation_an_as_a, article_missing_before_noun]` — 발음이 **첫 자리** |
| 지시문 | 제품 조립기 `build_system_prompt` 산출 **5,181자**에 `- Sound to coach today: "an_as_a"` **1건** |

즉 §4 의 *"지금 코드로 계획을 다시 생성하면 소리 줄이 붙은 갈래가 「제품 데이터」로 성립한다"* 는
**실측으로 확인됐다.** ⚠️ 그 확인은 사본에서 했으므로 **공유 dev DB 는 여전히 계획 2행이고 낡았다** —
21회 회차는 공유 DB 에서 계획을 다시 생성하는 것부터 시작한다(결정 53 이 그것을 승인했다).

⚠️ **`TASK-100` 이 지목했던 「JSON 거부」는 이 판정을 막지 않는다** — 같은 세션·같은 프롬프트로 2회
재시도해 둘 다 저장까지 갔다(실패 1 · 성공 2 → 간헐적). 원인은 미확정이고 거부 사유가 이제 원문
길이·디코드 사유·끝부분을 담아 다음 실패가 스스로 원인을 말한다.

⛔ **`session_plans` 정체(2026-09-08 이후 0건)의 원인은 그 결함이 아니다** — pending 15건이 전부
`attempts=0`·`last_error` null 이고 떠 있는 백엔드 환경이 `WORKER_ENABLED=false` 다(`ps -Eww` 확인).
아무도 그 job 을 집지 않았다. `TASK-104` 가 그것을 소유한다.
