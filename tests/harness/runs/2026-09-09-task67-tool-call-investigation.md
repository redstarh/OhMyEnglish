...
> ⛔ **2026-09-09 정정 — 이 문서의 전제가 너무 강했다. 「앱 프롬프트로는 불리지 않는다」가 아니다.**
> 조사를 마친 뒤 DB 를 직접 쿼리해 얻은 사실: `pronunciation_attempts` **4행이 전부
> `signal_source='nova_tool'`** 이고 **`target_sound` 가 4행 전부에 있다**
> (`am_as_i_m`·`w_as_vw`·`an_as_a`·`an_as_a`). 그 네 행은 **실물 마이크 세션**에서 왔다 —
> `bbfc3908`(mic-1) 3행 · `7b43ce56`(mic-2) 1행. **앱 프롬프트로 돌아간 실물 세션에서 Nova 가
> tool 을 불렀고 `target_sound` 까지 채웠다.**
>
> **그래서 아래 관측의 참인 범위는 이것이다** — 앱 프롬프트 + **스파이크 경로** + `p1k.wav`
> 픽스처에서 tool 이 0건이다. 「앱 프롬프트가 tool 을 막는다」로 넓히면 거짓이다.
>
> ⚠️ **이것은 경로 차이의 또 한 사례다** — 같은 `p1k.wav` 가 스파이크에서는 한글로 전사되고
> 앱 경로에서는 영어로 복원된다(다른 세션이 원자료로 확인). 그 문제의 소유자는 `TASK-65` 이고,
> 이 회차의 0건도 그 갈라짐 안에 있을 수 있다. **배제하지 못했다.**
>
> ⛔ **결정 49 가 이 전제 위에서 내려졌다** — 대장의 결정 49 항목에 정정을 남겼다.
> 아래 §1~§6 은 **그때 관측한 그대로 보존한다**(무엇을 재서 무엇을 얻었는지가 근거다).
