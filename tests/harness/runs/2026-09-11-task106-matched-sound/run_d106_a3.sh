#!/bin/bash
# TASK-106 — 하네스 팔 A2(onlysound · 소리 키를 픽스처 오류에 맞춤) 8회 + 대조군 B 2회.
...
PF=$RD/prompt_onlysound.txt
N=${1:-8}
for i in $(seq 1 "$N"); do
  TS=$(date -u +%Y%m%dT%H%M%SZ)
  echo "########## 팔 A3(onlysound · 초점 발음 1개) $i/$N — $TS"
  $PY "$SP" --wav p2m.wav,p2a.wav --tools --prompt-file "$PF" \
    --out "$EV/D106-onlysound-p2m+p2a-$TS.json" 2>&1 | tail -6
  sleep 3
  if [ $((i % 4)) -eq 0 ]; then
    TS=$(date -u +%Y%m%dT%H%M%SZ)
