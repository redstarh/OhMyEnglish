...
>    최신 행이 `2026-09-11 00:08` → **`2026-09-11 22:41:27`** (`24597f0f`).
>    ⚠️ **이 값을 처음 `22:40` 으로 적었고 그것은 마지막 재시도가 끝나기 «전에» 읽은 값이다** —
>    동료 세션이 `22:41` 을 세어 어긋남을 알렸고 다시 조회해 정정했다(2026-09-12).
>    ⛔ **교훈**: 「그 턴에 직접 돌린 출력만 적는다」로는 부족하다 — **마지막 단계가 끝난 뒤에
>    다시 읽어야 한다.** 여기서 마지막 단계는 `24597f0f` 의 2회차 재시도였고 그것이 최신 행을 만들었다.
