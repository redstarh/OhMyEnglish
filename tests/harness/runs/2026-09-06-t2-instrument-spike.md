...
## 6. 추가 보고 — 관측과 추론의 경계 (에이전트가 스스로 그었다)

`started.count`가 0/1로 흔들리는 **기전**은 일부가 추론이다. 다음 세션이 추론을 관측으로 읽지 않게
그 선을 그대로 옮긴다.

**관측**: 3회차 시각(ms) — 오디오 프레임 `43497 · 43497 · 43501` · `AudioWorkletNode("pcm-capture")`
생성 `43498` · `session_ended 43503` · 소켓 개설 `43481`. `started = {count: 1, when: [0]}`.

**추론(직접 관측 아님)** — 셋 다 에이전트가 명시했다:
1. "43501 프레임이 통과한 것"은 **순서에서의 추론**이다. 계측이 `start` 호출을 **어느 프레임에서 났는지
   태그하지 않는다.**
2. "43501이 `voiceRef.current` 대입 이후였다"도 추론이다 — **대입 시각 자체를 재지 않았다**
   (43498은 `AudioWorkletNode` **생성자** 시각이고 대입은 `VoiceIo.start()` 반환 이후다).
3. **2회차는 시각 추적 계층이 없었다.** "세 프레임 전부보다 늦었다"는 `count = 0`과 3회차 기전에서
   **역추론한 것**이고 그 회차의 직접 시각 관측은 없다.
4. `when: [0]`이 컨텍스트 suspend 때문인지 `currentTime`이 아직 한 렌더 퀀텀(16kHz에서 8ms)도 지나지
   않아서인지 **가르지 못했다** — 43498과 43501 사이가 3ms라 `running`이어도 `0`이 나온다.

→ **확정된 것은 결론뿐이다: 세션이 22~23ms라 모듈 로드와 프레임 도착 순서가 회차마다 뒤바뀌고
그래서 `3`은 나올 수 없다.** 기전의 세부는 계측에 프레임 태깅이 없어 미확정이다(`TASK-31`).

### teardown 결함의 두 번째 차원 — 범위

트리거는 §8-②의 단일 UPDATE이고, 그 서브쿼리가 `where ep.user_id = <시드 사용자>`로만 좁혀
**회차가 건드리지도 않은 패턴 7건 전부를 다시 썼다**(실측 `UPDATE 7`).

### ⚠️ baseline은 파일이 아니라 DB 표였다 — 팀리드가 파일 사본을 떴다

`harness_pattern_baseline`은 §8-0이 회차 시작마다 `drop`하고 다시 만든다. 즉 **teardown이 ②를 못
끝내고 죽은 뒤 새 회차가 열리면 오염된 값이 새 baseline이 되고 원값의 사본이 영구히 사라진다.**

팀리드가 그 자리에서 확인·조치한 것:
- 현재 상태 — baseline 표 **7행** · 표 대 `error_patterns` **drift 0** · 발음 값
  `2 / 2026-09-03 13:51:26.680389+00`. **정합하다.**
- 검증된 사본을 파일로 떴다 → **`tests/harness/runs/2026-09-06-pattern-baseline.tsv`**(추적됨).
