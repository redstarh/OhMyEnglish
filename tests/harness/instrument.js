...
(() => {
  ...
  /**
   * A1-5 판정을 **골라내기 없이** 낸다. 기대 배열을 넣으면 **넷**을 함께 재서 돌려준다
   * (`terminalMatches` · `noExcess` · `reachedExpected` · `nonDecreasing`) + 진단 필드와 `verdict`.
   * ⚠️ **이 줄이 「셋」으로 낡아 있었다**(5차 리뷰가 어휘 스윕으로 잡았다) — 바로 아래 규칙을 이 줄이
   * 어겼다. **개수를 말하는 문장은 조건을 더할 때마다 함께 고친다.**
