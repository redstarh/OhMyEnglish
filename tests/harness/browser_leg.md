...
> ## ⛔ 판별력 미확인 단정 7건 — **재설계 전에는 `PASS`로 보고하지 않는다**
>
> **독립 리뷰(codex, 2026-09-06)가 찾았다.** 상세와 공통 형태는
> `docs/design/2026-09-06-review-outcomes.md` §4가 소유한다 — 여기서 재서술하지 않는다.
>
> **A1-4 · A1-5 · A1-7 · A3-1 · A4-1 · A4-2 · A5-1**
>
...
>
> **새 단정을 쓸 때 이 셋을 먼저 배제한다**:
> ① **계수 지점이 효과 지점과 다르다** — 만든 것을 세고 쓴 것을 세지 않는다.
> ② **구조만 세고 내용을 보지 않는다** — 껍데기가 남으면 통과한다.
> ③ **기대값이 0이 될 수 있다** — `0 == 0`은 구현이 없어도 성립한다.
>
> **규약**: 단정을 채택하기 전에 **그 대조가 무력화에서 실제로 FAIL을 내는지** 확인한다.
> 확인하지 않았다면 "대조 있음"이 아니라 **"판별력 미확인"**으로 적는다.
> 후속은 원장 `TASK-29`가 소유한다.
