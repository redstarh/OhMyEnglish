...
>
> **자기완결이다.** DB 접근·teardown·프리플라이트를 `tests/harness/README.md`나 `docs/ops/2026-08-26-test-harness.html`에 **위임하지 않는다.** 그렇게 정한 이유는 그 두 문서가 dev DB 이관(podman `:5433` → homebrew `postgresql@17` `:5432`) 뒤에도 `podman exec` 명령을 들고 있었기 때문이다(함정 H-T) — 따라가면 앱이 보는 DB가 아닌 곳을 건드린다.
>
> ⚠️ **그 명령들은 2026-09-17에 고쳤다**(`TASK-149`·`TASK-152` — 두 문서 모두 `psql_cli.py`나 psql 절대경로를 가리킨다). **그래도 이 문서의 자기완결은 유지한다**: 위임하지 않는 값은 「그 문서가 지금 맞는가」가 아니라 **판정 근거가 한 곳에 있는 것**이고, 그 두 문서는 이 레그가 재지 않는 범위(러너 기동·A계층)까지 함께 갖고 있어 낡는 속도가 다르다.
>
...
| A3-3 | 드릴 계약 | **결과 화면은 수를 그리지 않는다**(캡틴 결정 10·18 · `TASK-34`). 두 갈래다 — ① 미달 안내 문장은 **미달일 때만** 그려진다(달성 문장도 점수판이 된다) ② `main` 안의 숫자 토큰이 **API 가 실어 보낸 학습자 문구의 숫자**에 전부 포함된다 — ⓑ+ⓒ 혼합(문장은 ⓒ 하드코딩 · 기대 숫자 집합은 ⓑ 런타임 유도). ⛔ **드릴 두 수를 문자열로 찾지 않는다** — 교정문에 우연히 든 수가 오탐이 되고 `아홉`·`2회` 같은 다른 표기로 새면 못 잡는다. ⛔ **`bodyText` 가 아니라 `main` 만 읽는다** — `next dev` 주입 스크립트에 숫자가 섞여 항상 FAIL 이 난다 | **일부러 두 수를 렌더하면 FAIL** — 2026-09-09 에 임시로 `<p>{observed} / {expected}</p>` 를 넣고 회차를 돌려 `['2', '20']` 을 관측했고 곧 되돌렸다. **오탐 대조도 함께 있다** — API 가 준 교정문의 숫자는 FAIL 을 내지 않는다(`test_c3_gates.py`) |
...
### 8-3. ⛔ 표 건수가 기준선과 다를 때 — 회귀로 읽기 전에 **누구 것인지 먼저 가른다**

**2026-09-10 실측**: 한 세션이 마감 기록에 8개 표를 「전부 기준선」으로 적었는데 몇 시간 뒤
`learning_sessions`·`utterances`·`pronunciation_attempts`·`analysis_jobs` 넷이 **높아졌다.**
회귀도 그 세션의 잔여도 아니라 **다른 세션이 앱 경로 세션을 돌리는 중**이었다.
⛔ **이 리포는 여러 세션이 같은 dev DB 를 동시에 쓴다** — 표 건수는 **내 것만의 함수가 아니다.**

**가르는 순서 (전부 읽기다)**:

1. **내 잔여인지 본다** — 내 회차 기록의 세션 ID 를 **명시로** 조회한다. 0건이면 내 것이 아니다
2. **남의 것을 본다** — `select left(id::text,8), status, started_at from learning_sessions
   where started_at > '<내 회차 시각>' order by started_at`. ⛔ **잡힌 것을 지우지 않는다.**
   세션 ID 와 `started_at` 을 보고에 적고 판정을 호출자에게 넘긴다(`H-AM` 과 같은 부류다)
3. **불변으로 단정할 수 있는 것만 지표로 쓴다** — `schema_migrations` 와 **§9 보존 세션의 결과 API
   상태**다. 그 둘은 남이 세션을 만들어도 바뀌지 않는다
4. **`error_patterns`·`review_tasks` 가 다르면 그때는 의미가 있다** — 그 둘은 세션 생성만으로는
   늘지 않고 **분석 워커를 지나가야** 바뀐다(`H-AY`)

⛔ **그래서 인계 문서(`handoff/**`)에 표 건수를 적지 않는다.** 적는 순간 낡고, 다음 세션이 그것을
회귀로 오독한다. 적을 자리는 **회차 기록**이고 거기서는 「그 회차의 삭제 직후 값」이라는 시점이
분명하다. 이 리포가 실측한 유일한 구조적 해법이 **개수를 세는 서술을 아예 세지 않는 서술로 바꾸는
것**이다.
...
- **단정 목록** — C1 `A1-0~A1-8` · C2 `A2-1~A2-3` · C3 `A3-1~A3-3` · C4 `A4-1~A4-2` · C5 `A5-1~A5-2`. **2026-09-06 재설계는 단정을 늘리지 않고 7건의 내용을 바꿨다.** ⛔ **총계를 적지 않는다** — 단정이 늘 때마다 낡는다. 이전 판이 「단정 18건」이라고 적었고 2026-09-09 에 `TASK-34` 가 `A3-3` 을 더하자 **실제로 낡았다.** 개수가 필요하면 이 목록을 세면 된다.
...
- **유도 방식 분류(재설계로 바뀌지 않았다)**: ⓐ 픽스처 연역 — `A1-1~A1-5`, `A1-8` · ⓑ 런타임 유도 — `A1-0`, `A1-6`, `A1-7`, `A2-1~A2-3`, `A3-2`, `A4-1`, `A4-2`, `A5-2` · **ⓒ 하드코딩 — `A3-1`, `A5-1`** · **ⓑ+ⓒ 혼합 — `A3-3`**(문장은 화면이 소유하고 기대 숫자 집합은 API 에서 유도한다). ⛔ **여기도 개수를 적지 않는다** — 위 줄과 같은 이유다. 재설계가 바꾼 것은 **계수 지점과 대조**이고 기대값의 출처는 그대로다. ⓒ 2건은 이제 전역 검색이 아니라 **요소 지목 + 등호**로 재므로 항진명제·동적 문구 경로가 **둘 다 닫혔다** — 「다른 출현은 전부 주석이다」라는 grep 근거에 더는 의존하지 않는다.
