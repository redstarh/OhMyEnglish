...
| 계층 | 질문 | 구현 상태 | 1회차 결과 |
|---|---|---|---|
| **E** | 반복 오류를 **패턴으로 인식·병합·순위화**하는가 | 구현됨 | E1·E2·E3 **PASS** |
...
5. **`target_form`의 의미**(F-2와 직결) — 패턴 수준의 일반화된 형태인가, 문장별 교정형인가.
   전자면 프롬프트가 일반형을 요구해야 하고, 후자면 컬럼이 `error_occurrences`로 내려가야 한다.
