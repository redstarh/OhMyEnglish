...
def classify_failure(o: dict) -> str:
    ...
    **가장 상류의 지표는 `resumeLog` 길이다** — 계측의 `getUserMedia` 대체본이 호출마다
    ...
    if recv.get("session_ended", 0) >= 1:
        # ⚠️ **이 갈래는 래퍼를 실제로 발동시켜 보다가 발견했다** (2026-09-06):
        #    `stub`(비-unresponsive)에
        #    대고 주입 회차를 돌리면 세션이 22 ms 에 정상 종료하고 앱이 결과 화면으로 이동해
        #    **전사문 컨테이너가 사라진다.** 그때 진단이 "H-AE 나 세션 길이"로 떨어졌는데 원인은
        #    **어댑터 모드를 잘못 골랐다**는 것이다 — 전혀 다른 처방이다.
        return (
            "세션이 이미 **정상 종료**했다(`session_ended`) → 앱이 결과 화면으로 이동해 전사문"
            " 컨테이너가 사라졌다. 주입 회차라면 **어댑터가 `stub_unresponsive` 가 아니다**"
            " (§6 주입 창은 그 모드에서만 열린다)."
