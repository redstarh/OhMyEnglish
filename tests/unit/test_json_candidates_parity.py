...
⛔ **`test_plan_models.py::test_fenced_regex_matches_analysis_module`(M9 · 2026-09-04)과 중복이
아니다 — 축이 다르다.** 그쪽은 `_FENCED.pattern` **문자열**을 비교하고 이쪽은 `_json_candidates`
**출력**을 비교한다. ⇒ 패턴 문자열이 같아도 **함수 본문이 다르면** 출력이 갈리는데 M9 는 그것을
못 잡고, 반대로 동등한 다른 정규식이면 이쪽만 통과한다.
⚠️ **하나를 지우려면 어느 구멍이 열리는지 먼저 보라.** 그리고 AC#1 이 「하나로 합친다」로 정해지면
**둘 다 무의미해진다**(같은 함수를 자기와 비교하게 된다) — 그때는 둘을 함께 지운다.
"""
