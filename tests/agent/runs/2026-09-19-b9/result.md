...
| 종료 HEAD | `2b537ec` — **앱 소스 무변경.** 회차 중 `git log` 가 늘지 않았음 |
...
**공유 인스턴스 판정**: 회차 중 `find app -newermt '-30 minutes'` 가 0건이고 `git log` 도 늘지
않았음. 포트 둘은 호출 세션이 소유한 것이라 죽이지 않았음. 전역·공유 상태는 바꾸지 않았음.
