...
| 시나리오 | 착수 시 | 회차 끝 |
|---|---|---|
| `TS-23` | `In Progress` · AC 0/3 | `Done` · AC 3/3 — §10 이 그 경위를 가짐 |
| `TS-24` | `In Progress` · AC 0/3 | `Done` · AC 3/3 — §11 이 차단 해제를 가짐 |
| `TS-25` | `In Progress` · AC 0/3 | `Done` · AC 3/3 — §10 |
...
| `TS-24` | #1 오류 요약이 사용자 타임존 기준으로 갈림 | **회차 중 차단 → 승인 뒤 통과** | §6-1 이 차단을, §11 이 통과를 가짐 |
...
## 10. 회차 막바지에 AC 문면이 고쳐졌음 — 그 뒤의 최종 상태

⛔ **이 절은 다른 세션의 개입을 기록하는 자리임.** 위 §4 의 판정표는 **회차 중의** 판정이고,
이 절이 그 뒤에 일어난 일과 최종 상태를 가짐.

`TASK-231` 을 등록하고 내가 커밋(`7fd236c`)을 낸 직후, **호출 세션이 `TS-23` 과 `TS-25` 의 AC
문면을 고쳤음**(`06:05 UTC`). 고친 방향은 `TASK-231` 의 「제안」과 같았음 — 구현이 아니라 AC 를
고쳤음.

| 시나리오 | 고쳐진 AC | 내 측정이 그것을 덮는가 |
|---|---|---|
| `TS-23` #3 | 「계획이 있으면 엔드포인트가 추천 이유와 난이도를 줌 — 질문과 초점 패턴은 의도적으로 내리지 않음」 | **덮음.** `evidence/01`·`23-final-next-plan.json` 이 `reason` 과 `target_level` 둘을 담음. 저장된 계획 6건이 네 요소를 전부 갖는 것도 DB 로 확인했음 |
| `TS-25` #3 | 「학습이 없던 날도 목록에 보임 — 빈 목록 상태가 성립하지 않는 것이 의도임」 | **덮음.** `evidence/17` 의 날짜 30줄 가운데 `2026-09-16`·`09-15`·`09-14` 등이 「· 학습함」 없이 날짜만으로 렌더됨 |

⛔ **새로 재지 않고 이미 있던 증거로 대조해 체크했음** — 고쳐진 문면이 내가 이 회차에 실제로 잰
것과 같은 것을 확인한 뒤에만 체크했고, 문면이 바뀌었다는 이유로 체크하지는 않았음.

⇒ 이 시점의 상태: `TS-23` `Done` · `TS-24` `Blocked`(AC#1) · `TS-25` `Done` · `TS-26` `Done`.
⚠️ **그 뒤 §11 이 `TS-24` 의 차단을 풀었음 — 최종 상태는 §11 끝에 있음.**

⚠️ **`TS-23`·`TS-25` 두 파일의 커밋은 이 회차가 하지 않았음.** 그 파일에는 호출 세션의 AC 문면
수정과 내 AC 체크가 **섞여** 있고, 같은 파일이라 경로로 가를 수 없음 — 내가 커밋하면 다른
세션의 변경을 삼키는 일이 됨(§5-1 의 「경로를 열거해도 닫히지 않는다」가 그 형태임). 그래서
미커밋으로 두고 그 세션에 넘겼음.

⚠️ `TASK-231` 의 AC(「문면이 구현 계약과 일치하도록 고쳐졌거나 …」)는 이 수정으로 **충족됐음.**
등록한 쪽이 나라 그 사실을 여기 적되, 그 태스크의 상태 전환은 그 파일을 소유한 세션에 맡겼음.

## 11. `TS-24` AC#1 의 차단이 풀렸음 — 승인 뒤의 2차 측정

⛔ **§6-1 은 차단 상태의 기록이고 이 절이 그 뒤를 가짐.** §6-1 을 지우지 않는 이유는 「무엇이 막고
있었는가」가 다음 회차의 값어치이기 때문임.

`06:11 UTC` 에 호출 세션이 `users.timezone` 임시 변경을 **승인**했음(조건 넷). 그 승인으로 AC#1 을
축 둘로 갈라 닫았음. **유료 호출은 0건이고 워커를 켜지 않았음** — `llm_calls` 가 `34 → 34` 로
움직이지 않았음.

### 11-1. 워커 없는 우회가 판별력을 주는가 — 먼저 재서 「아니다」를 확인했음

호출 세션이 먼저 보라고 한 우회임: 표에 행을 직접 심어 읽기 경로를 재는 것. **행을 심는 것만으로는
가리지 못함**을 추측이 아니라 측정으로 확인했음.

```
select current_date, (now() at time zone 'Asia/Seoul')::date, (now() at time zone 'Pacific/Midway')::date
→ 2026-09-19 | 2026-09-19 | 2026-09-18
```

`current_date`(UTC) 와 `Asia/Seoul` 날짜가 **같으므로**, 그 상태에서 어떤 행을 심어 읽어도 두 구현
(컬럼을 읽음 · `current_date` 를 읽음)이 **같은 답**을 냄. 즉 그 검사는 아무것도 배제하지 못함.
`Pacific/Midway`(UTC-11) 는 날짜가 갈리므로 판별력을 줌 — 그것이 타임존 변경이 필요한 이유임.
⇒ **우회는 「워커를 안 켠다」에는 성공하고 「타임존을 안 바꾼다」에는 실패함.** 그래서 둘을 섞어
씀: 행은 직접 심고(워커 0) 판별력은 타임존 변경으로 만듦(유료 호출 0).

### 11-2. 축 B — 공유 상태를 건드리지 않고 함정 구간을 재현함 · 통과

`ended_at = 2026-09-18 15:30:00+00` 인 완료 세션 1건을 심었음. 그 값은 **함정 구간
(`15:00~24:00 UTC` = `00:00~09:00 KST`)** 안이고, UTC 날짜는 `2026-09-18` · KST 날짜는
`2026-09-19` 임(DB 로 직접 확인함).

| | 심기 전 | 심은 뒤 |
|---|---|---|
| `completed_today` | `false` | **`true`** |
| `completed_scenarios` | `0` | **`1`** |

`ended_at::date`(UTC) 를 썼다면 `2026-09-18` 이 오늘(`2026-09-19`) 과 달라 **세지 않았을 것임.**
증거: `evidence/30-axisB-before.json` · `evidence/31-axisB-after.json`.

### 11-3. 축 A — 오류 요약의 「오늘」이 컬럼에서 오는가 · 통과

`daily_error_summary` 에 판별용 행 **둘**을 심었음 — `2026-09-19`(발생 91건) 과
`2026-09-18`(발생 82건). 한 행만 심으면 「그 행을 골랐다」와 「그 날짜를 골랐다」를 가를 수 없음.

| `users.timezone` | `current_date` | 응답 `summary_date` | 응답 `occurrence_count` | `patterns[0].target_form` |
|---|---|---|---|---|
| `Asia/Seoul` (원값) | `2026-09-19` | `2026-09-19` | `91` | — |
| `Pacific/Midway` | `2026-09-19` (그대로) | **`2026-09-18`** | **`82`** | **`Midway 날짜 쪽`** |
| `Asia/Seoul` (복원) | `2026-09-19` | `2026-09-19` | `91` | — |

⇒ `current_date` 가 `2026-09-19` 로 **고정된 채** 응답이 `2026-09-18` 로 옮겨 갔음. 즉 「오늘」의
정본은 **`users.timezone` 컬럼**이고 `current_date` 가 아님. R13-3 이 요구한 것이 성립함.

`/api/history` 도 같은 창에서 `days[0].day` 가 `2026-09-18` 로 함께 움직였음 — 두 엔드포인트가
**같은 「오늘」** 을 씀(`api/daily.py` 가 타임존을 한 번만 읽어 넘기는 규약의 관측된 결과임).

### 11-4. 승인 조건 넷을 어떻게 지켰나

| 조건 | 지킨 방법 |
|---|---|
| ① 원값을 먼저 읽어 남김 | 스크립트 첫 단계가 `select timezone from users` 임. `Asia/Seoul` 을 얻고 **예상과 다르면 중단**하게 두었음(다른 세션이 이미 바꿨을 경우) |
| ② 복원을 «읽어» 확인 | `update` 뒤 `select` 로 다시 읽어 `Asia/Seoul` 을 확인하고, **엔드포인트가 `2026-09-19`/`91` 로 돌아온 것도** 확인했음. 명령 성공을 근거로 쓰지 않았음 |
| ③ `ALTER DATABASE`·`ALTER SYSTEM` 금지 | 쓰지 않았음. 범위는 `ohmyenglish.users.timezone` **행 값 하나**임. StockAgent 가 보는 `current_date` 는 건드리지 않았음 |
| ④ 저장된 `timestamptz` 를 변환해 UPDATE 하지 않음 | `timestamptz` 컬럼을 한 번도 `update` 하지 않았음. 바꾼 것은 `text` 컬럼 하나임. 날짜 변환은 **읽을 때만** 일어났음 |

⚠️ **공유 상태 노출 창을 적음**: `06:11:57.092909` → `06:11:57.38278` = **290밀리초**.
변경·판정·복원을 한 스크립트에 넣어 사이에 사람의 판단이 끼지 않게 했음
(`evidence/36-driver-axisA-timezone.sh` 가 그 스크립트임).

### 11-5. 그래도 재지 못한 절반 — 정적 대조만 남김

`daily_error_summary` 의 **쓰기** 경로(`_UPSERT_SQL`) 는 분석 워커에만 있어 지나가지 않았음.
⛔ **정적 대조는 측정이 아니므로 그렇게 표시함.** 읽은 것 둘:

- 그 SQL 의 앵커가 `(u.created_at at time zone $3)::date` 이고 `$3` 를
  `timezone_of(conn, user_id)` 가 채움 (`services/daily_summary.py:55`·`:70`·`:304`).
- `daily_summary.py`·`weekly_report.py`·`learner_time.py` 의 `current_date` 언급 4건은
  **전부 「쓰지 않는다」는 주석**이고 실제 사용은 0건임. 세어만 보고 「0건」이라 적지 않고 4건
  전문을 열어 확인했음(`evidence/33`).

⇒ 쓰기 경로를 실제로 지나가는 것은 **워커를 켜는 회차의 몫**임. 이 회차는 그것을 했다고 적지 않음.
...
| 시나리오 | 상태 | AC |
|---|---|---|
| `TS-23` A5 다음 세션 계획 | `Done` | 3/3 |
| `TS-24` A6 일일 요약·완료 판정 | `Done` | 3/3 |
| `TS-25` A7 연속 학습일·히스토리 | `Done` | 3/3 |
| `TS-26` A8 주간 리포트 | `Done` | 3/3 |

**AC 12개 전부 통과 · 실패 0건 · 차단 0건.** 프로그램이 기대와 다르게 동작한 자리는 이 회차에서
관측되지 않았음.

⚠️ **「전건 통과」가 「전부 검증됐다」는 뜻이 아님.** 이 회차가 지나가지 못한 경로 둘을 남겨 둠:
`daily_error_summary` 의 쓰기 경로(§11-5) 와 `summarize_week` job 의 처리 절반(§5-7). 둘 다
분석 워커가 켜져야 지나가고, 이 회차는 그것을 했다고 적지 않음.

⛔ **코드를 한 줄도 고치지 않았음.** 심은 행은 전부 되돌렸고 표 행수와 네 엔드포인트 응답이 회차
시작 값과 같음. `users.timezone` 은 원값 `Asia/Seoul` 임.
